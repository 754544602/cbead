		...
		$.ajax({
			...
			success: function(data) {
				if(data.flag == 0) {
					...
					$('.plan-table thead tr').append("<th width='30px'>序号</th><th class='class-theme'>培训主题</th>");
					for(let i = 0; i < data.data.length; i++) {
						$('.plan-table tbody').append('<tr><td style="text-align:center;width:30px">'+(i+1)+'</td><td class="class-theme-td">' + data.data[i].projectName + '</td></tr>'); //渲染培训主题
					}
					...
					$.each(data.data, function(i, n) {
						...
						//开始与结束之间增加线
						for(let k = $('tbody tr').eq(i).children('.start-state').index()-2; k < $('tbody tr').eq(i).children('.end-state').index()-1; k++) {
							if(n.projectStates == '1') {
								...
								$('tbody tr').eq(i).children('.state-day').eq(k).addClass('sur-use line').data("classData", n);
							}
						}
					});
					//在院人数
					$.each($('.plan-table thead .state-day'), function(i, n) {
						...
						for(let j = 0; j < data.data.length; j++) {
							...
							if(classData != undefined) {    /*&& classData.projectStates != '1'*/
					...
					$('.line').tooltip({
						...
						content: function() {
							...
							var showBox = '<table class="tip-table" style="max-width:500px">' +
								...
								'<tr><td class="tip-lable" width="80px">班主任：</td><td>' + line.projectHeadmasterName + '</td></tr>' +
						...
						onShow: function() {
							...
							var maTop = ($(this).height()-22)/2*-1;
							if(!borderState){
								maTop = ($(this).height()-22)/2;
							}
							console.log(maTop)
							$(this).tooltip('tip').css({
								backgroundColor: '#666',
						...
						$('.plan-table2').css('margin-top',($('.state-day').width()-30)+'px');
						$('.index-plan').css('height',($('.plan-table2').height()+150)+'px');
						!!window.ActiveXObject || "ActiveXObject" in window?$('.class-theme').css('width',($('.class-theme-td').width())+'px'):$('.class-theme').css('width',($('.class-theme-td').width()+1)+'px')
				} else {
					setTimeout($yt_baseElement.hideLoading(), 500);
				}
			}

var beforeApproList = {
					...
					if(datas.length > 0) {
						...
						$.each(datas, function(i, n) {
							trStr += '<tr>' +
								...
								'<td style="text-align: right;">' + (n.advanceAmount == "" ? "--" : $yt_baseElement.fmMoney(n.advanceAmount)) + '</td><td>' + n.applicantTime + '</td>' +
	...
	toDetails: function() {
		$('.yt-link.to-detail').off("click").on('click', function() {
			...
			/*页面跳转打开新页面*/
			var pageUrl = "view/system-sasac/expensesReim/module/beforehand/beforeApplyDetail.html?processInstanceId=" + processInstanceId+"&advanceId="+advanceId;//即将跳转的页面路径
			var goPageUrl = "view/system-sasac/expensesReim/module/approval/beforehandApproveList2.html";//左侧菜单指定选中的页面路径
			...
			success: function(data) {
				...
				if(data.flag == 0) {
					if(datas.length > 0) {
						...
						$('.page2').hide();
						//拼接暂无数据效果
						htmlTbody.append(beforeApproList.noDataTrStr(8));
					}
				}
	...
	goApprovePage: function() {
		...
		$(".apprv-btn").off().click(function() {
			...
			if(state == 'activitiStartTask') {
				...
				pageUrl = 'view/system-sasac/expensesReim/module/busiTripApply/priorApproval.html?advanceId=' + advanceId;
			}
			/**
			 * 调用显示loading方法
			 */

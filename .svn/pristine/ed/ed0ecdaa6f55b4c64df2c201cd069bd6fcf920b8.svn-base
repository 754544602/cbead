var apply = {
	...
	init: function() {
		//点击取消返回上一页
		$(".yt-cancel-btn").click(function(){
			window.history.back();
		});
		//信息边栏添加显示类
		$(".info-sidebar").addClass("bottom-hidden-div");
		//点击信息边栏
		$(".sidebar-head").off().on().click(function(){
			if($(this).hasClass("bottom-hidden-div")){
				$(".info-sidebar").addClass("bottom-hidden");
				$(this).removeClass("bottom-hidden-div");
			}else{
				$(".info-sidebar").removeClass("bottom-hidden");
				$(this).addClass("bottom-hidden-div");
			}
		});
		//生成审批流程页面
	...
	events: function() {
		...
		$("#approveSubBtn,#approveSaveBtn").on("click", function() {
			...
			if(thisBtn.hasClass("save-btn")) {
				ajaxUrl = "http://192.168.1.38:15656/sasac-business/budget/prjSummaryApp/saveBudgetPrjSummaryAppInfo";
			};
			//判断是否验证成功
			if(validFlag) {
				//获取表单数据
						...
						if(data.flag == 0) {
							...
							if(thisBtn.hasClass("save-btn")) {
								...
								$yt_common.parentAction({
									...
									data: {
										url: 'view/system-sasac/budget/module/projectApproval/budgetDraftsList.html' //要跳转的页面路径  
									}
								});
							}
	...
	applicationApply: function() {
		if(apply.budgetPrjAppId != "" && apply.budgetPrjAppId != undefined && apply.budgetPrjAppId != null) {
			...
			$.ajax({
				type: "post",
				url: 'http://192.168.1.38:15656/sasac-business/budget/prjSummaryApp/getBudgetPrjSummaryAppInfo',
				async: false,
				data: {
					ids: "14,9"
				},
				success: function(respond) {
					...
					if(respond.flag == 0) {
						...
						//.yearGoalId = data.yearGoalInfo.goalId;
						//调用拼接附件方法

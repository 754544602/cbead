var indexObj = {
	...
	setAgendaItems:function(){
		...
		//进入加载第一页数据
		indexObj.getMyToDoInfoListPageByParams(indexObj.pageIndex, '', false, function(total){
			//首次加载调用总数
		...
		$('.backlog-model').on('click', '.approve', function(){
			...
			if (taskKey == 'activitiStartTask') {
				if (code == 'ADV_APP_FLOW'||code == 'TRAVEL_APP_FLOW'||code == 'HYSQ_APP_FLOW'||code == 'JDSQ_APP_FLOW') {
					...
					urlStr = urlStr + 'view/system-sasac/expensesReim/module/reimApply/expenseDetail.html?appId=' + id;
				...
				if (code == 'ADV_APP_FLOW'||code == 'TRAVEL_APP_FLOW'||code == 'HYSQ_APP_FLOW'||code == 'JDSQ_APP_FLOW') {
					...
					urlStr = urlStr + 'view/system-sasac/expensesReim/module/approval/loanApproval.html?loanId=' + id;
				} else if (code == 'CPRJ_APP_FLOW') {

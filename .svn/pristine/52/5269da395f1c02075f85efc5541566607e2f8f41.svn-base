var reimFormPreview = {
	...
	init: function() {
		//调用顶部功能按钮操作事件
		reimFormPreview.heardFunBtnEvent();
		//获取页面跳转传输的参数对象
		var requerParameter = $yt_common.GetRequest();
		// 获取传输的项目ID
		reimFormPreview.reimId = requerParameter.reimId;
		//获取当前日期
		$("#createFormDate").text(sysCommon.getNowDate());
		//调用获取报销单信息方法
		reimFormPreview.getReimInfo();
	...
	getReimInfo:function(){
		var qrCodeContextUrl =  $yt_option.websit_path + 'view/system-sasac/expensesReim/module/reimApply/reimApprove.html?reimId='+reimFormPreview.reimId;
		 $.ajax({
		 	type:"post",
		 	url:"sz/reimApp/getReimPreviewDetailByReimAppId",
		 	async:false,
		 	data:{
		 		reimAppId:reimFormPreview.reimId,
				previewFalg:true,

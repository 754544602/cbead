(function($, window) {
	var serveFunds = {
		...
		init: function() {
			...
			if(pid) {
				//参数存在时获取对应的详情数据
				ts.getSpecialExpenditureAppInfoByAppId(pid);
			} else {
				//获取费用类型
				ts.getCostTypeList();
				//所属预算项目
			...
			//调用获取审批流程数据方法
			sysCommon.getApproveFlowData("SZ_SPECIAL_EXPENDITURE_APP");
			//当前登录人code
						...
						if(verifyBudgetExpendSum) {
							...
							if($('.prior-approval').val()) {
								...
								if(total > advan) {
									...
									//保存方法
									me.submitSpecialExpenditureAppInfo(data, thisBtn);
								}
							} else {
								//保存方法
								me.submitSpecialExpenditureAppInfo(data, thisBtn);
		...
		setSaveEvent: function() {
			...
			$('#savePayment').on('click', function() {
				...
				var data = me.getSubData();
				//保存方法
				me.submitSpecialExpenditureAppInfoToDrafts(data, thisBtn);
			});
		},
		/*
		 * 
		...
		getCostTypeList: function(code) {
			$.ajax({
				...
				async: true,
				data: {
					formAppCode: 'SPECIAL_EXPENDITURE_APP'
				},
				success: function(data) {
					//获取数据list
					var list = data.data || [];
		 ...
		 */
		submitSpecialExpenditureAppInfo: function(subData, thisBtn) {
			var me = this;
			//禁用并移除所有的事件
			thisBtn.off();
			$.ajax({
				type: "post",
				url: "sz/specialExpenditureApp/submitSpecialExpenditureAppInfo",
				async: true,
				data: subData,
				success: function(data) {
					if(data.flag == 0) {
		...
		submitWorkFlow: function(subData, thisBtn) {
			$.ajax({
				type: "post",
				url: "sz/specialExpenditureApp/submitWorkFlow",
				async: false,
				data: subData,
				success: function(data) {
					$yt_alert_Model.prompt(data.message);
		 ...
		 * @param {Object} subData
		 */
		submitSpecialExpenditureAppInfoToDrafts: function(subData, thisBtn) {
			$.ajax({
				type: "post",
				url: "sz/specialExpenditureApp/submitSpecialExpenditureAppInfoToDrafts",
				data: subData,
				success: function(data) {
					$yt_alert_Model.prompt(data.message);
					if(data.flag == 0) {
		 ...
		 */
		getSpecialExpenditureAppInfoByAppId: function(specialExpenditureAppId) {
			var me = this;
			$.ajax({
				type: "post",
				url: "sz/specialExpenditureApp/getSpecialExpenditureAppInfoByAppId",
				data: {
		...
		getSubData: function() {
			...
			return {
				specialExpenditureAppId: $('#specialExpenditureAppId').val(), //specialExpenditureAppId	专项支出表id
				specialExpenditureAppName: $('#specialExpenditureAppName').val(), //specialExpenditureAppName	专项支出事由
		...
		showDetail: function(d) {
			...
			$('#specialExpenditureAppId').val(d.specialExpenditureAppId); //specialExpenditureAppId	专项支出申请表id
			$('#specialExpenditureAppNum').val(d.specialExpenditureAppNum); //specialExpenditureAppNum	专项支出单号
			$('#formNum').text(d.specialExpenditureAppNum);
			$('#specialExpenditureAppName').val(d.specialExpenditureAppName); //reimAppName	报销事由
			//costType	费用类型
		 ...
		 * 一般费用   funeven
		 */
		showSocialSecurityFun: function() {
			//清空原有代码
			$('.index-main-div').html('');
			//提示文字隐藏
			$('.qtip-text-div').hide();
			...
			$('.approve-div,.else-div').show();
			//加载区域页面
			sysCommon.loadingWord('view/system-sasac/expensesReim/module/special/costDetail.html');
		...
		setCostTrainApplyInfoList: function(list, exports) {
			...
			var total = 0;
			$.each(list, function(i, n) {
				html += '<tr class="' + (exports ? 'exports' : '') + '" costnameval="' + n.trainType + '" pid=""><td class="cost-name">' + n.trainTypeName + '</td><td class="moneyText standard-money">' + ($yt_baseElement.fmMoney(n.standard)) + '</td><td class="people-num">' + n.trainOfNum + '</td><td class="day-num">' + n.trainDays + '</td><td class="moneyText smallplan-money">' + serveFunds.fmMoney(n.averageMoney) + '</td><td class="special-instruct">' + n.remark + '</td><td><input type="hidden" class="popM" value="4"><span class="operate-update"><img src="../../../../../resources-sasac/images/common/edit-icon.png"></span><span class="operate-del"><img src="../../../../../resources-sasac/images/common/del-icon.png"></span></td></tr>';
		...
		clearPriorApproval: function() {
			...
			switch(costType) {
				case 'SOCIAL_SECURITY_FEE': //社会保障费
					break;
				case 'ACCRUED_TAX': //应交税费
					break;
				case 'LABOUR_UNION_FUNDS': //工会经费
					break;
				case 'PARTY_BUILDING_FUNDS': //党建经费
					break;
				case 'OTHER_EXPENSES': //其他费用
					break;
				default:
					break;
			}

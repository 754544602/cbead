(function($, window) {
	var serveExamine = {
		...
		init: function() {
			var ts = this;
			$('.approve-div').html(sysCommon.createFlowApproveMode());
			//加载区域页面
			sysCommon.loadingWord('view/projectManagement-sasac/expensesReim/module/busiTripApply/serveTrainApproval.html');
			var pid = $yt_common.GetQueryString('appId');
			var fun = $yt_common.GetQueryString('fun');
			ts.getAdvanceUpdateAppInfoDetailByAdvanceAppId(pid);
			ts.start();
			ts.events();
		},
		/**
		...
		events: function() {
			...
			$('#submitPayment').on('click', function() {
				...
				if($yt_valid.validForm($('.approve-div'))) {
					...
					//获取提交数据
					var d = {
						appId: me.saveData.advanceAppId, //appId	表单申请Id
						parameters: '', //parameters	JSON格式字符串, 
						processInstanceId: me.saveData.processInstanceId, //processInstanceId	流程实例ID, 
					}
		...
		showDetail: function(d) {
			var me = this;
			//advanceAppId	事前申请Id
			$('#formNum').text(me.non(d.advanceAppNum));
			$('#advanceAppNum').text(me.non(d.newAdvanceAppNum)); //advanceAppNum	事前申请单号
			...
			$('#jobName').text(me.non(d.applicantUserJobName)); //applicantUserJobLevelName	申请人职务名称
...
//			if(data.trainApplyInfoList.length==0 && data.costTrainApplyInfoList.length == 0 && data.costTeachersFoodApplyInfoList.length == 0 && data.costTeachersLectureApplyInfoList.length == 0 && data.costTeachersTravelApplyInfoList.length == 0 && data.costTeachersHotelApplyInfoList.length == 0) {
...
//			}
		//师资-培训信息json
				...
				me.setCostList(data.projectBudgetList,1);//变更前
				me.setCostList(data.projectBudgetDetails,2);//变更后
		},
		/**
		 ...
		 * type  1变更前 2变更后
		 * */
		setCostList:function(list,type){
			var html = '';
			var tBody1,tBody2
			if(type==1){
				tBody1 = $('#teachBeforeTable tbody');
				tBody2 = $('#trainingBeforeTable tbody');
			}else if(type==2){
				tBody1 = $('#lectureFeeTable tbody');
				tBody2 = $('#trainingFeeTable tbody');
			}
			tBody1.empty();
			tBody2.empty();
			$.each(list, function(i, n) {
			...
			if(list.length != 0) {
				$.each(list, function(i, n) {
					html += '<tr pid="' + n.trainAppId + '" trainType="' + n.trainType + '">' +
						...
						'<td class="standard">' + serveExamine.fmMoney(n.standard) + '</td>' +
		...
		submitReimAppInfo: function(subData, thisBtn) {
			$.ajax({
				...
				success: function(data) {
					...
					if(data.flag == 0) {
						//跳转列表页面
						window.location.href = $yt_option.websit_path + 'view/projectManagement-sasac/expensesReim/module/approval/projectBefList.html?state=1';
					}
					thisBtn.attr('disabled', false).removeClass('btn-disabled');
				}
			});

...
$(function(){
	...
	if(requerParameter){
		//获取传输的项目ID
		tableIdObj = requerParameter.tableOjbData;
		//帐套Id
		configCommonObj.booksId = requerParameter.booksId;
		//阶段code
	    configCommonObj.budgetStage = requerParameter.budgetStage;
		if(tableIdObj){
			tableIdObj = JSON.parse(tableIdObj); 
		}
	}
	//调用tab切换方法
	switcherTab();
    ...
    $("button.page-retu-btn").click(function(){
    	   window.location.href= $yt_option.websit_path+'view/system-sasac/budget/module/expenditure/budgetTableStyle.html?booksId='+configCommonObj.booksId+'&budgetStage='+configCommonObj.budgetStage;
    	   /*$yt_common.parentAction({
			url:$yt_option.parent_action_path,//父级中转地址,固定配置项,只有统一修改处理。
			funName:'locationToMenu',//指定方法名，定位到菜单方法
			data:{
				url:'view/system-sasac/budget/module/expenditure/budgetTableStyle.html'//要跳转的页面路径
			}
		});*/
    });
    //配置按钮点击事件
    $("#createConfigTableDiv").on('click','th.indicate:not(.dis)',function (){
    	...
    	$(".rule-table td").removeClass("td-shadow cell-shadow").removeAttr("style");
    ...
    //保存表配置
    $(".table-btn .save-config").click(function(){
    	...
    	$.ajax({
    		type:"post",
    		url:"budget/table/saveTableFilterInfo",
    		async:false,
	...
	$("#code-list .yt-common-btn").click(function (){
		if($(".code-list .yt-table-active").length==0){
			...
			//获取表格选中的字典编码code
			var selCode = $(".code-list .yt-table-active td:eq(0)").text();
			//调用查询字典编码数据方法
			...
			if(codeDatas.length > 0 && codeDatas !=undefined && codeDatas!=null){
				$.each(codeDatas, function(i,n) {
					if(selCode !="" && selCode == n.code){
						numOptions+='<option value="'+n.code+'" selected="selected" sel-val="'+ n.name+'">'+n.code+' '+n.name+'</option>'
					}else{
						numOptions+='<option value="'+n.code+'" sel-val="'+ n.name+'">'+n.code+' '+n.name+'</option>'
					}
				});
		            ...
		            if(codeDatas != "" && codeDatas.length>0){
		            	$.each(codeDatas,function(i, n) {  
			                ...
			                if(n.name.indexOf(text) != -1) {  
			                    thisSel.append('<option value="'+n.code+'" sel-val="'+ n.name+'">'+n.code+' '+n.name+'</option>');  
			                }else if(n.code.indexOf(text) != -1){//编码
			                	thisSel.append('<option value="'+n.code+'" sel-val="'+ n.name+'">'+n.code+' '+n.name+'</option>'); 
			                }
			            });  
		            }
	...
	//获取单位编码的预算属性
	//getConfigData.getAttrList($('#unitCodeMark'),'UNIT_NUM');
	//获取单位编码下拉数据
	getConfigData.getUnitOpts();
	//单位编码的放大镜点击事件
	/*$('.unit-code-search').off('click').on('click',function(){
		if($('.unit-code-row').find("select.attributes").val() == "0") {
			$yt_alert_Model.prompt("请选择引用字典/预算属性");
			return false;
		}
		$('.unit-code-row').find(".search-img").parent().prev().find(".word-num-sel").addClass("select-code");
		//显示字典编码弹出框
		sysCommon.showModel($("#code-list"));
		$("#code-list .yt-search-btn").prev().val("");
		getConfigData.getCodeList($('.unit-code-row'));
	});*/
});
/**
 * 
...
function switcherTab() {
		var index = 1;
		//$('.tab-nav:eq(1)').addClass('active-nav').siblings('.tab-nav').removeClass('active-nav');
 ...
 * 
 */
function switcherTabCopy() {
	/*$('.tab-nav').off().on('click', function() {
		var index = $(this).index();
		$(this).addClass('active-nav').siblings('.tab-nav').removeClass('active-nav');
		$('.tab-content').eq(index).css("display", 'block').siblings('.tab-content').hide();
		//清空过滤表配置下拉列表和输入框
		$(".table-config-panel select").niceSelect();
		//表名
		$(".table-config-panel select").each(function(){
			$(this).find("option:eq(0)").prop("selected","selected");
		});
		$(".table-config-panel input").val("");
		//调用根据选中的预算表查询数据表过滤配置数据
		var configTab = $("#table-list .table-active");
...
var getConfigData = {
	...
	getTableData :function (){
		...
		//显示操作按钮
		$(".save-config,.canel-config").show();
		$(".tab-content:eq(1) .table-name").text(tableIdObj.tableName);
		$(".table-config-panel").show().next().hide();
		//清除行、列配置区域显示
		$(".row-config-panel .model-content,.column-config-panel .model-content").hide().next().show();
	...
	createAttrList:function (obj,configData,haveVal){
		...
		var attrDoc = '<div class="box-row">'+
	  				...
	  				'<a onclick="getConfigData.attrEvent(this)" style="margin-left: 30px;" class="yt-link del-attr"><img src="'+getConfigData.delIconUrl+'"/></a>'+
	  				'</div>'+
	  	    	'</div>';
	  	    	attrDoc = $(attrDoc);
	  	    	//初始化下拉列表
        ...
        backFunction: function(text) {  
	            ...
	            if(codeDatas != "" && codeDatas.length>0){
	            	$.each(codeDatas,function(i, n) {  
	            		//名称
		                if(n.name.indexOf(text) != -1) {  
		                    attrDoc.find("select.word-num-sel").append('<option value="'+n.code+'" sel-val="'+ n.name+'">'+n.code+' '+n.name+'</option>');  
		                }else if(n.code.indexOf(text) != -1){//编码
		                	 attrDoc.find("select.word-num-sel").append('<option value="'+n.code+'" sel-val="'+ n.name+'">'+n.code+' '+n.name+'</option>'); 
		                }
		            });
	 ...
	 */getAttrList:function(ele,selectData){
		...
		if(ele.parents(".row-config-panel").length>0){
			...
			var tableConfig = $(".table-config-panel .table-name").data("tableConfig");
			//行的
			if(tableConfig.r){
				optionStr +=',';
	...
	getTableConfig:function (tableDoc,configData){
		$.ajax({
			type:"post",
			url:"budget/table/getTableFilterMapByParams",
			async:false,
			...
			success:function (data){
				if(data.flag == 0){
					...
					//存储配置信息
					$(".table-config-panel .table-name").data("tableConfig",data.data);
					//不可编辑的数据列表
			...
			success:function (data){
				if(data.flag == 0) {
                  if(data.data.length > 0){
                    $.each(data.data, function(i,n) {
                		if(n.type == "r"){
                			$(".rule-table tbody tr .indicate").eq(n.orderNum - 1).data("configData", n);
                			//给配置成功的添加样式
                			$(".rule-table tbody .indicate").each(function(){
                			...
                			//判断是否是否是第26张表,进行特殊处理
                			/*if(configData.tableCode == "T26"){
                				//比对行
                				$(".rule-table tbody tr:not(.flag-tr)").each(function(){
                					if(crNum[1].toUpperCase() == "R"+$(this).index()){
                						//比对列
                						$(this).find("td").each(function(i,t){
                							//比对列
                							if("C"+($(t).index()+1) == crNum[0].toUpperCase()){
                								var tickImg = '<img src="../../../../../resources-sasac/images/common/config.png" alt="" class="tick-img">';
	                							$(t).data("crConfigData",n).html(tickImg);
                							}
                						});
                					}
                				});
                			}else{}*/
                			var configIndex = $(".rule-table .indicate-head .no-th-sty").length;
                			//比对行
                			var tdNum = "";
                			$(".rule-table tbody tr .indicate").each(function(i, r) {
                				//找到对应的行
                		 	...
                		 	//给配置成功的添加样式
                		 	$(".rule-table .indicate-head .indicate").each(function(){
                		 		if($(this).text() == n.crNum.toUpperCase()){
                		 			$(this).addClass("config-th-sty");
                		 		}
                		 	});
		...
		var htmlStr = '<div class="model-content dn">'
		            ...
		            if(flag == "c"){
		            	htmlStr+='<div class="box-row">'
		            	        ...
		            	        +'<div class="box-content box-lable1" style=""><span style="letter-spacing: 15px;">所属</span>表：</div>'
		            	...
		            	htmlStr+='<div class="box-row">'
		            	        +'<div class="box-content box-lable1" style="width: 90px;"><span style="letter-spacing: 15px;">所属</span>表：</div>'
		            	        +'<div class="box-content table-name"></div>'
		            ...
		            htmlStr +='<div class="box-row">'
		                    +'<div class="box-content box-lable" style="float: left;display: inline;">属性：</div>'
		            		...
		            		+'</div>'
					        +'</div>'
					        +'<div class="box-row property-val-model" style="display: none;">'
		            		+'<div class="box-content box-lable1 formual dn" style="width: 132px;">'
					        ...
					        +'</div>';
					htmlStr+='<div class="config-btngroup" style="padding-bottom: 0px;">'
	    ...
	    */
	     $(".config-btngroup .tb-save-btn").attr('disabled', false).off('click').on('click', function(){
			...
			if(getConfigData.configDatas){
				...
				$.ajax({
					type:"post",
					url:"budget/table/saveTableCongfigDetail",
					async:false,
					...
					success:function (data){
						...
						//刷新列表配置信息
						var configTab = $("#table-list .table-active");
						//getConfigData.getConfigDataList(configTab.data("tableData"));
			        ...
			        backFunction: function(text) {  
				            ...
				            if(datas != "" && datas.length>0){
				            	$.each(datas,function(i, n) {  
				            		//名称
					                if(n.name.indexOf(text) != -1) {  
					                    $('#unitCode').append('<option value="'+n.code+'" sel-val="'+ n.name+'">'+n.code+' '+n.name+'</option>');  
					                }else if(n.code.indexOf(text) != -1){//编码
					                	 $('#unitCode').append('<option value="'+n.code+'" sel-val="'+ n.name+'">'+n.code+' '+n.name+'</option>'); 
					                }
					            });  
...
var  tableRelaCalcuObj ={
	 ...
	 */calcuTableInit:function(thisInput,configSelCell){
		...
		//点击确定按钮
		$("button.calcu-sure-btn").click(function(){
			var  validFlag = true;
			//判断计算公式是否有数据
			if($("#calcuInput").val() == ""){
	...
	getPropertyTabInfo:function(){
		//调用获取相同属性的表格
	 	$.ajax({
			type: 'post',
			url:'budget/table/getSameAttrTabListByTableId',
			data:{

...
var planeListInfo = {
	...
	init: function() {
		...
		$('.valid').click(function() {
			var pkId = "";
			//有效数据
			var effData=$('.select-teacher-checkbox.check');
			//无效数据
			...
			if(effData.length==0){
				...
				$yt_alert_Model.prompt("请选择无效列表进行标记");
			}
		...
		$('.novalid').click(function() {
			...
			$.each($('.select-teacher-checkbox.check'), function(c, z) {
				...
				if(invData.length==0){
					...
					$yt_alert_Model.prompt("请选择有效列表进行标记");
					$('.invalid-tbody tr.yt-table-active').removeClass('yt-table-active');
				}
		});
		//删除
		$('.deletedata').click(function() {
			var pkId = [];
			//对账单信息条数
			var effdata=$('.project-tbody').attr('effdata');
			$.each($('.select-teacher-checkbox.check'), function(c, z) {
...
//				},
				success: function(data) {
					if(data.flag == 0) {
						...
						htmlTbody.attr('effData',data.data.rows.length);
						var htmlTr = '';
						var htmlRightTr = '';
						var num1 = 0;
						htmlTbody.empty();
							...
							$.each(data.data.paymentList, function(j, n) {
								...
								$.each(data.data.rows, function(i, v) {
									if(v.identification == n.identification) {
										n.differencetotal = Number(n.differencetotal) - Number(v.totalAmount);
										numb += 1;
										if(numb == 1) {
											num1 += 1;
											htmlTr = '<tr>' +

var personnelFunds = {
	...
	//获取类名为.hoursInput的对象集合
		var hoursInputList = $(".hoursInput");
		//遍历集合并设置金额格式
		hoursInputList.each(function() {
			var thisObj = $(this);
			if(thisObj.val()) {
				thisObj.val($yt_baseElement.fmMoney(thisObj.val(),1));
			}
			thisObj.focus(function() {
				var thisInput = $(this);
				if(thisInput.val()) {
			...
			thisObj.blur(function() {
				var thisInput = $(this);
				if(thisInput.val()) {
					thisInput.val($yt_baseElement.fmMoney(thisInput.val(),1));
				}
			});
		});
		//.弹窗的培训费其他tab中，“小计”字段变成自动计算可修改，计算公式：小计=标准*人数*天数
		$('.standard-money,.people-num,.day-num').on('blur', function() {
		  if(($('input.day-num').val() == "" || $('input.day-num').val() == "0" ) 
		  && ($('input.standard-money').val() == "" || $('input.standard-money').val() == "0") 
		  && ($('input.people-num').val() == "" || $('input.people-num').val() == "0")){
		    $('input.smallplan-money').val("0.00")
		  }else{
		    //标准
        var standard = $yt_baseElement.rmoney($('input.standard-money').val() || '1');
        //人数
        var people = Number($('input.people-num').val() || '1');
        //天数
        var day = Number($('input.day-num').val() || '1');
        //计算小计
		...
		$('.predict-standard-money,.predict-people-num').on(
				...
				function() {
					...
					$('input.predict-people-num').val($('input.predict-people-num').val() || '1');
					$('input.predict-all-money').val($yt_baseElement.fmMoney(standard * people || '0'));
				});
	...
	getData: function() {
		...
		if($("#startTimeTop")[0]){
			...
			$("#endTimeTop").calendar({
				...
				callback: function() {
					//调用公用的清除验证信息方法
					sysCommon.clearValidInfo($("#endTimeTop"));
				}
			});
		}
	...
	init: function() {
		...
		if('expenditure'==requerParameter.workCode){
			$("#predictCostDiv").hide();
			$("#preTaxDiv").hide();
		}
		//初始化下拉列表
		...
		//初始化项目列表
		personnelFunds.getCostList();
		personnelFunds.initProjectSelect();
		...
		/*
		 支出总金额计算*/
		$('#lectureFeeTable,#trainingFeeTable').on('blur','.budget-cost',function(){
			var allmoney=0;
			$('#lectureFeeTable tbody tr').each(function(i,n){
				allmoney+=$yt_baseElement.rmoney($(n).find('.budget-cost').val());
			})
			$('#trainingFeeTable tbody tr').each(function(i,n){
				allmoney+=$yt_baseElement.rmoney($(n).find('.budget-cost').val());
			})
			$('#serveAllMoney').text(allmoney);
			$('#TotalMoneyUpper').text(arabiaToChinese(allmoney + ''));
		})
		/*
		 学员人数限制数字
		 * 
		 * */
		$('.trainOfNum').on('input',function(){
...
//		});
				...
				switch(tabIndex) {
					...
					case 2:
						...
						trHtml = '<tr>' +
							...
							'<td class="text-overflow-sty" title="" data-text="remarks">' + (specialRemark == '' ? '' : specialRemark) + '</td>' +
							'<td>' +
		...
		$("#trainingFeeTable").on('click', '.operate-update', function() {
			...
			specialinstruct = specialinstruct == '' ? '' : specialinstruct;
		...
		$("#predictCostTable").on('click', '.operate-update', function() {
			...
			var predictAllMoney = $(thisTr).find(".predict-all-money").text();
			var specialinstruct = $(thisTr).find(".special-instruct").text();
			specialinstruct = specialinstruct == '' ? '' : specialinstruct;
			//隐藏input讲师val
			var lecturerNameVal = $(thisTr).find(".lectureId").val();
			...
			$('#predictCostPopM').find('.predict-all-money').val(predictAllMoney);
	...
	initProjectSelect:function(){
		if($("span#regionDesignation")[0]){
			...
			$.ajax({
				...
				async:false,
				data:{
					projectId:$yt_common.request_params?$yt_common.request_params.projectId:null
				},
				beforeSend:function(){
				},
				success:function(data){
					if(data.flag==0){
						$("#regionDesignation").empty();
						$("#regionDesignation").append('<option value="">请选择</option>');
						$.each(data.data, function(i, n) {
							$("#regionDesignation").append($('<option value="' + n.projectId + '">' + n.projectCode+'-'+n.projectName + '</option>').data('data',n));
						});
						$("#regionDesignation").niceSelect({
							search: true,
							backFunction: function(text) {
								//回调方法,可以执行模糊查询,也可自行添加操作  
								$("#regionDesignation option").remove();
								if(text == "") {
									$("#regionDesignation").append('<option value="">请选择</option>');
								}
								//遍历存储的全局用户名遍历,匹配数据,进行模糊查询操作  
								$.each(data.data, function(i, n) {
									if((n.projectCode+'-'+n.projectName).indexOf(text) != -1) {
										$("#regionDesignation").append($('<option value="' + n.projectId + '">' + n.projectCode+'-'+n.projectName + '</option>').data('data',n));
									}
								});
							}
						});
						$("#regionDesignation").off('change').on('change',function(){
							if($(this).val()!=''){
	...
	setCostList:function(data){
			...
			$.each(data, function(i, n) {
				html='<tr>' +
						...
						'</td>' +
						'<td>' +
						'<input type="text" class="yt-money-input budget-cost" data-val="budgetCost" placeholder="请输入"/>' +
						'</td>' +
						'<td>' +
						'<textarea class="yt-textarea accordingInstructions" style="height:28px;line-height:28px;padding:0 4px;width:95%" data-val="accordingInstructions" placeholder="请输入依据说明"></textarea>'+
						'</td>' +
						'</tr>';
				if(n.costType == 1) {
					$('#lectureFeeTable').append(html);
				}
				if(n.costType == 2) {
...
//				personnelFunds.hideOtherAlert();
	...
	addPredictCostList: function(tr) {
		...
		if(true) {
			...
			/*标准小计*/
			var predictAllMoney = $("#predictCostPopM .predict-all-money").val();
			/*特殊说明*/
			...
			html = '<tr pid="">' +
				...
				'<td class="moneyText predict-smallplan-money sum-pay">' + smallplanMoney + '</td>' +
				'<td class="moneyText predict-all-money" style="display:none;">' + predictAllMoney + '</td>' +
				'<td class="special-instruct">' + specialInstruct + '</td>' +
				'<td><input type="hidden" class="popM" value="' + personnelFunds.tabIndex + '"/><span class="operate-update"><img src="../../../../../resources-sasac/images/common/edit-icon.png"></span>' +
				'<span class="operate-del"><img src="../../../../../resources-sasac/images/common/del-icon.png"></span></td>' +
				'</tr>';
...
//				personnelFunds.hidePredictCostAlert();
			...
			$('#predictCostTable tbody .predict-all-money').each(function() {
				costTotalAfter += $yt_baseElement.rmoney($(this).text());
			});
			$('#predictCostTable tbody .costTotal-after').text($yt_baseElement.fmMoney(costTotalAfter));
			//调用初始化数据方法
			personnelFunds.initModel(personnelFunds.thisPopM);
			//计算总金额
			personnelFunds.updateTotalNum();
			...
			$yt_alert_Model.alertOne({
				...
				confirmFunction: function() { //点击确定按钮执行方法 
					...
					$("#predictCostTable").find("tbody tr:not(.last)").each(function() {
						sum += $yt_baseElement.rmoney($(this).find(".predict-smallplan-money").text());
					});
					$("#predictCostTable").find(".costTotal").text($yt_baseElement.fmMoney(sum));
					var costTotalAfter = 0;
					$("#predictCostTable").find("tbody tr:not(.last)").each(function() {
						costTotalAfter += $yt_baseElement.rmoney($(this).find(".predict-all-money").text());
					});
					$("#predictCostTable").find(".costTotal-after").text($yt_baseElement.fmMoney(costTotalAfter));
					//计算总金额
					personnelFunds.updateTotalNum();
				}
			});
...
//		});
	...
	initPopMTab: function() {
		...
		$("#trainingTotal").val("");
		$(".preTaxPay,.aupr").text($yt_baseElement.fmMoney(0));
	},
	/*费用申请新增和修改时公共操作*/
	costPopModelAddOrUp: function() {
		...
		var trainingPerNumber = $("#trainingPopTable .trainingPerNumber")
				.text();
		...
		$("#trainingTotal").blur(
				function() {
					...
					if (trainingTotal) {
						...
						$("#trainingFeePopM .aupr").text(
								$yt_baseElement.fmMoney(aupr))
					}
				})
	},
	//计算日期差
	...
	updateTotalNum: function() {
		...
		var afterNum = 0;
		//税后
		var aft = $('#carFeeTable,#hotelFeeTable,#dietFeeTable,#trainingFeeTable,#predictCostTable').find('.costTotal');
		//单独计算税后合计
		var aftt = $('#lectureFeeTable').find('.costTotal');
		aft.each(function() {
		...
		$("#applySumMoney").text($yt_baseElement.fmMoney(afterNum || '0')).attr('num', afterNum);
		//课酬相关税金
		...
		//var sumMoneyLower = arabiaToChinese($yt_baseElement.rmoney($("#serveAllMoney").text()) || '0');
		...
		preTaxAllMoney=(parseInt(preTaxAllMoney*100)/100).toFixed(2);
		...
		var surtax=vatSalesTax*0.12;
			...
			var moneySum = surtax + fundsTotalNum;
			$("#serveAllMoney").text($yt_baseElement.fmMoney(moneySum || '0')).attr('num', moneySum);
			var sumMoneyLower = arabiaToChinese($yt_baseElement.rmoney($("#serveAllMoney").text()) || '0');
		
		$("#中心预算余额,.total-money-up").text(sumMoneyLower);
	...
	initModel: function(thisPopM) {
		...
		$("#trainingDay").text('0');
		costPopModel.find(".aupr,.athPay").text($yt_baseElement.fmMoney('0'));
		//调用获取地址数据方法
//		personnelFunds.addressList = sysCommon.getAddressInfoList();
		personnelFunds.addressList = [];
		//调用获取热门城市方法
//		personnelFunds.hotCityList = sysCommon.getHotCityList();
		personnelFunds.hotCityList = [];
		...
		$('#oherTrainingAlert.yt-edit-alert .yt-eidt-model-bottom .yt-model-sure-btn').off().on("click", function() {
			...
			if( /*$("#trainingFeeTable .yt-tbody tr").size() < 1*/ true) {
				...
				if(isFalse) {
					//获取数据
					/*费用名称*/
					var costName = $("#predictCostPopM .cost-name option:selected").text();
					var costNameVal = $("#predictCostPopM .cost-name option:selected").val();
					/*标准*/
					var standardMoney = $("#predictCostPopM .standard-money").val();
					/*人数*/
					var peopleNum = $("#predictCostPopM .people-num").val();
					/*天数*/
					var dayNum = $("#predictCostPopM .day-num").val();
					/*小计*/
		...
		$('#predictCostPopM.yt-edit-alert .yt-eidt-model-bottom .yt-model-save-btn').off().on("click", function() {
			...
			if(isFalse) {
				...
				thisTr.replaceWith('<tr pid="">' +
					...
					'<td class="moneyText standard-money">' + standardMoney + '</td>' +
					'<td class="people-num">' + peopleNum + '</td>' +
					'<td class="moneyText smallplan-money sum-pay">' + smallplanMoney + '</td>' +
					'<td class="moneyText predict-all-money" style="display:none;">' + predictAllMoney + '</td>' +
					'<td class="special-instruct">' + specialInstruct + '</td>' +
					'<td><input type="hidden" class="popM" value="' + personnelFunds.tabIndex + '"/><span class="operate-update"><img src="../../../../../resources-sasac/images/common/edit-icon.png"></span>' +
					'<span class="operate-del"><img src="../../../../../resources-sasac/images/common/del-icon.png"></span></td>' +
	...
	lectureFeePopM: function(thisTr) {
		...
		$(personnelFunds.thisPopM.find(".afterTaxPay,.teachingHours")).blur(function() {
			...
			if(personnelFunds.thisPopM.find(".afterTaxPay").val() && personnelFunds.thisPopM.find(".teachingHours").val()) {
				...
				athPay = $yt_baseElement.rmoney(afterTaxPay) / $yt_baseElement.rmoney(personnelFunds.thisPopM.find(".teachingHours").val());
				preTaxPay = $yt_baseElement.preTaxPaymoney(afterTaxPay);
				personnelFunds.thisPopM.find(".preTaxPay").text(
            	$yt_baseElement.fmMoney(preTaxPay));
		...
		$('#costPopModel.yt-edit-alert .yt-eidt-model-bottom .yt-model-sure-btn').off().on("click", function() {
			...
			if(isFalse) {
				...
				var preTaxPay=0;
				var afterTaxPay = $yt_baseElement.fmMoney(personnelFunds.thisPopM.find(
						'.afterTaxPay').val());

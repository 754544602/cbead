				...
				$("#" + c.controlId).find(".tabD a").mouseup(function() {
					...
					if(c.dateFmt != "yyyy-MM-dd HH:mm:ss" && c.dateFmt != "yyyy-MM-dd HH:mm") {
						...
						if(!c.isMultiple){
							...
							if(n.val().split(',').indexOf(a)!=-1){
								var arr = n.val().split(',');
								arr.splice(n.val().split(',').indexOf(a),1);
								$.each($("#" + c.controlId).find(".tabD a.select"),function(x,y){
									var text = $(y).text();
									text.length==1?text='0'+text:'';
									if(a.split('-')[0]==$("#" + c.controlId).find(".currentYear").text()&&a.split('-')[1]==$("#" + c.controlId).find(".currentMonth").text().lengh==1?'0'+$("#" + c.controlId).find(".currentMonth").text():$("#" + c.controlId).find(".currentMonth").text()){
										text==a.split('-')[2]&&!$(y).hasClass('nextD')&&!$(y).hasClass('prevD')?$(y).removeClass("select"):'';
									}
								})
								$(this).removeClass("select");
								n.val(arr.join(','));
//								n.val().split(',').length>1?n.val(n.val().replace(','+a,'')):n.val(n.val().replace(a,''))
							}else{
								n.val()==''?n.val(a):n.val(n.val()+','+a);
							}
						}
					}
					if(c.dateFmt != "yyyy-MM-dd HH:mm:ss" && c.dateFmt != "yyyy-MM-dd HH:mm") {
						if(c.promHide&&!c.isMultiple) {
							$("#" + c.controlId).remove();
						}
						//回调
						c.callback();
          ...
          if(!isNaN(lowerLimit)){
				...
				for(var x = 0; x < 6; x++) {
					...
					for(var y = 0; y < 7; y++) {
						...
						if(!isNaN(lowerLimit) && lowerLimit > new Date(newDate.getFullYear(), newDate.getMonth(), j) || !isNaN(upperLimit) && new Date(newDate.getFullYear(), newDate.getMonth(), j) > upperLimit){
							...
							if(c.isMultiple){
								$.each(i,function(x,y){
									if(newDate.getFullYear() == new Date(y).getFullYear() && newDate.getMonth() == new Date(y).getMonth() && j == new Date(y).getDate()) p = "select";
								})
							}else{
							...
							if(c.isMultiple){
								$.each(i,function(x,y){
									if(newDate.getFullYear() == new Date(y).getFullYear() && newDate.getMonth() - 1 == new Date(y).getMonth() && h == new Date(y).getDate()) p = "select";
								})
							}else{
							...
							if(c.isMultiple){
								$.each(i,function(x,y){
									if(newDate.getFullYear() == new Date(y).getFullYear() && newDate.getMonth() + 1 == new Date(y).getMonth() && d == new Date(y).getDate()) p = "select";
								})
							}else{
			...
			function z(a) {
				...
				var b = w();
				c.isMultiple?b=new Date(b[0]):'';
				var d = "<table class='tabM'>";
				d += "<tr>";
			...
			/*设置时分*/
			function lowerTime(lowerLimits){
				if(lowerLimits){
					if(lowerLimits.getFullYear()==$('#'+c.controlId).find(".currentYear").text()&&lowerLimits.getMonth()+1==$("#" + c.controlId).find(".currentMonth").text()&&lowerLimits.getDate()==($("#" + c.controlId).find(".tabD a.select").text() || $("#" + c.controlId).find(".tabD a.current").text())){
						if(Number($("#" + c.controlId).find(".tH").val()) < lowerLimits.getHours()) {
							$("#" + c.controlId).find(".tH").val(lowerLimits.getHours());
						}
						if(Number($("#" + c.controlId).find(".tm").val()) < lowerLimits.getMinutes() && $("#" + c.controlId).find(".tH").val()==lowerLimits.getHours()) {
							$("#" + c.controlId).find(".tm").val(lowerLimits.getMinutes());
						}
					}
				}
			}
			function upperTime(upperLimits){
				if(upperLimits){
					if(upperLimits.getFullYear()==$('#'+c.controlId).find(".currentYear").text()&&upperLimits.getMonth()+1==$("#" + c.controlId).find(".currentMonth").text()&&upperLimits.getDate()==($("#" + c.controlId).find(".tabD a.select").text() || $("#" + c.controlId).find(".tabD a.current").text())){
						if(Number($("#" + c.controlId).find(".tH").val()) > upperLimits.getHours()) {
							$("#" + c.controlId).find(".tH").val(upperLimits.getHours());
						}
						if(Number($("#" + c.controlId).find(".tm").val()) > upperLimits.getMinutes() && $("#" + c.controlId).find(".tH").val()==upperLimits.getHours()) {
							$("#" + c.controlId).find(".tm").val(upperLimits.getMinutes());
						}
					}
				}
			}
			function start() {
				...
				$("#" + c.controlId).find(".prevMonth").mouseup(function() {
					...
					if(!isNaN(lowerLimit)){
		            	lowerTime(lowerLimits)
		          	}
					if(!isNaN(lowerLimit)){
						upperTime(upperLimit)
					}
				});
				...
				var upperLimit = !!c.upperLimit ? (Object.prototype.toString.call(c.upperLimit) === "[object String]" ? new Date(c.upperLimit) : (Object.prototype.toString.call(c.upperLimit) === "[object Function]" ? new Date(c.upperLimit()):(!!c.upperLimit.val() ? new Date(c.upperLimit.val().replace(/-/g,'/')) : NaN))) : c.upperLimit;
       			var lowerLimit = !!c.lowerLimit ? (Object.prototype.toString.call(c.lowerLimit) === "[object String]" ? new Date(c.lowerLimit) : (Object.prototype.toString.call(c.lowerLimit) === "[object Function]" ? new Date(c.lowerLimit()):(!!c.lowerLimit.val() ? new Date(c.lowerLimit.val().replace(/-/g,'/')) : NaN))) : c.lowerLimit;
				...
				if(!isNaN(lowerLimit)){
		            var lowerLimits = lowerLimit;
    				lowerLimits.setTime(lowerLimits.getTime()+24*60*60*1000);
	          	}
				//输入框获取焦点选中
				...
				dateButtonBox.find(".calendar-hms input").off("keydown").on("keydown", function(event) {
					...
					if(event.keyCode == 9) {
						if($(this).hasClass("tH")){
							$(this).siblings('.tm').select();
							$yt_common.eventStopPageaction(e);
							return false;
						}
						if($(this).hasClass("tm")){
							$(this).siblings('.tH').select();
							$yt_common.eventStopPageaction(e);
							return false;
						}
					}
			...
			n.bind("click focus",
				function(e) {
					if(/*$("#" + c.controlId + ":hidden").length != 0*/true) {
						...
						} else if(c.dateFmt == "yyyy-MM-dd"||c.dateFmt == "yyyy-MM-dd HH:mm") { //年月日  年月日时分

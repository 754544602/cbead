...
var quList = {
	...
	init: function() {
		...
		//点击问题修改按钮
		$('.title-content').off().on('click', '.content-update-name', function() {
		...
		//点击标题修改按钮
		$('.title-content').on('click', '.modify-big-title', function() {
			
			var disabled=$(this).parent().parent().find('.title-name-input').attr('disabled');
			if(disabled!=null){
				$(this).text("保存");
				$(this).parent().parent().find('.title-name-input').addClass('title-input-repair');
				$(this).parent().parent().find('.title-name-input').removeAttr('disabled');
//				$(this).parent().find('.title-name-input').blur(function(){
//					$(this).parent().find('.modify-big-title').text("修改");
//					$(this).parent().find('.title-name-input').removeClass('title-input-repair');
//					$(this).parent().find('.title-name-input').attr('disabled','disabled');
//				});
			}else{
				$(this).text("修改");
				$(this).parent().parent().find('.title-name-input').removeClass('title-input-repair');
				$(this).parent().parent().find('.title-name-input').attr('disabled','disabled');
		...
		$('.form-bottom-btn').on('click', function() {
			...
			var html = '<div style="margin-left:65px;width: 400px;padding:10px 0px;position: relative;">' +
		...
		if(queType == "【单选】") {
			...
			if(queChoose.length >= 2) {
				$('.radio-title-approval.update-choose').next().find('.que-choose').each(function(i, n) {
					if(i > 0) {
						var formHtml = '<div style="margin-left:65px;width: 400px;padding:10px 0px">' +
		...
		if(typeName == 1) {
			...
			if(num == "") {
				approvalHtml = '<div class="topic-div"><input type="hidden" class="quest-type" value="1"/>' +
								...
								'<span class="que-type" style="color: #3c4687;">【' + typeName + '】</span>' +
...
//							'<div style="height: 40px;position: relative;width:775px;margin:0 auto;">' +
//							'<input type="hidden" class="item-code" value="' + v.itemCode + '"/>' +
//							'<span style="position: absolute;top:7px;width: 6px;left:-45px;height: 26px;background-color: #de595a;border-radius: 2px;"></span>'+
//							'<input style="color:#de595a;position: absolute;top: 5px;left:-35px;background:#edebec;" class="yt-input title-name-input" disabled="disabled" value="' + v.itemName + '"></input>';
							...
							htmlTr += '<div class="tab-content">' +
								...
								'<div style="background:#edebec;height:40px;">'+
								'<div style="float:left;background:#edebec;">' +
								'<input type="hidden" class="item-code" value="' + v.itemCode + '"/>' +
								'<span style="width: 6px;height: 26px;background-color: #de595a;border-radius: 2px;float:left;margin-top:7px;"></span>'+
								'<input style="color:#de595a;left:10px;top:5px;background:#edebec;" class="yt-input title-name-input" disabled="disabled" value="' + v.itemName + '"></input>';
								//判断是否为卷首或卷尾
								...
								htmlTr+='<span class="title-update-name " style="display: inline-block;position: relative;">' +
									'<img style="position: absolute;top: 2px;left: -20px;"src="../../resources/images/icons/amend.png" /><span class="modify-big-title">修改</span>' +
									'</span>';
								}
//						
								//判断是否为授课效果或者是教学活动
								if(v.itemCode!="teachingEffect"&&v.itemCode!="teachingActivites"){
									htmlTr+='</div>' +
											...
											'<span style="position: relative;"><img style="position: absolute;top: 2px;left: -17px;" src="../../resources/images/classStudent/wejuan-add.png" /></span><span style="font-size: 16px;">新增问卷项</span>' +
											'</div>' +
											'</div>' +
											'</div>';
								}else{
								...
								$.each($.parseJSON(v.itemsDetailsJson), function(j, n) {
									...
									if(n.types == 5) {//类型为固定值
										...
										if(v.itemCode == "other") {
											...
											//htmlTr.find('.q-info').addClass('q-info-bottom');
											//htmlTr.find('div:eq(2)').children().eq(0).remove();
...
//											console.log(htmlTr.find(".q-info .topic-div").length);
										...
										itemVal+='<div class="radio-title-approval" style="height: 20px;">' +
											...
											'<span class="que-type" style="color: #3c4687;">【单选】</span>' +
										...
										itemVal+='<div class="radio-title-approval" style="height: 20px;">' +
												'<div style="display: inline-block;float: left;">' +
												'<span class="que-type" style="color: #3c4687;">【评分】</span>' +
												'<span class="que-title">' + n.title + '</span>' +
												'</div>' +
												'<span class="content-update-name">' +
												'<img style="margin-right: 2px;"src="../../resources/images/classStudent/wenjuan-modify.png" />修改</span>' +
									...
									itemVal+='<div class="radio-title-approval" style="height: 20px;">' +
											'<div style="display: inline-block;float: left;">' +
											'<span class="que-type" style="color: #3c4687;">【开放】</span>' +
											'<span class="que-title">' + n.title + '</span>' +
											'</div>' +
											'<span class="content-update-name">' +
											'<img style="margin-right: 2px;"src="../../resources/images/classStudent/wenjuan-modify.png" />修改</span>' +
									...
									itemVal+='<div class="radio-title-approval" style="height: 20px;">' +
											'<div style="display: inline-block;float: left;">' +
											'<span class="que-type" style="color: #3c4687;">【多选】</span>' +
											'<span class="que-title">' + n.title + '</span>' +
											'</div>' +
											'<span class="content-update-name">' +
											'<img style="margin-right: 2px;"src="../../resources/images/classStudent/wenjuan-modify.png" />修改</span>' +

...
//		var pid = $yt_common.GetQueryString('reimId');
	...
	events: function() {
        ...
        $("#printBeforeForm").click(function(){
        	var pageUrl = "view/projectManagement-sasac/expensesReim/module/print/beforeApplyDetailMain.html?expenditureAppId="+priorApproval.reimId+ '&costType='+priorApproval.detailDatas.advanceCostType;//即将跳转的页面路径
        ...
        $("#beforeEdit").on("click",function(){
			 //即将跳转页面
			var pageUrl=$yt_option.websit_path+'view/projectManagement-sasac/expensesReim/module/busiTripApply/serveApply.html?advanceId='+priorApproval.reimId;
			//调用公用的跳转方法
...
//		me.setMeetingCostList(data.meetingCostList);
		...
		$.ajax({
			...
			success: function(data) {
				if(data.flag == 0) {
					...
					/*var costTotalNum = $yt_baseElement.rmoney($("#lectureFeeTable .costTotal").text());
					if(isNaN(costTotalNum)){
						costTotalNum = 0;
					}else{
						costTotalNum = costTotalNum;
					}
					var advanceAmountNum = parseFloat(d.advanceAmount);
					$("#surtax").text($yt_baseElement.fmMoney(advanceAmountNum - costTotalNum));
					console.log($("#surtax").text($yt_baseElement.fmMoney(advanceAmountNum - costTotalNum)));
					...
					$.each(d.costData.costPredictInfoList, function(i,n) {
						afterTaxAllMoney += n.averageMoney;
					});
					$("#vatSalesTax").text($yt_baseElement.fmMoney(afterTaxAllMoney * 0.03));
					$("#surtax").text($yt_baseElement.fmMoney(afterTaxAllMoney * 0.03*0.12));
					//隐藏部门或中心可用余额
	...
	submitAdvanceAppInfo: function(d) {
		$.ajax({
			...
			success: function(data) {
				...
				if(data.flag == 0) {
					//成功跳转到事前申请审批列表页面
					window.location.href = $yt_option.websit_path + 'view/projectManagement-sasac/expensesReim/module/approval/beforehandApproveList.html';
	...
	showBusinessFun: function() {
		...
		$('#doAdvanceBox').hide();
		//加载区域页面
		sysCommon.loadingWord('view/projectManagement-sasac/expensesReim/module/reimApply/hospitalitySpendingDetail.html');
	},
	/**
	 * 一般费用
	 */
	...
	showTravelFun: function() {
		...
		$('#doAdvanceBox').hide();
		//加载区域页面
		sysCommon.loadingWord('view/projectManagement-sasac/expensesReim/module/reimApply/travelSpendingDetails.html');
	},
	/**
	 * 培训
	 */
	showTrainFun: function() {
		...
		$('#doAdvanceBox').show();
		//加载区域页面
		sysCommon.loadingWord('view/projectManagement-sasac/expensesReim/module/busiTripApply/trainApproval.html');
	},
	/**
	 * 会议
	 */
	showMettingFun: function() {
		...
		$('#doAdvanceBox').hide();
		//加载区域页面
		sysCommon.loadingWord('view/projectManagement-sasac/expensesReim/module/beforehand/meetingCostApplyDetails.html');
	},
	/**
	 * 
	 * 表格金额合计更新

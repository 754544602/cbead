(function($, window) {
	var serveFunds = {
		...
		init: function() {
			var ts = this;
			sysCommon.loadingWord('view/system-sasac/expensesReim/module/busiTripApply/serveTrainApply.html');
			//获取修改参数
			...
			ts.getTrainingTypeOption('TRAIN_TYPE');
			ts.subBtn();
			ts.saveBtn();
		},
		/**
			...
			$("#hotelDate").calendar({
				...
				callback: function() {
					...
					if(tdStartDate != "" && tdEndDate != "") {
						...
						//计算入住天数
						$(".hotel-form .hotel-num").css("color", "#333333").text(diff_day + 1);
						//获取住宿费
		...
		subBtn: function() {
			...
			$('#submitPayment').on('click', function() {
				...
				if($yt_valid.validForm($('.base-info-form-modle,.approve-div'))) {
					//验证预算余额与申请金额
					var verifyBudgetExpendSum = serveFunds.verifyBudgetExpendSum();
...
//					}).text(balance ? serveFunds.fmMoney(balance) + '万元' : '--');
		...
		submitWorkFlow: function(subData /*, thisBtn*/ ) {
			$.ajax({
				...
				success: function(data) {
					...
					if(data.flag == 0) {
						//打开单据粘贴单
						//						window.open($yt_option.websit_path + 'view/system-sasac/expensesReim/module/print/InvoicePasting.html?reimId=' + data.data);
						//跳转审批页面
						$yt_common.parentAction({
							url: $yt_option.parent_action_path, //父级中转地址,固定配置项,只有统一修改处理。
							funName: 'locationToMenu', //指定方法名，定位到菜单方法
							data: {
								url: 'view/system-sasac/expensesReim/module/approval/projectBefList.html' //要跳转的页面路径
							}
						});
					}
					//					thisBtn.attr('disabled', false).removeClass('btn-disabled');
		...
		changeCostPredictInfoList: function() {
			...
			$.each(trs, function(i, n) {
				...
				list.push({
					...
					specialInstruct: tr.find('.special-instruct').text(), //specialInstruct	特殊说明
					setMethod: '', //setMethod	结算方式
		...
		setTrainApplyInfoList: function(list) {
			...
			$.each(list, function(i, n) {
				...
				$('#approvaNum').val(n.approvaNum); //approvaNum 培训人数
				$('#chargeStandard').val($yt_baseElement.fmMoney(n.chargeStandard)||0); //workerNum 收费标准	
				$('.details-textarea').val(n.reason); //reason 调整原因
			});
		...
		setCostTeachersFoodApplyInfoList: function(list) {
			...
			$.each(list, function(i, n) {
				html += '<tr pid="' + n.foodId + '">' +
					...
					'<td style="text-align: center !important;" class="dec">' + (n.remark || "无") + '</td>' +

...
//		$yt_common.user_info
	...
	events: function() {
		...
		$(".tab-header button.print-btn").on("click",function(){
			...
			console.log(thisTrData);
			if(selTrLen == 0){
				$yt_alert_Model.prompt("请选择一行数据进行操作");
			}else{
				var pageUrl = "";//即将跳转的页面路径
				pageUrl = "view/projectManagement-sasac/expensesReim/module/print/finalStatement.html?finalAppId="+thisTrData.finalAppId+'&costType='+thisTrData.costType;
...
//			var loanId = thisTr.find("input.tableId").val();
			if(selTrLen == 0){
				...
				var pageUrl = "view/projectManagement-sasac/expensesReim/module/payment/budgetAuditReduce.html?advanceAppId="+ advanceAppId +"&costType=" + costType;;//即将跳转的页面路径
				//调用公用的打开新页面方法传输参数不需要左侧菜单
				$yt_baseElement.openNewPage(2,pageUrl);
			}
//			window.location.href=$yt_option.websit_path+'view/projectManagement-sasac/expensesReim/module/payment/budgetAuditReduce.html';
		});
		...
		$('.pending1,.pending2').on('click', '.apprv-btn', function(){
			...
			if(taskKey == 'activitiStartTask'){
				//修改
				var pageUrl = "view/projectManagement-sasac/expensesReim/module/reimApply/expenseAccount.html?appId="+ advanceAppId +"&fun=" + fun;//即将跳转的页面路径
				window.location.href = $yt_option.websit_path + pageUrl;
			} else {
				//审批
				var pageUrl = "view/projectManagement-sasac/expensesReim/module/reimApply/finalExamine.html?appId="+ advanceAppId +"&fun=" + fun; //即将跳转的页面路径
				window.location.href = $yt_option.websit_path + pageUrl;
			}
		...
		$('.pending1,.pending2').on('click', '.to-detail', function(){
			...
			//详情页面
			var pageUrl = "view/projectManagement-sasac/expensesReim/module/reimApply/finalDetail.html?appId="+ advanceAppId;//即将跳转的页面路径
			//调用公用的打开新页面方法传输参数不需要左侧菜单
			$yt_baseElement.openNewPage(2,pageUrl);
		});
		...
		$('.already-printed').click(function() {
			...
			if(ids) {
				expenApplApprList.updateFinalIsPrintStateYes(ids);
				check = true;
			} else {
		...
		$('.non-printing').click(function() {
			...
			if(ids) {
				expenApplApprList.updateFinalIsPrintStateNo(ids);
				check = true;
			} else {
				$yt_alert_Model.prompt('请选择数据进行操作');
			}
	 ...
	 * @param {Object} ids
	 */
	updateFinalIsPrintStateYes: function(ids) {
		$.ajax({
			type: "post",
			url: "sz/finalApp/updateFinalIsPrintStateYes",
			async: false,
			data: {
				ids: ids
			},
	 ...
	 * @param {Object} ids
	 */
	updateFinalIsPrintStateNo: function(ids) {
		$.ajax({
			type: "post",
			url: "sz/finalApp/updateFinalIsPrintStateNo",
			async: false,
			data: {
				ids: ids
			},
	...
	getProcessingAndFinishList: function(stateCode,printState) {
		...
		$('.page2').pageInfo({
			...
			pageNum: 15, //每页显示条数  
			pageSize: 10, //显示...的规律  
			url: 'sz/finalApp/getUserFinalAppInfoBusinessListToPageByParams', //ajax访问路径  
			data: {
				...
				if(data.flag == 0) {
					if(datas.length > 0) {
						...
						$.each(datas, function(i, n) {
							...
							trStr += '<tr>' +
								'<td><label style="position:relative;z-index:100" class="check-label yt-checkbox"><input type="checkbox" name="test" value="' + n.finalAppId + '"/></label>' + '</td>'+
						...
						$('.page2').hide();
						//拼接暂无数据效果
						htmlTbody.append(expenApplApprList.noDataTrStr(10));
					}
				}
				...
				$.ajax({
					...
					success: function(data) {
						if(data.flag==0){
							...
							expenApplApprList.getProcessingAndFinishList('WF_PASS_QUERY_SQL_PARAMS');
						}else{
							$yt_alert_Model.prompt(data.message);
						}
					},

var pay = {
	 ...
	 */
	submitPaymentAppInfo: function(subData) {
		$.ajax({
			type: 'post',
	...
	appendMsgList: function(tr) {
		...
		var pid = $('#budgetProject option:selected').val();
		//付款日期
		var payDate = $('#payDate').val();
		//金额
		var Unit = $('#Unit').val();
		var html = '<tr pid="'+pid+'" type="'+type+'" payee="' + budgetProject + '" payment="' + payDate + '" paymentmoney="' + Unit + '">' +
		...
		//获取所有的金额
		var tds = $('.pay-detail-tabel tbody .paymentmoney');
		var total = 0;
		//计算合计金额
		$.each(tds, function(i, n) {
			total += pay.rmoney($(n).text());
		});
		var fmTotal = pay.fmMoney(total);
		//赋值合计金额
		$('.pay-detail-tabel tbody .payee-other-total-money').text(fmTotal);
		//已付款金额
		$('#alreadyMoney').text(fmTotal);
		//计算未付款金额
		var mustMoney = pay.rmoney($('#mustMoney').text());
		$('#alreadyMoney').text(pay.fmMoney(mustMoney-total));
...
//		$yt_baseElement.fmMoney($('.payee-personal tbody .payee-personal-money').text(fmTotal));
	...
	setFormData: function(d) {
		...
		//事前申请编号
		$('.befor-examine').text(d.advanceAppNum);
		if(d.advanceAppNum){
		...
		$('#specialName').text(d.specialName).attr('val',d.specialCode);//specialCode	所属预算项目code
	...
	setCostNormalList: function(list) {
			...
			$.each(list, function(i, n) {
				html += '<tr>' +
					'<td class="ordinar-name" style="text-align: left;">'+ n.normalName + '</td>' +
					'<td class="payDet-money" style="text-align: right;">' + ($yt_baseElement.fmMoney(n.normalAmount)) + '</td>' +
					'</tr>';
				total += +n.normalAmount;
			});
			$('.ordinary-payment-approval #paymentDetailedTable tbody .pay-detail-tabel-total').before(html);
			$('.ordinary-payment-approval #paymentDetailedTable tbody .total-money').text($yt_baseElement.fmMoney(total));
	},
	/**
	 * 
	 * 表格金额合计更新
	...
	payDetailList:function(){
		...
		$('table.pay-detail-tabel tbody tr:not(.pay-detail-tabel-total)').each(function(i, n) {
			tr = $(n);
			list.push({
				receivablesId:tr.attr('pid'),//receivablesId	收款方Id
				receivablesType:tr.attr('type'),//receivablesType	收款方类型
				receivablesType:tr.attr('payment'),//paymentDate	支付时间
				receivablesType:pay.rmoney(tr.attr('paymentmoney')),//paymentAmount	支付金额
			});
...
$(function() {
	...
	pay.getCommentByProcessInstanceId();
})
...
//							$("#" + addFile).prev().val(resultData.obj.naming).data("fileInfo", resultData.obj);
		...
		$(".update-couse-btn").off('click').click(function() {
			if($("input[name='couseCheck']:checked").length>0&&$(".reception-list-tbody").find(".yt-table-active").length>0){
				...
				$(".add-course-box input[name=courseTypeDetails]").val(couseInfo.courseTypeDetails);
				//初始化上课时间
...
//									var latitude = parseFloat();
									...
									success: function(data) {
										...
										if(data.data.isConflict==1){
											...
											$('.yt-alert-one').css({
												...
												'margin-left': '-325px',
												'top':'50%',
												'margin-top':-($('.yt-alert-one').height()/2)+'px'
											})
										}else{
									          updateClassRoom();
										}
		...
		$('.merge-score-couse').click(function() {
			...
			$('.merge-score-form .yt-model-sure-btn:eq(0)').off().on('click', function() {
				if(!$yt_valid.validForm($(".merge-score-form"))){
					return false;
				}
				//课程合并ID
				var mergeCourseId = "";
		...
		$(".address-type").change(function() {
			if($(this).val() == "1") {
				...
				if(!!window.ActiveXObject || "ActiveXObject" in window){
					$('.rooms-thead').css('width','620px');
					$('.rooms-name').css('width',($('.rooms-name-td')[0].offsetWidth)+'px');
				}else{
					$('.rooms-name').css('width',($('.rooms-name-td')[0].offsetWidth+2)+'px');
					$('.rooms-type').css('width',($('.rooms-type-td')[0].offsetWidth+2)+'px');
					$('.rooms-count').css('width',($('.rooms-count-td')[0].offsetWidth+2)+'px');
				}
			} else {
				$('.sign-address').css('margin-top','10px');
				$(".internal-address").hide();
					...
					if(data.data.rows.length > 0) {
						...
						$.each(data.data.rows, function(i, n) {
							...
							if(rowSpanStr != n.courseDate) {
								...
								htmlTr += '<td class="bgn-tr" width="100px">' + n.courseDate + '</td>';
								rowSpanNum = 1;
							} else {
								rowSpanNum++;
							}
							...
							htmlTr += '<td width="110px"> <label class="check-label yt-checkbox">' +
								'<input type="checkbox" name="couseCheck" value="1"/>' + n.courseTime +
								'</label><div class="checkbox-top"></div></td>' +
								'<td width="90px">' + (n.courseTypeCode == 'teachingActivities' ? n.courseTypeDetails : n.courseTypeName) + '</td>' +
								'<td width="100px" style="'+cssTd+'">' + n.teacherName + '</td>' +
								'<td class="course-tbody-theme" style="text-align:left;">' + n.courseName + '</td>' +
								'<td class="course-tbody-files" style="'+cssTd+'">' + fileName + '</td>' +
								'<td width="150px" style="text-align:left;"><input class="class-room-ids" type="hidden" value="' + n.classroomIds + '"/>' + (n.classroomNames == "" ? n.address : n.classroomNames) + '</td>' +
								'</tr>';
							htmlTr = $(htmlTr).data("couseInfo", n);
							htmlTbody.append(htmlTr);
						});
						$('.course-overflow').css('height',$(window).height()-260+'px');
						if($('.course-table').height()>$('.course-overflow').height()){
							if(!!window.ActiveXObject || "ActiveXObject" in window){
								$('.course-thead').parents('div').css('padding-right','16px')
						   }else{
								$('.course-thead').parents('div').css('padding-right','6px')
						   }
						}else{
								$('.course-thead').parents('div').css('padding-right','0px')
						}
						$('.course-thead-theme').css('width',$('.course-tbody-theme')[0].clientWidth+1+'px');
						$('.course-thead-files').css('width',$('.course-tbody-files')[0].clientWidth+1+'px');
						$('.course-list-tbody tr').eq(lastTr).find("td").eq(0).attr("rowSpan", rowSpanNum);
					...
					if(data.data.rows.length > 0) {
						...
						$.each(data.data.rows, function(i, n) {
							htmlTr = '<tr>' +
								...
								'</label></td>' +
								'<td rowspan="3" width="" class="room-name rooms-name-td"><span>' + n.classroomName + '</span></td>' +
								'<td rowspan="3" width="130px" class="rooms-type-td">' + n.types + '</td>' +
								'<td rowspan="3" width="100px" class="tr rooms-count-td">' + n.galleryful + '</td>' +
								'<td width="50px">上午</td>' +
								stateDay +
								'</tr>' +
								'<tr>' +
...
var feedBack = {
	...
	event:function(){
		...
		function questionEditSubmit(that){
			...
			$('.question-edit-sure-btn').off('click').click(function(){
					$('#pop-modle-alert').show();
				//单选多选
					...
					$.each(data.data, function(i, m) {
						...
						if(m.itemsDetailsJson.length != 0) {
							...
							$.each(m.itemsDetailsJson, function(a, s) {
								if(s.itemDetailsSpecific.length==0&&s.types != 1&&s.types != 4){
									return true;
								}

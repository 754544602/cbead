...
//		
		...
		if(classList != null) {
			var projectUser;
			var projectHeadmaster;
			var projectSell;
			var projectAid;
			$.each(classList.data, function(i, n) {
				//项目主任
				projectUser = n.projectUser;
				//班主任
				projectHeadmaster = n.projectHeadmaster;
				//项目销售code
				projectSell = n.projectSell;
				//项目助理code
				projectAid = n.projectAid;
				//判断当前登录人是否为项目主任或班主任
				if(projectUser.indexOf(userName) != -1 || projectHeadmaster.indexOf(userName) != -1 || projectSell.indexOf(userName) != -1 || projectAid.indexOf(userName) != -1) {
		...
		$(".projectName").change(function(){
   			...
   			if (classData != undefined) {
   				$('#projectUserName').text(classData.projectUserName);
   				$('#traineeCount').text(classData.traineeCount);
   			}
	...
	addLineInfo:function(){
		...
		var lineTrHtml = '<tr class="add-textList tr-border-style">'+
							'<td style="text-align:center;" class="order-by">'+num+'</td>'+
							'<td class="td-input">'+
							...
							'<td  style="position: relative;">'+
								'<input style="margin-left:10px" type="text" class="yt-input dateLength td-input-time" validform="{isNull:true,size:10,msg:\'请输入会序时长\'}"/> '+
								'<span class="valid-font z-style"></span>'+
								'<p class="time-txt" style="float: left;margin: 4px;">'+'分钟'+'</p>'+
							'</td>'+
							'<td style="text-align: center;" class="td-img">'+
								'<img class="orderBy-dele-img img-size" src="../../resources/images/open/tiaoxu.png"/>'+
							'</td>'+
							'<td style="text-align: center;" class="handle-td td-img">'+

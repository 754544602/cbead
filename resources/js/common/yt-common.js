...
var $yt_baseElement={
   ...
   /**
	...
	fmMoney: function(s, n) {
		s = String(s).replace(".", "$#$").replace(/\./g, "").replace("$#$", ".");
		n = n > 0 && n <= 20 ? n : 2;
	...
	showLoading:function(){
		...
		if(window != top){
			window.parent.$yt_baseElement.showLoading()
		}else{
			//存储时间戳
			$yt_baseElement.timestamp = new Date().getTime();
			if($(".loading-model").length == 0){
				var loadingStr='<div class="loading-model" style="text-align: center;position: fixed;background: none;z-index:1001">'
						  +'<div><img class="loading-img" style="width:160px;height:160px" src="'+$yt_option.websit_path+'resources/images/icons/loading.gif"/></div>'
						  +'<div>';
				$("body").append(loadingStr);	
			}
			
			$yt_alert_Model.getDivPosition($(".loading-model"));
			$(".loading-model,#heard-nav-bak,#pop-modle-bg").show();
	 ...
	 * 
	 */
	hideLoading:function(backFun){
		if(window != top){
			window.parent.$yt_baseElement.hideLoading(backFun);
		}else{
			var managerTime = 500;
			var setTime =0;
			//获取当前的时间戳
			...
			//判断时间戳的差小于等于1000
			if(thisTime<=managerTime){
				setTime = managerTime-thisTime;
				setTimeout(function(){
				$("#pop-modle-bg,#heard-nav-bak").hide();
				$(".loading-model").hide();
	...
	pageLoadingShow:function(){
		...
		var loadingStr='<div class="page-loading">'
		              +'<img class="loading-img" style="width:130px;height:130px" src="'+$yt_option.websit_path+'resources/images/icons/loading.gif"/>'
		              +'</div>';
   ...
   hideMongoliaLayer:function(){
		...
		if(param!=""&&param!=undefined){
			...
			for(var i = 0; i < strs.length; i ++) {
				reulst[strs[i].split("=")[0]]=decodeURIComponent(strs[i].split("=")[1]); 
			} 
		}
		return reulst;
	}
...
var $yt_alert_Model={
	...
	setFiexBoxHeight:function (para){
		var fiexBox = $(para);
		$.each(fiexBox, function(i,n) {
			var boxHeight = $(n).height()+46;
...
$.fn.extend({
    ...
    setCheckBoxState:function(obj){
    	$(this).each(function (i,n){
	    	 ...
	    	 */
	    	if(obj == "check"){
	    		$(n).parent().addClass("check");	
	    		$(n)[0].checked = true;
	    	}else{
	    		$(n).parent().removeClass("check");
	    	}
	    	/**
	    	 * 取消选中
	    	 */
	    	if(obj == "uncheck"){
	    		...
	    		$(n)[0].checked = false;
	    		//清楚禁用样式
	    		$(n).parent().removeClass("yt-check-disabled");	
	    	}
	    	/**
	    	 * 禁用
	    	 */
	    	if(obj == "disabled"){
	    		$(n)[0].disabled=true; 
	    		$(n)[0].checked = false;
	    		$(n).parent().addClass("yt-check-disabled");	
	    	}
	    	/**
	    	 * 取消禁用
	    	 */
	    	 ...
	    	 */
	    	if(obj == "half"){
	    		$(n).parent().addClass("yt-checkbox-half");
	    	}else{
	    		$(n).parent().removeClass("yt-checkbox-half");
	    	}
    	});
   ...
   */
   setRadioState:function(obj){
   		$(this).each(function (i,n){
	   	    ...
	   	    if(obj == "uncheck"){
	   			...
	   			$(n)[0].checked = false;
	   			//删除禁用样式
	   			$(n).parent().removeClass("yt-radio-disabled");
	   			//判断当前单选按钮祖父级是否有from标签元素
		   		if($(n).parents("form").length==0){
		   			//如果没有父级没有from,找到body下的所有name属性名与当前获取到的name相同的单选按钮,不包含form表单下的单选,父级label删除选中的样式
			   		$(n).parents("body").find(".yt-radio input[type='radio'][name='"+docName+"']").not("form input[type='radio']").parent().removeClass("check");
		   		}else{
		   			//如果包含就删除当前单选按钮父级from表单下所有的单选name值与当前name值相等的,单选按钮父级label选中类样式
		   			$(n).parents("form").find(".yt-radio input[type='radio'][name='"+docName+"']").parent().removeClass("check");
		   		}
	   	    }
	   	    /**
	   	    ...
	   	    if(obj == "disabled"){
	   			...
	   			$(n)[0].disabled = true;
	   			$(n)[0].checked = false;
	   	    	$(n).parent().addClass("yt-radio-disabled").removeClass("check");
	   	    }
	   	    /**
	   	     * 取消禁用
	   	     */
    ...
    */
    ...
    function create_nice_select($select) {
      ...
      $dropdown.find('.current').html($selected.data('display') || $selected.text());
       ...
       if($selected.text() !="请选择" && $selected.text()!='全部'){
       	$dropdown.find('.current').css('color','#333');
      ...
      $dropdown.find(".search-current").on("focus",function(){
      	  var isClear = false;
      	  if($(this).val()==null || $(this).val() == undefined || $(this).val() == "" || $(this).val() == "请选择" || $(this).val() == "全部"){
      	  	 isClear = true;
      	  }
      	  if(isClear)
      	  {
      	  	$dropdown.find(".current,.search-current").val("");
      	  }else{
    ...
    */
	  	$dropdown.find(".search-current").keyup(function(event){
	  		...
	  		$(this).css('color',($(this).val() != '请选择' && $(this).val()!= '') ? '#333' : '#999');
	  		//调用回调方法,传入数据
    ...
    function bindFunction(obj,option){
	     ...
	     */
	      ...
	      if ($dropdown.hasClass('open')) {
	      	//调用判断下拉列表高度是否超出当前页面,改变显示位置
	      	selectScrollChange($dropdown);
    ...
    * 关闭当前选中的列表
    */
    $(document).off("click.nice_select").on('click.nice_select', function(event) {
   ...
   //texteara 高度自适应
    autoHeight: function(){
        ...
        function _adjustH(elem){
            ...
            return $obj.css({height: $obj.attr('_initAdjustHeight'), 'overflow-y': 'hidden','min-height':'28px'})

                    .height( elem.scrollHeight );
        }
    }
})
	jQuery.extend({ajaxDownloadFile: function(options) {
			options.url.indexOf('?')==-1?options.url = options.url+'?yitianSSODynamicKey='+$yt_common.getyitianSSODynamicKey():options.url = options.url+'&yitianSSODynamicKey='+$yt_common.getyitianSSODynamicKey();
	        var config = $.extend(true, { method: 'post' }, options);
	        var $iframe = $('<iframe id="down-file-iframe-box" style="position: fixed;left: -100000px;" name="down-file-iframe" />');
	      
		    var $form = $('<form enctype="multipart/form-data" target="down-file-iframe" style="position: fixed;left: -100000px;" method="' + config.method + '" accept-charset="UTF-8" />');
		    $form.attr('action', config.url);
		    for (var key in config.data) {
		    $form.append("<input type='hidden' name='" + key + "' value='" + config.data[key] + "' />");
		     }
		    $(document.body).append($form);
	      	if($("#down-file-iframe-box").length ==0){
	...
	/**
    * 下拉列表高度到页面底部,不够显示,改动列表显示上方
    */
   function selectScrollChange(dropdown){
		/**
      	 * 算取列表显示位置
      	 */
      	//获取整个文档的高度
      	...
      	if(botHei<objHeight){
      		$(dropdown).find("ul").removeClass("page-select-ul-posi");
      		$(dropdown).find("ul").css({"top":"inherit","bottom":"27px"});
      	}else{
      		$(dropdown).find("ul").addClass("page-select-ul-posi");
      	}
   }
...
(function($) {
	...
	$.ajax = function(opt) {
		...
		if(yitianSSODynamicKey!=undefined){
			ytParams['yitianSSODynamicKey'] = yitianSSODynamicKey;
		}
			...
			success: function(data, textStatus) {
				...
				try {
					if((jQuery.parseJSON(data).flag == undefined ? "" : jQuery.parseJSON(data).flag.toString()) == "1") {
						...
						if(!$yt_option.is_cas){
							loginUrl = $yt_option.websit_index;
//							location.href = $yt_option.websit_index;
//							loginUrl = loginUrl.substr(0, loginUrl.indexOf("systemIndex=")) + "systemIndex=" + window.location.href;
                		}else{
                			//loginUrl = $yt_option.logoutUrl;
                		}
...
$(document).ready(function(){
	//搜索框回车事件
	$('input.selectParam').unbind('keydown').bind('keydown', function (event) {
		if (event.keyCode == "13") { 
			$(this).siblings('.search-btn').click()
		}
	})
	$('.key-word input').unbind('keydown').bind('keydown', function (event) {
		if (event.keyCode == "13") { 
			$(this).siblings('.search-btn').click()
		}
	})
	$('input#keyword').unbind('keydown').bind('keydown',function (event) {
		if (event.keyCode == "13") { 
			$(this).siblings('.search-btn').click()
		}
	});

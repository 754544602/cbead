var  breakDownReport = {
	...
	init:function(){
		...
		if(requerParameter !=null && requerParameter !=""){
			//表Id
			breakDownReport.tableId = requerParameter.tableId;
			//阶段
			breakDownReport.budgetStage = requerParameter.budgetStage;
			//调用获取表样式信息方法
			breakDownReport.getBudgetTableDetail();
		}
		//调用操作事件方法
		breakDownReport.events();
	},
	events:function(){
		/*打印按钮操作事件*/
		$("#printBtn").off().on("click", function() {
			//调用执行打印的方法
            breakDownReport.doPrint();
		});
		//点击下载按钮
		$("#downLoadBtn").click(function(){
			//配置统一参数
			var yitianSSODynamicKey  = $yt_baseElement.getToken();
			window.location.href = $yt_option.base_path + 'budget/main/exportResolveAdjustReport?tableId=' + breakDownReport.tableId + '&budgetStage='+breakDownReport.budgetStage+"&yitianSSODynamicKey="+yitianSSODynamicKey+"&ajax=1";
		});
	},
	/**
	 * 获取预算表样式详情
	...
	getBudgetTableDetail: function() {
		$.ajax({
			type: 'post',
			url: 'budget/main/getResolveAdjustReport',
			async: false,
			data: {
				tableId:breakDownReport.tableId,
				budgetStage:breakDownReport.budgetStage
			},
			success: function(data) {
				...
				//调用获取预算表配置方法
				breakDownReport.getConfigInfo();
				//调用获取预算表格数据方法
				breakDownReport.getTableInfoList(budgetData);
				//调用获取差额明细信息方法
				breakDownReport.getMoneyDetail(dataObj);
			}
		});
	},
	/**
			...
			success: function(data) {
				...
				if(data.flag == 0) {
					if(data.data.length > 0) {
						breakDownReport.configData = data.data;
	...
	doPrint: function() {
		...
		startBreakDownPasting = "<!--startBreakDownPasting-->"; //设置打印开始区域   
		endBreakDownPasting = "<!--endBreakDownPasting-->"; //设置打印结束区域   
		prnhtml = bdhtml.substr(bdhtml.indexOf(startBreakDownPasting)); //从开始代码向后取html   
		prnhtml = prnhtml.substring(0, prnhtml.indexOf(endBreakDownPasting)); //从结束代码向前取html   
		window.document.body.innerHTML = prnhtml;
		window.focus();
        if(breakDownReport.isIE()){

var budgetAuditReduce = {
	...
	events: function() {
		...
		//点击保存并进行费用分摊按钮,跳转页面
		$("#saveAndCostSharingBtn").click(function(){
			//判断是否有空行
			var nullTr = "";
			$.each($(".this-budget-table tr.list-tr"), function(i,n) {
				if($(n).find(".img-icon span").eq(0).hasClass("operate-save")){
					nullTr = 1;
				}
			});
			if(nullTr == 1){
				$yt_alert_Model.prompt("请将数据填写完整并保存");
				return false;
			}
			if($yt_baseElement.rmoney($(".budget-sum-me-money").text()) != $yt_baseElement.rmoney($("#totalAmount").text())){
				$yt_alert_Model.prompt('金额必须等于申请支出扣减预算总金额');
				return false;
			}
			var me = $(this);
			var list1 = [];
			var list2 = [];
			var special = '';
			var updateBudgetSubList;
			var addBudgetSubList;
			var advanceAppId = budgetAuditReduce.advanceAppId;
			//金额列合计
			var count = 0;
			$('.this-budget-table tbody tr.operate-tr').each(function(i,n) {// 遍历 tr
				...
				count += amount;
				updateBudgetSubList = JSON.stringify(list1);
			});
			$('.this-budget-table tbody tr.add-tr').each(function(i) {// 遍历 tr
				//var two = $(this).find('.budget-info-td-two').text();
				var	special = $(this).find('.budget-info-td-one').text();
				var amount = $yt_baseElement.rmoney($(this).find('.budget-money-td span').text() || '0');
				list2.push({
				...
				count += amount;
				addBudgetSubList = JSON.stringify(list2);
			});
			//申请支出总金额
			var total = $yt_baseElement.rmoney($('#totalAmount').text() || '0');
			//金额列合计必须等于申请支出总金额，保存时校验；如果校验结果失败，显示提示信息：“手动核减后金额必须等于申请支出总金额”，且校验失败时，不能保存
			//if($yt_baseElement.fmMoney(count) == $yt_baseElement.fmMoney(total)) {
				budgetAuditReduce.saveBudgetSubInfo(updateBudgetSubList,addBudgetSubList,advanceAppId);
			/*} else {
				$yt_alert_Model.prompt('手动核减后金额必须等于申请支出总金额');
			}*/
			var expenditureAppId = $yt_common.GetQueryString('advanceAppId');
			var pageUrl = "view/system-sasac/expensesReim/module/payment/costSharingList.html?expenditureAppId=" + expenditureAppId;
		...
		$(".this-budget-table").on("click", ".operate-del", function() {
			if($(this).parent().parent().attr('pkId') != ""){
				budgetAuditReduce.subInfoIdList.push($(this).parent().parent().attr('pkId'));
			}
			...
			if(isTrue) {
				...
				if(budgetAuditReduce.verifyBudgetExpendSum(thisTr)){
					//if(money <= total){
						...
						var total = $yt_baseElement.rmoney($('#totalAmount').text() || '0');
						//输入的金额
						var money1 = $yt_baseElement.rmoney($(".budget-sum-me-money").text()) * 10000;
						var money2 = $yt_baseElement.rmoney($(this).parent().prev().find(".input-money").val()) * 10000;
						var money = (money1+money2)/10000;
						/*if(money != total){
							$yt_alert_Model.prompt('金额必须等于申请支出总金额');
							$(this).focus();
						}else{*/
							/*赋值*/
		...
		$(".this-budget-table").on("click", ".add-row", function() {
			//判断是否有空行
			if($(this).parent().parent().prev().find(".img-icon span").eq(0).hasClass("operate-save")){
				$yt_alert_Model.prompt("请将数据填写完整并保存");
				return false;
			}
			var thisBtn = $(this);
			var thisTr = thisBtn.parents("tr");
			var tr = $('<tr class="list-tr add-tr" pkId = "">' +
	...
	saveBudgetSubInfo: function(updateBudgetSubList,addBudgetSubList,advanceAppId) {
		var subInfoId = budgetAuditReduce.subInfoIdList.toString();
		$.ajax({
			type: "post",
			url: "sz/expenditureApp/saveOrUpdateBudgetSubInfo",
			async: true,
			data: {
				expenditureAppId : advanceAppId,
				addBudgetSubList : addBudgetSubList,
				updateBudgetSubList : updateBudgetSubList,
				subInfoId : subInfoId
			},
	...
	applicant: function(data) {
		...
		$("#totalAmount").text($yt_baseElement.fmMoney(data.deductBudgetAmount));
	...
	showCurrentBudgetSubList: function(data) {
		...
		$.each(data, function(i, n) {
			SubHtml += '<tr class="" specialCode="' + n.specialCode + '" specialName="' + n.specialName + '" prjCode="' + n.prjCode + '" prjName="' + n.prjName + '" amount="' + n.amount + '" createTime="' + n.createTime + '">' +
				'<td><span class="specialName">' + n.specialName + '</span></td>' +
	...
	showCurrentBudgetSub: function(data) {
		...
		var SubHtml = '';
		$.each(data, function(i, n) {
			SubHtml += '<tr class="list-tr operate-tr" pkId = '+n.pkId+' prjCode = '+ n.prjCode +' prjName = '+ n.prjName +'><td style="text-align: left;padding-left:15px"><div style="float: left;" class="theif-budget-object"><input type="hidden" class="special-code" value="'+ n.specialCode +'"><span class="budget-info-td-one">'+ n.specialName +'</span></div><div style="float: right;margin-right: 10px;"><div style="padding-top: 5px;display: inline-block;">部门可用预算余额：<span class="text-money code-dept">'+ (n.deptBudgetBalanceAmount ? $yt_baseElement.fmMoney(n.deptBudgetBalanceAmount) + '万元' : '--') +'</span></div><div style="padding-top: 5px;display: inline-block;margin-left: 20px;">单位可用预算余额：<span class="text-money code-mon">'+ (n.budgetBalanceAmount ? $yt_baseElement.fmMoney(n.budgetBalanceAmount) + '万元' : '--') +'</span></div></div></td><td class="budget-money-td" style="text-align: right;"><span>'+  $yt_baseElement.fmMoney(n.amount) +'</span></td><td class="img-icon"><span class="operate-update"><img src="../../../../../resources-sasac/images/common/edit-icon.png"></span><span class="operate-del"><img src="../../../../../resources-sasac/images/common/del-icon.png"></span></td></tr>';

...
//		$yt_common.user_info
		...
		//初始化打印按钮是否显示
		//expenApplApprList.showOrDisplayBtn();
		//保存弹窗当前日期值
	...
	events: function() {
		...
		$(".tab-header button.print-btn").on("click",function(){
			//判断是否选择表格数据
			var selTrLen = $(".tab-div .tab-content tbody tr.yt-table-active").length;
			//获取当前选中行数据
			var  thisTrData = $(".tab-div .tab-content tbody tr.yt-table-active").data("dataStr");
			if(selTrLen == 0){
				...
				if($(this).val() == "printExpend"){
					pageUrl = "view/system-sasac/expensesReim/module/print/expenditureVoucher.html?expenditureAppId="+thisTrData.expenditureAppId;
				}
				//打印支出凭单明细
...
//			window.location.href=$yt_option.websit_path+'view/system-sasac/expensesReim/module/payment/budgetAuditReduce.html';
		...
		$('.pending1,.pending2').on('click', '.apprv-btn', function(){
			...
			if(taskKey == 'activitiStartTask'){
				...
				//审批
				var pageUrl = "view/system-sasac/expensesReim/module/reimApply/expenseExamine.html?appId="+ advanceAppId +"&fun=" + fun; //即将跳转的页面路径
				window.location.href = $yt_option.websit_path + pageUrl;
			}
		...
		$('.screen input').change(function() {
			...
			expenApplApprList.getProcessingAndFinishList(stateCode,sta);
		});
		/*全选 反选*/
		var check = true;
		$('.all-checked').click(function() {
			if(check) {
				$('.pending2 tbody').setCheckBoxState("checkAll");
				$('.pending2 tbody tr').addClass('yt-table-active');
			} else {
				$('.pending2 tbody').setCheckBoxState("unCheckAll");
				$('.pending2 tbody tr').removeClass('yt-table-active');
			}
			check = !check;
		});
		/*打印*/
		$('.already-printed').click(function() {
			var ids = '';
			$('.pending2 tbody .check').each(function(i, n) {
				ids += $(n).find('input').val() + (i < $('.pending2 tbody .check').length - 1 ? ',' : '');
			});
			if(ids) {
				expenApplApprList.updateExpenditureIsPrintStateYes(ids);
				check = true;
			} else {
		...
		$('.non-printing').click(function() {
			...
			if(ids) {
				expenApplApprList.updateExpenditureIsPrintStateNo(ids);
				check = true;
			} else {
				$yt_alert_Model.prompt('请选择数据进行操作');
			}
		});
		$(".pending2 .yt-tbody").on('change','input[type=checkbox]',function(){
			if($(this)[0].checked){
	...
	/**
	 * 打印
	 * @param {Object} traineeId
	 */
	updateExpenditureIsPrintStateYes: function(ids) {
		$.ajax({
			type: "post",
			url: "sz/expenditureApp/updateExpenditureIsPrintStateYes",
			async: false,
			data: {
				ids: ids
			},
	 ...
	 * @param {Object} traineeId
	 */
	updateExpenditureIsPrintStateNo: function(ids) {
		$.ajax({
			type: "post",
			url: "sz/expenditureApp/updateExpenditureIsPrintStateNo",
			async: false,
			data: {
				ids: ids
			},
	...
	//判断打印详情按钮是否可用
	showOrDisplayBtn:function(){
		$(".tab-div .tab-content tbody tr").off().on("click",function(){
			$(this).addClass('yt-table-active').siblings().removeClass('yt-table-active');
			//获取当前选中行的code
			var  thisTrCode = $(".tab-div .tab-content tbody tr.yt-table-active").attr("costtype");
			if( thisTrCode == 'SOCIAL_SECURITY_FEE'|| thisTrCode == 'SPECIAL_DEDUCTION'){
				$(".tab-header button.print-btn[value='printExpendDetail']").attr("disabled",true).addClass("disabled-btn");
			}else{
				$(".tab-header button.print-btn[value='printExpendDetail']").attr("disabled",false).removeClass("disabled-btn");
			}
		})
	},
	...
	getProcessingAndFinishList: function(stateCode,printState) {
		...
		$('.page2').pageInfo({
			...
			pageNum: 15, //每页显示条数  
			pageSize: 10, //显示...的规律  
			url: 'sz/expenditureApp/getUserExpenditureAppInfoBusinessListToPageByParams', //ajax访问路径  
			data: {
				...
				if(data.flag == 0) {
					if(datas.length > 0) {
						...
						$.each(datas, function(i, n) {
							...
							trStr += '<tr costtype="' + n.costType + '">' +
								'<td><label style="position: relative;z-index: 100;" class="check-label yt-checkbox"><input type="checkbox" name="test" value="' + n.expenditureAppId + '"/></label>' + '</td>'+
								'<td><a class="yt-link to-detail">' + n.expenditureAppNum + '</a><input type="hidden" class="hid-advance-id" value="' + n.expenditureAppId + '"/><input type="hidden" class="processInstanceId" value="' + n.processInstanceId + '"/><input type="hidden" class="costType" value="' + n.costType + '"/></td>' +
								'<td class="tl" style="text-align:left;">' + (n.expenditureAppName == "" ? "--" : n.expenditureAppName) + '</td>';
								//支出总金额,判断费用类型为 薪酬税费，专项扣除   、SOCIAL_SECURITY_FEE公积金将 支出总金额 与 扣除预算总金额 数据交换
								/*var  totalAmount = "";
								if(n.costType == 'ACCRUED_TAX' || n.costType == 'SPECIAL_DEDUCTION' || n.costType =='SOCIAL_SECURITY_FEE'){
									totalAmount = n.deductBudgetAmount;
								}else{
									totalAmount = n.totalAmount;
								}*/
								trStr += '<td style="text-align: right;">' + (n.totalAmount == "" ? "--" : $yt_baseElement.fmMoney(n.totalAmount)) + '</td>';
							...
							trStr +='<td class="print-state" style="color: ' + (n.isPrint == 0 ? '#333333' : '#417095') + ';">'+ (n.isPrint == 0 ? '未打印' : '已打印' ) + '</td>'+
									'<td>' + (stateCode != 'WF_SUSPENDING_QUERY_SQL_PARAMS' ? '' : '<a class="yt-link apprv-btn" taskKey="' + n.taskKey + '">' + (n.taskKey == 'activitiStartTask' ? '处理' : '审批') + '</a><span class="center-line">|</span>') + '<a class="yt-link log-mod">日志</a></td></tr>';
				...
				$.ajax({
					...
					success: function(data) {
						if(data.flag==0){
							...
							expenApplApprList.getProcessingAndFinishList('WF_PASS_QUERY_SQL_PARAMS',"2");
						}else{
							$yt_alert_Model.prompt(data.message);
						}
					},

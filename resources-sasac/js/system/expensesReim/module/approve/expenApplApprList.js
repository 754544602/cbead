var expenApplApprList = {
	...
	events: function() {
		...
		$('.pending1,.pending2').on('click', '.apprv-btn', function() {
			...
			if(taskKey == 'activitiStartTask') {
				...
				//审批
				var pageUrl = "view/system-sasac/expensesReim/module/reimApply/expenseExamine.html?appId=" + advanceAppId + "&fun=" + fun + '&childId=' + menuInfo.pkId + '&systemId=' + menuInfo.systemId; //即将跳转的页面路径
				window.location.href = $yt_option.websit_path + pageUrl;
			}
		...
		$('.pending1,.pending2').on('click', '.to-detail', function() {
			...
			var pageUrl = "view/system-sasac/expensesReim/module/reimApply/expenseDetail.html?appId=" + advanceAppId + '&childId=' + menuInfo.pkId + '&systemId=' + menuInfo.systemId; //即将跳转的页面路径
	...
	showOrDisplayBtn: function() {
		$(".tab-div .tab-content:visible tbody tr").off().on("click", function() {
			$(this).addClass('yt-table-active').siblings().removeClass('yt-table-active');
			//获取当前选中行的code
			var thisTrCode = $(".tab-div .tab-content:visible tbody tr.yt-table-active").attr("costtype");
			if(thisTrCode == 'SOCIAL_SECURITY_FEE' || thisTrCode == 'ACCRUED_TAX' || thisTrCode == 'SPECIAL_DEDUCTION') {
	...
	getProcessingAndFinishList: function(stateCode) {
		...
		$('.page2').pageInfo({
			...
			pageNum: 15, //每页显示条数  
			pageSize: 10, //显示...的规律  
			url: 'sz/expenditureApp/getUserExpenditureAppInfoWFListToPageByParams', //ajax访问路径  
			data: {
				...
				if(data.flag == 0) {
					if(datas.length > 0) {
						...
						$.each(datas, function(i, n) {
							...
							/*var tlAmt = 0;
							//单位类型为 薪酬税费，专项扣除   将 支出总金额 与 扣除预算总金额 数据交换
							if(n.costType == 'ACCRUED_TAX' || n.costType == 'SPECIAL_DEDUCTION') {
								tlAmt = n.deductBudgetAmount;
							} else {
								tlAmt = n.totalAmount;
							}*/
							trStr += '<tr costtype="' + n.costType + '">' +
								...
								'<td style="text-align: right;">' + (n.totalAmount == 0 ? "--" : $yt_baseElement.fmMoney(n.totalAmount)) + '</td>';
				...
				if(data.flag == 0) {
					if(datas.length > 0) {
						...
						$.each(datas, function(i, n) {
							...
							trStr += '<tr costtype="' + n.costType + '">' +
								'<td><a class="yt-link to-detail">' + n.expenditureAppNum + '</a><input type="hidden" class="hid-advance-id" value="' + n.expenditureAppId + '"/><input type="hidden" class="processInstanceId" value="' + n.processInstanceId + '"/></td>' +
								'<td class="tl" style="text-align:left;">' + (n.expenditureAppName == "" ? "--" : n.expenditureAppName) + '</td>' +
								'<td style="text-align: right;">' + (n.totalAmount == 0 ? "--" : $yt_baseElement.fmMoney(n.totalAmount)) + '</td>' +

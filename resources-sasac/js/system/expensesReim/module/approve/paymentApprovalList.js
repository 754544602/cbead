var beforeApproList = {
	...
	searchList: function() {
		...
		if(status == 'WF_SUSPENDING_QUERY_SQL_PARAMS') { // 待处理
			...
			beforeApproList.getOverendList(status);
	...
	switcher: function() {
		$('.tab-li').off().on('click', function() {
			...
			if(index == 1 ) {
				...
				$('.tab-content').eq(2).css("display", 'block').siblings('.tab-content').hide();
			} else {
	...
	getPendingList: function(stateCode) {
		...
		$('.page1').pageInfo({
			...
			pageNum: 15, //每页显示条数  
			pageSize: 10, //显示...的规律  
//			url: $yt_option.websit_path + "resources-sasac/js/testJsonData/paymentApprovalList.json", //ajax访问路径  
				...
				if(data.flag == 0) {
					if(datas.length > 0) {
						...
						$.each(datas, function(i, n) {
							trStr += '<tr>' +
								'<td><a class="yt-link to-detail">' + n.paymentAppNum + '</a><input type="hidden" class="pay-app-id" value="'+n.payAppId+'"><input type="hidden" class="pay-app-id" value="'+n.payAppId+'"><input type="hidden" class="processInstanceId" value="'+n.processInstanceId+'"/></td>' +
								'<td class="tl"><div style="white-space: normal;word-break: break-all;word-wrap: break-word;">' + (n.paymentAppName == "" ? "" : n.paymentAppName) + '</div></td>' +
								'<td>' + (n.costType == "" ? "" : n.costType) + '</td>' +
								'<td>' + n.applicantUserName + '</td>' +
								'<td>' + (n.applicantUserDeptName == "" ? "" : n.applicantUserDeptName) + '</td>' +
								'<td style="text-align: right;" class="moneyTreated">' + (n.totalAmount == "" ? "" : ($yt_baseElement.fmMoney(n.totalAmount))) + '</td>' +
								'<td>' + (n.applicantTime == "" ? "" : n.applicantTime) + '</td>' +
								'<td><a  class="yt-link flowBtn">' + n.nodeNowState + '</a></td>' +
								'<td class=""><a id=""class="yt-link apprv-btn" taskKey="'+n.taskKey+'">'+(n.taskKey == 'activitiStartTask' ? '处理' : '审批')+'</a><span class="center-line">|</span><a class="yt-link log-mod">日志</a></td>' +
								'</tr>';
						});
						...
						//绑定日志标签数据
						beforeApproList.initQtip();
						//绑定流程弹窗
						beforeApproList.flowWindow();
						//绑定详情页事件
						beforeApproList.toDetails();
//						$(".apprv-btn").click(function() {
...
//						});
						...
						htmlTbody.empty();
						//拼接暂无数据效果
						htmlTbody.append(beforeApproList.noDataTrStr(9));
					}
				}else{
					//提示数据访问失败
				}
...
//			url: $yt_option.websit_path + "resources-sasac/js/testJsonData/paymentApprovalList.json", //ajax访问路径  
			url: "sz/payApp/getUserPayAppInfoListToPageByParams", //ajax访问路径  
			///coso/website/resources-sasac/js/testJsonData/paymentApprovalList.json
			type: "post", //ajax访问方式 默认 "post"  
			data: {
				queryStateParams: stateCode,
				queryParams: keyWord
				...
				if(data.flag == 0) {
					...
					if(stateCode == "WF_SOLVED_QUERY_SQL_PARAMS") {
						if(datas.length > 0) {
							...
							$.each(datas, function(i, n) {
								trStr += '<tr>' +
								'<td><a class="yt-link to-detail">' + n.paymentAppNum + '</a><input type="hidden" class="pay-app-id" value="'+n.payAppId+'"><input type="hidden" class="processInstanceId" value="'+n.processInstanceId+'"/></td>' +
								'<td class="tl"><div style="white-space: normal;word-break: break-all;word-wrap: break-word;">' + (n.paymentAppName == "" ? "" : n.paymentAppName) + '</div></td>' +
								'<td>' + (n.costType == "" ? "" : n.costType) + '</td>' +
								'<td>' + n.applicantUserName + '</td>' +
								'<td>' + (n.applicantUserDeptName == "" ? "" : n.applicantUserDeptName) + '</td>' +
	 ...
	 * @param {Object} stateCode
	 */
	getOverendList: function(stateCode) {
		var keyWord = $('.search').val();
		$('.page3').pageInfo({
			pageIndex: 1,
			pageNum: 15, //每页显示条数  
...
//			url: $yt_option.websit_path + "resources-sasac/js/testJsonData/paymentApprovalList.json", //ajax访问路径  
			...
			success: function(data) {
				var datas = data.data.rows;
				var htmlTbody = $('.overend .yt-tbody');
				htmlTbody.empty();
				var trStr = "";
				if(data.flag == 0) {
//					WF_COMPLETED_QUERY_SQL_PARAMS
	...
	goApprovePage: function() {
		...
		$(".apprv-btn").off().click(function() {
			...
			//			var pageUrl= 'view/system-sasac/expensesReim/module/reimApply/reimApprove.html?reimId='+reimId;
			//			var pageUrl = 'view/system-sasac/expensesReim/module/payment/paymentApproval.html';
			/**
			 * 调用显示loading方法
			 */
			//			$yt_option.websit_path+
			//			parent.parent.$yt_baseElement.showLoading();
			/*调用阻止冒泡方法*/
			beforeApproList.eventStopPageaction();
			/*获取id*/
			var payAppId=$(this).parents('tr').find('.pay-app-id').val();
			var taskKey = $(this).attr('taskKey');
			if(taskKey == 'activitiStartTask'){
				//修改
				window.location.href = $yt_option.websit_path + 'view/system-sasac/expensesReim/module/payment/paymentApply.html?payAppId='+payAppId;
			}else{
				//审批
				window.location.href = $yt_option.websit_path + 'view/system-sasac/expensesReim/module/payment/paymentApproval.html?payAppId='+payAppId;
			}
	...
	toDetails: function() {
		$('.yt-link.to-detail').off("click").on('click', function() {
			...
			var payAppId=$(this).parents('tr').find('.pay-app-id').val();
			/*页面跳转打开新页面*/
			var pageUrl = "view/system-sasac/expensesReim/module/payment/paymentApplyDetail.html?processInstanceId=" + processInstanceId+"&payAppId="+payAppId;//即将跳转的页面路径
			var goPageUrl = "view/system-sasac/expensesReim/module/approval/paymentApprovalList.html";//左侧菜单指定选中的页面路径

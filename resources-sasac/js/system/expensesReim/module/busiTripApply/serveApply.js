(function($, window) {
	...
	var serveApply = {
		...
		init: function() {
			...
			//获取当前登录人code
			serveApply.parCode = $yt_common.user_info.posiCode;
			//申请人code
			$(".applicantUser").val($yt_common.user_info.userName);
			sysCommon.getApproveFlowData("SZ_TRAVEL_APP",'','{posCode:"'+ serveApply.parCode +'"}');
		...
		events: function() {
			...
			$('.docu-style-box').on('change', '.docu-style', function() {
				...
				serveApply.getSpecialDictList($('select.budget-item-one'));
				if(fun) {
					$('.qtip-text-div').hide();
					$('.mod-div').hide();
					//执行相应的显示方法
		...
		setSubmitEvent:function(){
			...
			$('#getPayment').on('click', function() {
				...
				//验证提示语句
				var validMsg = "";
				var applyTotalMoneyAmount = $yt_baseElement.rmoney($("#serveAllMoney").text());
				/**
				 * 
				...
				if(applyTotalMoneyAmount > budgetMoney){
					...
					var thisBtn = $(this);
					var verifyBudgetExpendSum = true; // me.verifyBudgetExpendSum();
		...
		getJsonVal: function() {
			...
			var deal = '';
			if (serveApply.parCode == 'QZZS' || serveApply.parCode == 'ZXZR') {
				deal = $yt_common.user_info.userName;
			} else{
				deal = $("#approve-users option:selected").val();
			}
			var getjsons = {
				advanceAppId: $("#advanceAppId").val(), //主表id
				advanceAppNum: $("#advanceAppNum").val(), //事前申请单号
				advanceAppReason: $("#advanceAppReason").val(), //事前申请事由
				costType: $(".docu-style-box .check input").val(), //费用类型w
				doAdvanceAmount: $yt_baseElement.rmoney($("#doAdvanceAmount").text()||0),//可用支出金额
				//isSpecial: $("#radiocheck .check input").val(), //是否为专项    
				advanceAmount: $yt_baseElement.rmoney($(".count-val-num").text() || '0'), //申请预算总金额
				//specialCode: $(".prior-approval").val(), //专项名称
				prjName: $('#prjName').val(),
				applicantUser: $(".applicantUser").val(), //申请人code
				specialCode: specialCode(), //specialCode	所属预算项目code
				parameters: '{posCode:"'+ serveApply.parCode +'"}', //JSON格式字符串
				dealingWithPeople: deal, //下一步操作人code
		...
		meetingList: function() {
			...
			var list = costType == 'MEETING_APPLY' ? [{
				...
				meetDays: +$('#calculationSession').val() || '0' + '', //meetDays 会期
		...
		appendPaymentList: function(tr) {
			...
			var html = '<tr Tcode="' + budgetfeiCode + '" Pcode="' + budgetCode + '" budgetProject="' + budgetProject + '" txtDate="' + budgetdata + '" place="' + budgetchang + '" costBreakdown= "' +
				budgetfei + '" bzy-span="' + BMoney + '" budgetMoney="' + numMoney + '" peoplenum="' + budgetpeople + '"> ' +
				'<td >' + budgetProject + '</td>' +
				'<td class="activityDate">' + budgetdata + '</td>' +
				'<td class="placeName">' + budgetchang + '</td> ' +
				'<td>' + budgetfei + '</td>' +
				/*'<td class="standardAmount" style="text-align: right;">' + budgetbiao + '</td> ' +*/
				'<td style="text-align: right;" class="money" money=' + numMoney + '>' + budgetMoney + '</td> ' +
				'<td class="peopleNum">' + budgetpeople + '</td> ' +
		...
		appendMsgList: function(tr, objId) {
			...
			if(tr) {
				tr.replaceWith('<tr pkId="'+ objId + '" applyName="' + applyName + '" Duties="' + Duties + '" Unit="' + Unit +'">' + 
								'<td class="order-number">1</td>'+
								'<td class="name" style="width: 100px;">' + applyName + '</td>'+
								'<td class="jobName" style="width: 100px;">' + Duties + '</td>'+ 
								'<td class="DutiesName" style="text-align: left;">' + Unit + '</td>'+ 
				...
				$('#targetList tbody').append('<tr pkId="" applyName="' + applyName + '" Duties="' + Duties + '" Unit="' + Unit + '">'+
										'<td class="order-number">1</td>'+
										'<td class="name" style="width: 100px;">' + applyName + '</td>' +
										'<td class="jobName" style="width: 100px;">' + Duties + '</td>'+
										'<td class="DutiesName" style="text-align: left;">'+ Unit + '</td>'+ 
		...
		submitAdvanceAppInfo: function(d, thisBtn) {
			...
			$.ajax({
				...
				success: function(data) {
					if(data.flag == 0) {
						d.appId = data.data;
						d.businessCode=$("#workflowCode").val();
						//提交业务流程
		...
		submitWorkFlow: function(d, thisBtn) {
			...
			$.ajax({
				type: "post",
				url: "sz/advanceApp/submitWorkFlowByBusinessCode",
				async: true,
				data: d,
				success: function(data) {
					if(data.flag == 0) {
			...
			$.ajax({
				...
				success: function(data) {
					if(data.flag == 0) {
						//成功跳转到事前申请审批列表页面
						//window.location.href = $yt_option.websit_path + 'view/system-sasac/expensesReim/module/approval/beforehandApproveList.html';
						var menUrl = 'view/system-sasac/expensesReim/module/approval/draftsList.html';
            if (window.frames.length == parent.frames.length) {
              //跳转至我的申请页面
              $yt_baseElement.openNewPage(1,menUrl,menUrl);
              window.close();
		...
		setformdata: function(d) {
			...
			//budgetBalanceAmount	预算剩余额度
			
			//advanceCostType	费用类型
			me.getCostTypeList(d.advanceCostType);
			//$('.docu-style-box .check-label input[value="' + d.advanceCostType + '"]').setRadioState('check');
			//advanceCostTypeName	费用类型名称
			//advanceAmount	预算总金额
		...
		showBusinessFun: function() {
			...
			//显示可支出金额字段
			$('#doAdvanceBox').hide();
			$("#workflowCode").val("JDSQ_APP_FLOW");
			sysCommon.getApproveFlowData("SZ_JDSQ_APP",'','{posCode:"'+ serveApply.parCode +'"}');
		},
		/**
		 * 一般费用
		 */
		...
		showTravelFun: function() {
			...
			//显示可支出金额字段
			$('#doAdvanceBox').hide();
			$("#workflowCode").val("TRAVEL_APP_FLOW");
		},
		/**
		 * 培训
		 */
		showTrainFun: function(code) {
			//清空原有代码
			$('.index-main-div').html('');
			//提示文字隐藏
			$('.qtip-text-div').hide();
		...
		showMettingFun: function() {
			...
			//显示可支出金额字段
			$('#doAdvanceBox').hide();
			$("#workflowCode").val("HYSQ_APP_FLOW");
			//加载类型
			serveApply.getMeetingTypeCode();
			sysCommon.getApproveFlowData("SZ_HYSQ_APP",'','{posCode:"'+ serveApply.parCode +'"}');
		},
		/**
		 * 差旅费列表相关事件
		 */
		...
		setTravelRouteList: function(tripPlanList) {
			...
			$.each(tripPlanList, function(i, n) {
				...
				tripHtml += '<tr busicode="' + n.travelType + '" usercode="' + n.travelPersonnels + '" rececode="' + (n.receptionCostItem) + '" >' +
					...
					'<td class="reception">' + (costItemName ? costItemName : '') + '</td>' +
					'<td>' +
					'<span class="operate-update"><img src="../../../../../resources-sasac/images/common/edit-icon.png"/></span>' +
					'<span class="operate-del"><img src="../../../../../resources-sasac/images/common/del-icon.png"/></span>' +
					'</td>' +
		...
		setCostSubsidyList: function(costSubsidyList) {
			...
			subHtml += '<tr class="last"><td>合计</td><td></td><td></td><td class="total-food"  style="text-align:right;">' + serveApply.fmMoney(totalFood) + '</td><td class="total-traffic"  style="text-align:right;">' + serveApply.fmMoney(totalTraffic) + '</td><td></td></tr>';
		...
		setCostTrainApplyInfoList: function(list) {
			...
			var total = 0;
			$.each(list, function(i, n) {
				html += '<tr costnameval="' + n.trainType + '" pid=""><td class="cost-name">' + n.trainTypeName + '</td><td class="moneyText standard-money">' + ($yt_baseElement.fmMoney(n.standard)) + '</td><td class="people-num">' + n.trainOfNum + '</td><td class="day-num">' + n.trainDays + '</td><td class="moneyText smallplan-money">' + n.averageMoney + '</td><td class="special-instruct">' + n.remark + '</td><td><input type="hidden" class="popM" value="4"><span class="operate-update"><img src="../../../../../resources-sasac/images/common/edit-icon.png"></span><span class="operate-del"><img src="../../../../../resources-sasac/images/common/del-icon.png"></span></td></tr>';
		...
		setCostPredictInfoList: function(list) {
			...
			$.each(list, function(i, n) {
				html += '<tr  pid=""><td class="cost-name">' + n.predictName + '</td><td class="moneyText predict-standard-money">' + ($yt_baseElement.fmMoney(n.predictStandardMoney)) + '</td><td class="predict-people-num">' + n.predictPeopleNum + '</td><td class="moneyText predict-smallplan-money">' + n.averageMoney + '</td><td class="special-instruct">' + n.remark + '</td><td><input type="hidden" class="popM" value="4"><span class="operate-update"><img src="../../../../../resources-sasac/images/common/edit-icon.png"></span><span class="operate-del"><img src="../../../../../resources-sasac/images/common/del-icon.png"></span></td></tr>';
		...
		setCostTeachersFoodApplyInfoList: function(list) {
			...
			$.each(list, function(i, n) {
				html += '<tr pid="' + n.foodId + '">' +
					...
					'<td class="avg">' + serveApply.fmMoney(n.averageMoney) + '</td>' +
		...
		setCostTeachersLectureApplyInfoList: function(list) {
			...
			$.each(list,function(i, n) {
				html += '<tr pid="' + n.teachersLectureId+ '">'+
						...
						'<td><input type="hidden" class="popM" value="0"/><span class="operate-update"><img src="../../../../../resources-sasac/images/common/edit-icon.png"></span><span class="operate-del"><img src="../../../../../resources-sasac/images/common/del-icon.png"></span></td></tr>';
			...
			$('#lectureFeeTable tbody .costTotal-after').text(serveApply.fmMoney(totalAfter));
		},
		/**
		 * //costTeachersTravelApplyInfoList	师资-城市间交通费json
		 * 设置 师资城市间交通费列表
		...
		setMeetingList: function(list) {
			$.each(list, function(i, n) {
				...
				$('#calculationSession').val(n.meetDays); //meetDays 会期
					...
					if(data.flag == 0 && list.length > 0) {
						$.each(list, function(i, n) {
							if(code && code == n.specialCode) {
								opts += '<option selected="selected" value="' + n.specialCode + '" include-attr="'+n.includeAttr+'" valid-type="'+n.validateType+'">' + n.specialName + '</option>';
							} else {
		...
		getBudgetBalanceAmount: function(specialCodeSeq) {
			$.ajax({
				type: "post",
				url: "budget/main/getBudgetAdvanceBalanceAmount",
				async: true,
				data: {
				...
				success: function(data) {
					if(data.flag == 0) {
						...
						$('#deptBudgetBalanceAmount').text(deptBudgetBalanceAmount ? serveApply.fmMoney(deptBudgetBalanceAmount <0 ?"0":deptBudgetBalanceAmount) + '万元' : '--');
						$("#deptBudgetBalanceAmountHidden").val(data.data.deptBudgetBalanceAmount);
						//单位预算额度
						var budgetBalanceAmount = data.data.budgetBalanceAmount;
						$('#budgetBalanceAmount').text(budgetBalanceAmount ? serveApply.fmMoney(budgetBalanceAmount) + '万元' : '--');
						//验证金额
						//serveApply.verifyBudgetExpendSum();
					}

var personnelFunds = {
	...
	initMoney: function() {
		...
		$('.predict-standard-money,.predict-people-num').on('blur',function() {
					...
					//赋值
					$('input.predict-smallplan-money').val($yt_baseElement.fmMoney(count || '0'))
			...
			$("#startTimeTop").calendar({
				...
				callback: function() {
					...
					if(tdStartDate != "" && tdEndDate != "") {
						...
						//计算会期
						$("#calculationSession").css("color", "#333333").text(diff_day + 1);
	...
	init: function() {
		...
		if($yt_common.request_params){
			if($yt_common.request_params.showType=='detail')$('.from-table-input').hide().siblings('.from-table-detail').show();
		}
		...
		$("#addPredictCostBtn").click(function() {
			//if($("#predictCostTable .yt-table .yt-tbody tr").size() < 1) {
			/*
			 * 预计收入费用
			 */
			...
			//}else {
			//	$yt_alert_Model.prompt("最多添加一条收入费用信息");
			//}
		});
		//关闭弹出框表单
				...
				switch(tabIndex) {
					...
					case 2:
						...
						trHtml = '<tr>' +
							...
							'<td class="text-overflow-sty" title="" data-text="remarks">' + specialRemark + '</td>' +
							'<td>' +
		...
		$("#trainingFeeTable").on('click', '.operate-update', function() {
			...
			$('#trainingFeePopM').find('select.cost-name').setSelectVal(costNameVal);
...
//				'<td class="day-num">' + dayNum + '</td>' +
	...
	showOtherAlert: function() {
		...
		sysCommon.showModel(alert);
  		//调用支持拖拽的方法 
		$yt_model_drag.modelDragEvent($("#oherTrainingAlert.yt-edit-alert .yt-edit-alert-title"));
	},
	/**
	 * 显示预计收入费用信息弹出框方法
	 */
	...
	costPopModelAddOrUp: function() {
		...
		$("#trainingTotal").blur(
				function() {
					...
					if (trainingTotal) {
						var aupr = $yt_baseElement.rmoney(trainingTotal)
								/ trainingDay / trainingPerNumber;
...
//		personnelFunds.hotCityList = sysCommon.getHotCityList();
	...
	trainingFeePopM : function(thisTr) {
		...
		$yt_alert_Model.getDivPosition($("#costPopModel"));
		$(
				'#costPopModel.yt-edit-alert .yt-eidt-model-bottom .yt-model-canel-btn,#costPopModel .closed-span')
		...
		$(
				'#oherTrainingAlert.yt-edit-alert .yt-eidt-model-bottom .yt-model-sure-btn')
				.off()
				.on(
						"click",
						function() {
							if (true) {
								var isFalse = $yt_valid
										.validForm(personnelFunds.thisPopM);
								if (isFalse) {
									var costName = $(
											"#trainingFeePopM .cost-name option:selected")
											.text();
									var costNameVal = $(
											"#trainingFeePopM .cost-name option:selected")
											.val();
									var standardMoney = $(
											"#trainingFeePopM .standard-money")
											.val();
									var peopleNum = $(
											"#trainingFeePopM .people-num")
											.val();
									var dayNum = $("#trainingFeePopM .day-num")
											.val();
									var smallplanMoney = $(
											"#trainingFeePopM .smallplan-money")
											.val();
									var specialInstruct = $(
											"#trainingFeePopM .specialInstruct")
											.val();
									/*specialInstruct = specialInstruct == '' ? '无'
											: specialInstruct;*/
									$("#trainingFeeTable .yt-tbody tr")
											.eq(-1)
											.before(
													'<tr costNameVal="'
															+ costNameVal
															+ '" pid=""><td class="cost-name">'
															+ costName
															+ '</td><td class="moneyText standard-money">'
															+ standardMoney
															+ '</td><td class="people-num">'
															+ peopleNum
															+ '</td><td class="day-num">'
															+ dayNum
															+ '</td><td class="moneyText smallplan-money">'
															+ smallplanMoney
															+ '</td><td class="special-instruct">'
															+ specialInstruct
															+ '</td><td><input type="hidden" class="popM" value="'
															+ personnelFunds.tabIndex
															+ '"/><span class="operate-update"><img src="../../../../../resources-sasac/images/common/edit-icon.png"></span><span class="operate-del"><img src="../../../../../resources-sasac/images/common/del-icon.png"></span></td></tr>');
									var totalAmount = 0;
									$(
											'#trainingFeeTable tbody .smallplan-money')
											.each(
													function() {
														totalAmount += $yt_baseElement
																.rmoney($(this)
																		.text())
													});
									$('#trainingFeeTable tbody .costTotal')
											.text(
													$yt_baseElement
															.fmMoney(totalAmount));
									personnelFunds
											.initModel(personnelFunds.thisPopM);
									$yt_alert_Model.prompt("填写的信息已成功加入到列表");
									personnelFunds.updateTotalNum()
								}
							} else {
		...
		$(
				'#costPopModel.yt-edit-alert .yt-eidt-model-bottom .yt-model-save-btn')
				.off()
				.on(
						"click",
						function() {
							...
							if (isFalse) {
								...
								var standardMoney = $(
										...
										.val();
								var peopleNum = $(
										"#trainingFeePopM .people-num").val();
								var dayNum = $("#trainingFeePopM .day-num")
										.val();
								var smallplanMoney = $(
										"#trainingFeePopM .smallplan-money")
								...
								$('#trainingFeeTable tbody .costTotal').text(
										$yt_baseElement.fmMoney(totalAmount));
								personnelFunds
										.initModel(personnelFunds.thisPopM);
								$("#costPopModel.yt-edit-alert,#heard-nav-bak")
										.hide();
								$("#pop-modle-alert").hide();
								$yt_alert_Model.prompt("填写的信息已成功加入到列表");
								personnelFunds.updateTotalNum()
	...
	lectureFeePopM: function(thisTr) {
		...
		$(personnelFunds.thisPopM.find(".afterTaxPay,.teachingHours")).blur(function() {
			...
			if(personnelFunds.thisPopM.find(".afterTaxPay").val() && personnelFunds.thisPopM.find(".teachingHours").val()) {
				...
				preTaxPay=$yt_baseElement.preTaxPaymoney($yt_baseElement.rmoney(afterTaxPay));

			...
			success: function(data) {
				...
				if(data.flag == 0) {
					...
					if(data.data.amountParams){
						$("#advanceAllAmount").text($yt_baseElement.fmMoney(data.data.amountParams.advanceAllAmount));//事前申请金额合计
						$("#expendAllAmount").text($yt_baseElement.fmMoney(data.data.amountParams.expendAllAmount));//支出金额(元)合计
						$("#differentAllAmount").text($yt_baseElement.fmMoney(data.data.amountParams.differentAllAmount));//差异金额(元)合计
					...
					if(datas.length > 0) {
						...
						$.each(datas, function(i, n) {
							trStr += '<tr>' +
								'<td>'+ (n.amountName == "师资费-课酬费" ? n.amountName + "<span style='font-size:12px;'>(不包含课酬相关税金)</span>" : n.amountName) +'</td>' +
								'<td style="text-align: right;">'+$yt_baseElement.fmMoney(n.advanceAmount)+'</td>' + //事前申请金额(元)
								'<td style="text-align: right;">'+$yt_baseElement.fmMoney(n.expendAmount)+'</td>' +  //支出金额(元)
								'<td style="text-align: right;" class="le-td">'+$yt_baseElement.fmMoney(n.differentAmount)+'</td>' +  //差异金额(元)
								'<td style="text-align: right;" class="gt-td">'+ (n.advanceScale * 100).toFixed(2) +'%</td>' +  //执行事前申请比例
								'</tr>';
						});

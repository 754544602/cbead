(function($, window) {
	var serveExamine = {
		...
		init: function() {
			var ts = this;
			serveExamine.appId = $yt_common.GetQueryString('appId'); //支出申请id
		...
		events: function() {
			...
			$('#advanceAppNum').click(function(){
				...
				var SQ = "SQ";
				if($("#costTypeName").text() == "培训费"){
					var pageUrl = "view/projectManagement-sasac/expensesReim/module/beforehand/beforeApplyDetail.html?advanceId=" + $("#advanceAppNumHiddenInput").val() + '&SQ=SQ';
				}else{
					var pageUrl = "view/system-sasac/expensesReim/module/beforehand/beforeApplyDetail.html?advanceId=" + $("#advanceAppNumHiddenInput").val() + '&SQ=SQ';
				}
			...
			$(".bottom-btn button.print-btn").on("click", function() {
				...
				if($(this).val() == "printBills") {
					pageUrl = "view/system-sasac/expensesReim/module/print/InvoicePasting.html?appId=" + serveExamine.appId;
				}
				//打印支出凭单
				if($(this).val() == "printExpend") {
					pageUrl = "view/system-sasac/expensesReim/module/print/expenditureVoucher.html?expenditureAppId=" + serveExamine.appId;
				}
				//打印支出凭单明细
				if($(this).val() == "printExpendDetail") {
					pageUrl = 'view/system-sasac/expensesReim/module/print/expenditureVoucherDetailed.html?expenditureAppId=' + serveExamine.appId+"&costType="+serveExamine.costType;
				}
				//调用公用的打开新页面方法传输参数不需要左侧菜单
				$yt_baseElement.openNewPage(2,pageUrl);
			});
 			//编辑页面跳转
       		 $("#expenseEdit").on("click",function(){
       			var systemId=$yt_common.GetQueryString("systemId");
				var menuId=$yt_common.GetQueryString("menuId");
				 //即将跳转页面
				var pageUrl=$yt_option.websit_path+'view/system-sasac/expensesReim/module/reimApply/expenseAccount.html?appId='+serveExamine.appId+"&systemId="+systemId+"&menuId="+menuId;
				//调用公用的跳转方法
			...
			$('table.payee-personal').on('click', '.to-data', function() {
				...
				//个人应收款总金额（元）
				$('#perPersonalTotal').text($yt_baseElement.fmMoney(tr.attr('personalTotal')) || '0.00');
				...
				//个人冲销后补领金额（元）
				$('.per-writeoff-money').text($yt_baseElement.fmMoney(tr.attr('personalWriteoff')) || '0.00');
				//收款方式：现金（元
				$('#perCash').text($yt_baseElement.fmMoney(tr.attr('cash')) || '0.00');
				//收款方式：公务卡（元）
				$('#perOfficialCard').text($yt_baseElement.fmMoney(tr.attr('officialCard')) || '0.00');
				//收款方式：转账（元）
				$('#perTransferAccounts').text($yt_baseElement.fmMoney(tr.attr('transferAccounts')) || '0.00');
				//身份证号码
		...
		getReimAppInfoDetailByReimAppId: function() {
			...
			$.ajax({
				...
				success: function(data) {
					...
					if(d.costType=='SPECIAL_DEDUCTION'){
							//将编辑按钮显示
							$("#expenseEdit").hide();
							//隐藏关闭按钮
							$("#closeBtn").show();
						...
						if(sysCommon.processState[0].operationState == '待申请人再次提交'||sysCommon.processState[0].operationState.indexOf('退回到申请人')>0){
							//将编辑按钮显示
							$("#expenseEdit").show();
							//隐藏关闭按钮
							$("#closeBtn").hide();
							//如果单据状态是退回删除详情页面的打印按钮,去除打印功能
							$("#printExpend,#printExpendDetail").remove();
						}
					}
					...
					if(d.costType=='ACCRUED_TAX' || d.costType=='SPECIAL_DEDUCTION'){
						...
						if(d.workFlowState == '已完结' || d.workFlowState == '已完成') {
							//最后一步显示财务支付相关
							serveExamine.setPayDetailList(d.payDetailList);
...
//					}
					...
					if($("#costTypeName").text() == "培训费") {
						$("#budgetPprojectDetailsTd").show();
						$("#budgetPprojectDetailsTd").prev("td").hide();
					}
					//隐藏部门或中心可用余额
		...
		showDetail: function(d) {
			...
			if(d.advanceAppNum) {
				...
				$('#advanceAppBalance').text(d.advanceAppBalance ? (fMoney(d.advanceAppBalance) + '元') : '0.00');
		...
		showCostDetail: function(data) {
			...
			$.each(gatheringUnitList, function(i, n) {
				otherPayeeHtml += '<tr loanAppId="' + n.loanAppId + '">' +
					'<td>' + n.unitName + '</td>' +
					'<td style="text-align:right;">' + $yt_baseElement.fmMoney(n.amount) + '</td>'+
//					if (n.reverseTheWayName == '借款单冲销') {
//						otherPayeeHtml += '<td class="to-detail"><a class="yt-link">' + n.reverseTheWayName + '</a></td>';
//					} else{
//						otherPayeeHtml += '<td>无</td>';
//					}
//					otherPayeeHtml += '<td >' + n.reverseLoanAmount + '</td>' + //冲销借款金额（元）
//					'<td >' + n.actualCollectionAmount + '</td>' + //实际收款金额（元）
					'<td >' + n.openBank + '</td>' +
					'<td >' + n.accounts + '</td>' +
					'<td >' + (n.isContract == 1 ? '是' : '否') + '</td>' +
					'<td >' + (n.isSettlement == 1 ? '汇款' : '支票') + '</td>' +
					'<td style="text-align:right;">' + (n.paidAmount == '' ? "0.00" : $yt_baseElement.fmMoney(n.paidAmount)) + '</td>' +
					'<td >' + (n.remarks || "") + '</td>' +
					'</tr>';
...
//					if (n.reverseTheWayName == '借款单冲销') {
//						gatheringPersonHtml += '<td class="to-detail"><a class="yt-link">' + n.reverseTheWayName + '</a></td>';
//					} else{
//						gatheringPersonHtml += '<td>无</td>';
//					}
//					gatheringPersonHtml += '<td style="text-align: right;">' + (serveExamine.fmMoney(n.writeOffAmount)) + '</td>'+
//				'<td style="text-align: right;">' + (serveExamine.fmMoney(n.replaceAmount)) + '</td>'+
				...
				'<td><a class="yt-link to-data">详情</a></td><td>' + (n.isContract == 1 ? '有' : '无') + '</td>'+
				'<td style="text-align:right;">' + (n.paidAmount == '' ? "0.00" : $yt_baseElement.fmMoney(n.paidAmount)) + '</td>'+
				'<td>' + (n.remarks || '') + '</td></tr>';
			...
			$.each(gatheringOtherList, function(i, n) {
				gatheringOtherHtml += '<tr loanAppId="' + n.loanAppId + '">' +
					'<td>' + n.otherName + '</td>' + //其他付款
					'<td style="text-align:right;">' + $yt_baseElement.fmMoney(n.amount) + '</td>'+ //金额（元）
//					if (n.reverseTheWayName == '借款单冲销') {
//						gatheringOtherHtml += '<td class="to-detail"><a class="yt-link">' + n.reverseTheWayName + '</a></td>';
//					} else{
//						gatheringOtherHtml += '<td>无</td>';
//					}
//					gatheringOtherHtml += '<td >' + n.reverseLoanAmount + '</td>' + //冲销借款金额（元）
//					'<td >' + n.actualCollectionAmount + '</td>' + //实际收款金额（元）
					'<td >' + (n.isContract == 1 ? '是' : '否') + '</td>' + //有无合同协议	
					'<td style="text-align:right;">' + (n.paidAmount == '' ? "0.00" : $yt_baseElement.fmMoney(n.paidAmount)) + '</td>' + //已支付金额
					'<td>' + (n.remarks || "") + '</td>' + //特殊说明
					'</tr>';
...
//				$(".subsidy-model").hide();
//			}
//			if(data.expenseDetails.length == 0 && data.costHotelList.length == 0 && data.costOtherList.length == 0 && data.costSubsidyList.length == 0) {
...
//					'</div></td></tr></table>');
//			}
//			if(data.expenseClassDetails.length == 0 && data.costTeachersFoodApplyInfoList == 0 && data.costTeachersLectureApplyInfoList == 0 && data.costTeachersTravelApplyInfoList == 0 && data.costTeachersHotelApplyInfoList == 0) {
			...
			if(list && list.length > 0) {
				$.each(list, function(i, n) {
					...
					html += '<tr class="">' +
						...
						'<td>' + n.paymentDate + '</td>' +
//						'<td>' + serveExamine.fmMoney(n.writeOffAmount) + '</td>' +
						'<td>' + serveExamine.fmMoney(n.cash) + '</td>' +
						'<td>' + serveExamine.fmMoney(n.officialCard) + '</td>' +
						'<td>' + serveExamine.fmMoney(n.transfer) + '</td>' +
						'<td class="row-total">' + (serveExamine.fmMoney(count)) + '</td>' +
						'</tr>';
					total += +count;
				});
			}

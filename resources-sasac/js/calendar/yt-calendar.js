...
jQuery.fn.extend({
	calendar: function(c) {
		...
		function s(a, b) { //初始化时间的列表
			...
			var upperLimit = !!c.upperLimit ? (Object.prototype.toString.call(c.upperLimit) === "[object String]" ? new Date(c.upperLimit) : (!!c.upperLimit.val() ? new Date(c.upperLimit.val()) : NaN)) : c.upperLimit;
			var lowerLimit = !!c.lowerLimit ? (Object.prototype.toString.call(c.lowerLimit) === "[object String]" ? new Date(c.lowerLimit) : (!!c.lowerLimit.val() ? new Date(c.lowerLimit.val()) : NaN)) : c.lowerLimit;
			if(!isNaN(lowerLimit)){
				lowerLimit = new Date(lowerLimit.getTime() - 24*60*60*1000)
				};
			for(var x = 0; x < 6; x++) {
				g += "<tr>";
				for(var y = 0; y < 7; y++) {
					var j = x * 7 + y + 1 - m;
					p = l = "";
					if(lowerLimit != NaN && lowerLimit > new Date(newDate.getFullYear(), newDate.getMonth(), j) || upperLimit != NaN && new Date(newDate.getFullYear(), newDate.getMonth(), j) > upperLimit)
						if(0 < j && j <= o) {
		...
		function start() {
			...
			//小时
			dateButtonBox.find(".calendar-hms input:eq(0)").val(inpuVal.getHours());
			//分钟
			dateButtonBox.find(".calendar-hms input:eq(1)").val(inpuVal.getMinutes());
			//秒
			dateButtonBox.find(".calendar-hms input:eq(2)").val(inpuVal.getSeconds());
			//输入框获取焦点选中
			dateButtonBox.find(".calendar-hms input").focus(function() {
				...
				if(Number($(this).val()) > 24 && $(this).hasClass("tH")) {
					$(this).val(24);
				}
			...
			//输入框中键盘落下事件
			dateButtonBox.find(".calendar-hms input").off().on("keydown", function(event) {
				//向下键
				if(event.keyCode == 40) {
					if($(this).val() == 0) {
						$(this).val(59);
			...
			/*今天*/
			$(dateButtonBox).find('.today').unbind('click').bind('click', function() {
				var dateStr = e + '-' + ((f + 1) < 10 ? '0' + (f + 1) : (f + 1)) + '-' + (q < 10 ? '0' + q : q);
				if(c.dateFmt == "yyyy-MM") {
					dateStr = e + '-' + ((f + 1) < 10 ? '0' + (f + 1) : (f + 1));
				} else if(c.dateFmt == "MM") {
					dateStr = (f + 1) < 10 ? '0' + (f + 1) : (f + 1);
				} else if(c.dateFmt == "yyyy") {
					dateStr = e;
				}
				$(n).val(dateStr);
		...
		if(c.nowData) {
			//默认选中当前时间
			if(c.dateFmt == "yyyy-MM") {
				$(n).val(e + '-' + ((f + 1) < 10 ? '0' + (f + 1) : (f + 1)));
			} else if(c.dateFmt == "yyyy-MM-dd") {
				$(n).val(e + '-' + ((f + 1) < 10 ? '0' + (f + 1) : (f + 1)) + '-' + (q < 10 ? '0' + q : q));
			} else if(c.dateFmt == "yyyy") {
				$(n).val(e);
			} else if(c.dateFmt == "MM") {
				$(n).val((f + 1) < 10 ? '0' + (f + 1) : (f + 1));
			}
		}
		//输入框的点击事件
		...
		n.bind("click focus",
			function(e) {
				if(/*$("#" + c.controlId + ":hidden").length != 0*/true) {
					...
					var a = $("#" + c.controlId),
						b = F(n[0]),
						d = $(n).offset().left + Number($(n).get(0).clientLeft) - 1;
					b = Number($(n).offset().top) + Number($(n).get(0).clientHeight) + 1;
					//获取当前页面滚动条距离顶部距离
					var winScrollTop = $(document).scrollTop();
					if(winScrollTop > 0){
						//判断如果当前页面滚动条距离顶部距离大于0,加上输入框高度,加上日期弹出框的高度
						b = b + Number($(n).height()) + Number($(a).height());
					}
					if((b+244)>$(window).height()){
						b = b-243-$(n).get(0).clientHeight;
					}
					if((d+222) >$(window).width()){
						d = d - 220 + $(n).get(0).clientWidth;
					}
					a.css({
						top: b + "px",
						left: d + "px"
		...
		$(document).mouseup(function(a) {
			if($(a.target).attr("id") != n.attr("id") && ($(a.target).parentsUntil("#" + c.controlId).parent().length == 0 ||
					$(a.target).parentsUntil("#" + c.controlId).parent()[0].id != c.controlId) &&  !$(a.target).hasClass('hover')) {
						$("#" + c.controlId).remove();
				}
		});
	}

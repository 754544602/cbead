...
(function($) {
	...
	function init(target) {
		...
		$(target).addClass('calendar').wrapInner(
			...
			//'<span>Aprial 2010</span>' +
			//'</a>' +
			'<a class="calendar-thisday" id="thisDay">' +
			'<span>今天</span>' +
			'</a>' +
			'</div>' +
	...
	function show(target) {
		...
		$(target).find('.calendar-title span').html('农历 ' + cyclical(opts.year - 1900 + 36) + '年 【' + Animals[(opts.year - 4) % 12] + '年】')
		...
		for(var i = 0; i < weeks.length; i++) {
			...
			for(var j = 0; j < week.length; j++) {
				var day = week[j];
				var bgColor = '';
				if(j == 0 || j == week.length - 1) {
					bgColor = 'background:#f5f5f5;';
				} else {
					bgColor = '';
				}
				var dayHtml = '<div class="clearfloat">';
				dayHtml += '<div class="td-day">' + day[2];
				var info = null;
				if(opts.month == day[1]) {
					info = currentCa[day[2] - 1];
					if(info.color) {
						var color = 'color="' + info.color + '"';
						dayHtml = '<div class="td-day" ' + color + '>' + day[2];
					}
					dayHtml += '</div>';
					if(opts.lunarDay) {
						//dayHtml += "<br/>";
						if(opts.solarTerms && info.solarTerms) {
							dayHtml += '<div color="#E0366E">' + info.solarTerms + '</div>';
						} else {
							dayHtml += '<div size="2" class="td-old" style="">' + cDay(info.lDay, info.lMonth) + '</div>';
						}
					}
				}
				dayHtml += '</div></div>';
				dayHtml += '<div class="td-coll"></div>';
				//单元格添加日期标记
				var day = $('<td style="' + bgColor + '" class="calendar-day fullcalendar-day calendar-other-month"></td>').data('info', info).attr('abbr', day[0] + '-' + day[1] + '-' + day[2]).html(dayHtml).appendTo(tr);
		...
		//临时添加背景样式
		$('.calendar-div td[abbr="2018-3-23"] .td-day').addClass('check');
		$('.calendar-div td[abbr="2018-3-15"],td[abbr="2018-3-23"],td[abbr="2018-3-29"],td[abbr="2018-3-30"]').find('.td-coll').addClass('bg');
	}
	...
	$('head').append('<style>.fullcalendar-day {border-left: 1px solid #eee;border-top: 1px solid #eee;} ' +
		...
		'.fullcalendar-detail{border-radius:5px;position: absolute; z-index: 111111; background-color: #E0366E; display: none; opacity: 0.8; padding: 5px; text-align: right;filter: Alpha(opacity=80);width:150px;}' +
	...
	$(function() {
		//今日按钮点击
		$('#conferenceCalendar').on('click', '#thisDay', function() {
			$.fn.fullCalendar.methods.moveTo($(".easyui-fullCalendar"), new Date());
		});
	});

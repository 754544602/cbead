(function($, window) {
	...
	function setCld(SY, SM) {
		...
		for(i = 0; i < 42; i++) {
			...
			solar_obj = $('#SD' + i);
			solar_obj.html('<div class="day-cont"><span class="td-day"></span><span class="td-old"></span></div><div class="day-coll"></div>');
			//lunar_obj = $('#LD' + i);
			...
			solar_obj.find('.td-day').text(sD + 1);
			if(sD > -1 && sD < cld.length) {
				...
				//solar_obj.css('color', cld[sD].color);
				
				if(cld[sD].l_d == 1) {
					solar_obj.find('.td-old').html('<span class="td-old">' + (cld[sD].isLeap ? '闰' : '') + cld[sD].l_m + '月' + (lunar_leap_d(cld[sD].lYear, cld[sD].l_m) == 29 ? '小' : '大') + '</span>');
				} else {
					solar_obj.find('.td-old').html('<span class="td-old">'+cal_d(cld[sD].l_d)+'</span>');
				}
				...
				if(s.length > 0) {
					solar_obj.find('.day-cont').html('<span class="td-old">'+s+'</span>');
				} //lunar_obj.innerHTML = s;
	...
	function appendCalTab() {
		var contentCalendar = $('#contentCalendar');
		contentCalendar.empty();

		var thead = '<table class="week" id="contCalenTable"><tr class="day"> <td>周日</td> <td>周一</td> <td>周二</td> <td>周三</td> <td>周四</td> <td>周五</td> <td>周六</td> </tr></table>';

		contentCalendar.append(thead);
		
		var contCalenTable = contentCalendar.find('#contCalenTable');
		...
		for(i = 0; i < 6; i++) {
			...
			for(j = 0; j < 7; j++) {
				Num = i * 7 + j;
				tab += '<td id="SD' + Num + '" onclick="addDay(' + Num + ')" ';
				//周六 周日 假期样式设定
				if(j == 0 || j == 6) {
					tab += ' class="aorange"';
				} else {
			...
			/*tab += '</tr></table></td></tr><tr><td><table style=""><tr style="text-align:center"> ';
			//农历
			for(j = 0; j < 7; j++) {
				Num = i * 7 + j;
				tab += '<td id="LD' + Num + '" onclick="addDay(' + Num + ')" class="bs" title=""> </td>';

			}*/
			tab += '</tr>';
			contCalenTable.append(tab);
		}
	}

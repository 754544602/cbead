...
var $yt_baseElement={
	...
	moneyInputInit:function(thisArea){
		...
		$(".yt-money-input").keyup(function(){ 
	        ...
	        if(tmptxt == 0 || tmptxt == "0"){
				...
				$(this).val(tmptxt.replace(".","$#$").replace(/\./g,"").replace("$#$",".")); 
			}
	    }).bind("paste",function(){     
	        var tmptxt=$(this).val();     
	        if(tmptxt == 0 || tmptxt == "0"){
   ...
   /**
	...
	rmoney: function(s) {
		//转成string类型
		s = s+"";
		return parseFloat(s.replace(/[^\d\.-]/g, ""))?parseFloat(s.replace(/[^\d\.-]/g, "")):0;
	},
	/**
	 * 税后和税前金额的自动计算
	 * @param {Object} 税前金额
	 */
	preTaxPaymoney: function(afterTaxPay) {
	 afterTaxPay = $yt_baseElement.rmoney(afterTaxPay)
		//应纳税额
		var taxpayable = 0;
		if(afterTaxPay<=800){
			taxpayable=0;
		}else if(afterTaxPay<=3360){
			taxpayable=(afterTaxPay-800)/4;
		}else if(afterTaxPay<=21000){
			taxpayable=0.16*afterTaxPay/0.84;
		}else if(afterTaxPay<=49500){
			taxpayable=(0.24*afterTaxPay-2000)/0.76;
		}else if(afterTaxPay<=49500){
			taxpayable=(0.24*afterTaxPay-2000)/0.76;
		}else if(afterTaxPay>49500){
			taxpayable=(0.32*afterTaxPay-7000)/0.68;
		}
		
		preTaxPay=Number(afterTaxPay)+Number(taxpayable);
		return preTaxPay;
	},
	/**
	 * 
	...
	showLoading:function(){
		...
		var loadingStr='<div class="yt-pop-model loading-model" style="text-align: center;position: fixed;background: none;">'
					  +'<div><img class="loading-img" style="width:160px;height:160px" src="./resources/images/icons/loading.gif"/></div>'
					  +'<div>';
   ...
   getToken: function() {
	 ...
	 * 
	 * */
	setProjectTypeName:function(projectType){
		if(projectType){
			if(projectType==2){
				return '委托'
   ...
   */
    ...
    */
    ...
    function create_nice_select($select) {
      ...
      $dropdown.find('.current').html($selected.data('display') || $selected.text());
       ...
       if($selected.text() !="请选择" && $selected.text()!='全部' && $selected.text() !='请输入'){
      ...
      $dropdown.find(".search-current").on("focus",function(){
      	  var isClear = false;
      	  if($(this).val()==null || $(this).val() == undefined || $(this).val() == "" || $(this).val() == "请选择" || $(this).val() == "全部" || $(this).val() == "请输入"){
      	  	 isClear = true;
      	  }
      	  if(isClear)
      	  {
        ...
        //判断是否包含此方法
	      ...
	      $dropdown.find('.current,.search-current').css("color","#333");
			...
			//调用判断下拉列表高度是否超出当前页面,改变显示位置
	   	    selectScrollChange($dropdown);
	   	    //添加选中option回调函数
	   	    if(option && option.optionCallBcak){
	   	    	option.optionCallBcak($option);
	   	    }
    ...
    */
    $(document).off("click").on('click.nice_select', function(event) {
    	...
    	var optionData =  elementDiv.data("optionData");
...
(function($) {
	...
	$.ajax = function(opt) {
		...
		//统一参数配置
		var dynamicKey = $yt_baseElement.getToken();
		...
		var _opt = $.extend(opt, {
			...
			success: function(data, textStatus) {
				...
				//成功回调方法增强处理
				try {
					if((jQuery.parseJSON(data).flag == undefined ? "" : jQuery.parseJSON(data).flag.toString()) == "1" || (jQuery.parseJSON(data).success == undefined ? "" : jQuery.parseJSON(data).success.toString()) == "1") {

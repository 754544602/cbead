var priorApproval = {
    ...
    $("#printBeforeForm").click(function() {
      var pageUrl = "view/projectManagement-sasac/expensesReim/module/print/beforeApplyDetailMain.html?expenditureAppId=" + priorApproval.reimId + '&costType=' + $("#advanceCostType").val(); //即将跳转的页面路径
      var goPageUrl = "view/projectManagement-sasac/expensesReim/module/approval/beforehandApproveList.html"; //左侧菜单指定选中的页面路径
					...
					if (data.data.length > 0) {
						$.each(data.data, function(i, n) {
							...
							if (n.dictTypeCode == "AMOUNT_NATURE") {
								optionText = '<option value="' + n.value + '">'
										+ n.disvalue + '</option>';
								...
								$("#moneyQuality").on(
										...
										function() {
											// 调用获取所属项目列表方法
											priorApproval.getProjectInfoList($(
													"#prjKeyWord").val(), $(
													this).val());
										});
							}
	...
	getInitFunDatas : function() {
		/**
		 * 
		 * 出差类型:TRAVEL_TYPE 交通工具:VEHICIE_CODE 住宿地点:HOTEL_ADDRESS 费用类型:COST_TYPE
		 * 资金性质:AMOUNT_NATURE
		 * 
		 */
		$.ajax({
						...
						function() {
							...
							$yt_common
									.parentAction({
										...
										data : {
											url : 'view/projectManagement-sasac/expensesReim/module/approval/beforehandApproveList.html'
										}
									});
						});

		// 是否属于专项选择事件
		$('.base-form-info input[name="special"]').on(
				"click",
				function() {
					// 1是2否
					if ($(this).val() == "2") {
						...
						$("#ppsProject").removeClass("valid-hint").next(
								".valid-font").html('');
						// 清空项目id
						$("#hidPrjId").val('');
					} else {
						...
						$("#projectNum").show();
						// 添加验证自定义属性
						$("#ppsProject").attr("validform",
								"{isNull:true,msg:\'请选择项目编号\'}");
					}
				});
	...
	clearPageData : function() {
		...
		$(
				"#priorApproval input:not(.radio-inpu,.hid-risk-code),#priorApproval textarea")
				.val('');
		$("#loanMoney").val('');
				...
				.pageInfo(
						{
							...
							url : $yt_option.websit_path
									+ 'resources-sasac/js/testJsonData/projectData.json', // ajax访问路径
												...
												.each(
														...
														function(i, n) {
															prjStr = '<tr>'
																	+ '<td>'
																	+ n.prjNum
																	+ '<input type="hidden" class="hid-prj-id" value="'
																	+ n.prjId
																	+ '"/>'
																	+ '<input type="hidden" class="hid-dept" value="'
																	+ n.deptSource
																	+ '"/>'
																	+ '</td>'
																	+ '<td>'
																	...
																	+ (n.budgetPrice == "" ? "--"
																			: $yt_baseElement
																					.fmMoney(n.budgetPrice))
																	+ '</td>'
																	+ '<td style="text-align: right;">'
																	+ (n.availablePrice == "" ? "--"
																			: $yt_baseElement
																					.fmMoney(n.availablePrice))
																	+ '</td>'
																	+ '</tr>';
															fromPrjTbody
																	.append(prjStr);
												...
												.each(
														...
														function(i, n) {
															applyStr = '<tr>'
																	+ '<td>'
																	+ n.travelAppNum
																	+ '<input type="hidden" class="hid-apply-id" value="'
																	+ n.travelAppId
																	+ '"/>'
																	+ '</td>'
																	+ '<td>'
	...
	setexaminedata : function(d) {
		$("#advanceCostType").val(d.advanceCostType);
		// advanceAppId 事前申请Id
	...
	showCostDetail : function(data) {
		...
		$.each(costReceptionistList, function(i, n) {
			receHtml += '<tr pkId="' + n.receptionObjectId
					+ '" class="" applyName="' + n.name + '" Unit="'
					+ n.jobName + '" Duties="' + n.unitName + '">'
					+ '<td><span class="num">' + (i + 1) + '</span></td>'
					+ '<td><span class="name-text">' + n.name + '</span></td>'
					+ '<td><span class="job-text">' + n.jobName
				...
				.each(
						...
						function(i, n) {
							detaHtml += '<tr Tcode="'
									...
									+ n.publicServiceProName
									+ '</span></td>'
									+ '<td><span class="activityDate">'
									+ n.activityDate
									+ '</span></td>'
									+ '<td><span class="place-name">'
									+ n.placeName
									+ '</span></td>'
									+ '<td><span>'
									+ n.costTypeName
		...
		detaHtml += '<tr><td>合计</td><td></td><td></td><td></td><td style="text-align: right;" class="total-money">'
				+ $yt_baseElement.fmMoney(thisTotal) + '</td><td></td></tr>';
		// 追加表格代码
		...
		$.each(tripPlanList, function(i, n) {
			...
			tripHtml += '<tr busicode="'
					...
					+ (getCostItemName(n.receptionCostItem) == '' ? '--'
							: getCostItemName(n.receptionCostItem)) + '</td>'
					+ '</tr>';
		});
				...
				.each(
						...
						function(i, n) {
							carHtml += '<tr>' + '<td><span>'
									...
									+ n.arrivalTime
									+ '</td>'
									+ '<td><input data-val="arrivalAddress" type="hidden" value="'
									+ n.arrivalAddress
									+ '"> <span data-text="arrivalAddressName">'
									+ n.arrivalAddressName
									+ '</span></td>'
									+ '<td><span data-text="vehicle">'
									+ n.vehicleName
									+ '</span><input type="hidden" class="hid-vehicle" value="'
									+ n.vehicle
									+ '"/><input type="hidden" class="hid-child-code" value=""/></td>'
									+ '<td class="font-right money-td" data-text="crafare">'
									+ (n.crafare == "" ? "--"
											: fMoney(n.crafare || '0'))
									+ '</td>'
									+ '<td class="text-overflow-sty" data-text="remarks" title="'
									+ n.remarks
									+ '">'
									+ (n.remarks || "无")
									+ '</td>'
				...
				.each(
						...
						function(i, n) {
							...
							hotelHtml += '<tr>' + '<td><span>'
									...
									+
									/*
									 * '<td data-text="hotelTime">' +
									 * $("#hotelDate").val() + '</td>' +
									 */
									'<td class="font-right">'
									+ avg
									+ '</td>'
									+ '<td risk-code-val="hotelCheckInDate" class="check-in-date"><span class="sdate">'
									+ n.hotelTime
									+ '</span></td>'
									+ '<td class="leave-date"><span class="edate">'
									+ n.leaveTime
									+ '</span></td>'
									+ '<td data-text="hotelDays">'
									+ n.hotelDays
									+ '</td><td class="font-right money-td" data-text="hotelExpense">'
									+ fMoney(n.hotelExpense || '0')
									+ '</td>'
									+ '<td><span data-text="hotelAddressName">'
									+ n.hotelAddressName
				...
				.each(
						...
						function(i, n) {
							otherHtml += '<tr>'
									...
									+ n.costType
									+ '"/></td>'
									+ '<td class="font-right money-td" data-text="reimAmount">'
									+ fMoney(n.reimAmount || '0')
									+ '</td>'
									+ '<td class="text-overflow-sty" data-text="remarks" title="'
									+ n.remarks + '">' + (n.remarks || "无")
									+ '</td>' + '</tr>';
							// 结算方式复选框存在并且有选中的值时
		...
		$.each(meetingCostList,
				function(i, n) {
					$("#meetHotel").text(
							$yt_baseElement.fmMoney((n.meetHotel || 0)));
					$("#meetFood").text(
							$yt_baseElement.fmMoney((n.meetFood || 0)));
					$("#meetOther").text(
							$yt_baseElement.fmMoney((n.meetOther || 0)));
					$("#meetAmount").text(
							$yt_baseElement.fmMoney((n.meetAmount || 0)));
					$("#meetAverage").text(
							$yt_baseElement.fmMoney((n.meetAverage || 0)));
				});
...
//				&& data.costTeachersFoodApplyInfoList == 0
//				&& data.costTeachersLectureApplyInfoList == 0
//				&& data.costTeachersTravelApplyInfoList == 0
//				&& data.costTeachersHotelApplyInfoList == 0) {
//			$("#costDetailDiv")
//					.html(
//							'<table style="width:100%"><tr style="border:0px;background-color:#fff !important;"><td align="center"style="border:0px;"><div class="no-data" style="width: 280px;padding:0;margin: 0 auto;">'
//									+ '<img src="../../../../../resources-sasac/images/common/no-data.png" alt="" style="padding:10px 0;">'
//									+ '</div></td></tr></table>');
//		}
//		if (data.costSubsidyList.length == 0 && data.costOtherList.length == 0
//				&& costHotelList.length == 0 && costCarfareList.length == 0) {
//			$(".cost-list-model")
//					.html(
//							'<table style="width:100%"><tr style="border:0px;background-color:#fff !important;"><td align="center"style="border:0px;"><div class="no-data" style="width: 280px;padding:0;margin: 0 auto;">'
//									+ '<img src="../../../../../resources-sasac/images/common/no-data.png" alt="" style="padding:10px 0;">'
...
//		}
		...
		$.ajax({
			...
			success : function(data) {
				if (data.flag == 0) {
					...
					var costTotalNum = $yt_baseElement.rmoney($("#predictCostTable .costTotal").text());
					//console.log(costTotalNum);
					var advanceAmountNum = parseFloat(d.advanceAmount);
					//console.log(advanceAmountNum);
					//$("#surtax").text($yt_baseElement.fmMoney(advanceAmountNum - costTotalNum));
					var afterTaxAllMoneyNum = $yt_baseElement.rmoney($("#afterTaxAllMoney").text());
					$("#vatSalesTax").text($yt_baseElement.fmMoney(afterTaxAllMoneyNum * 0.03));
					$("#surtax").text($yt_baseElement.fmMoney(afterTaxAllMoneyNum * 0.03 * 0.12));
					//隐藏部门或中心可用余额
	...
	getSaveData : function() {
		...
		d.advanceAttIdStr = priorApproval.getFileList(); // advanceAttIdStr
															// 附件idstr
					...
					success : function(data) {
						...
						if (data.flag == 0) {
							...
							window.location.href = $yt_option.websit_path
									+ 'view/projectManagement-sasac/expensesReim/module/approval/beforehandApproveList.html?state=1'
						}
						$yt_alert_Model.prompt(data.message);
						// 解除禁用
						thisBtn.attr('disabled', false).removeClass(
								'btn-disabled');
					}
				});
	},
	...
	showBusinessFun : function() {
		...
		// 加载区域页面
		sysCommon
				.loadingWord('view/projectManagement-sasac/expensesReim/module/reimApply/hospitalitySpendingDetail.html')
	},
	/**
	 * 一般费用
	 */
	...
	showTravelFun : function() {
		...
		// 加载区域页面
		sysCommon
				.loadingWord('view/projectManagement-sasac/expensesReim/module/reimApply/travelSpendingDetails.html');
	},
	/**
	 * 培训
	 */
	showTrainFun : function() {
		...
		// 加载区域页面
		sysCommon
				.loadingWord('view/projectManagement-sasac/expensesReim/module/busiTripApply/trainApproval.html')
	},
	/**
	 * 会议
	 */
	showMettingFun : function() {
		...
		// 加载区域页面
		sysCommon
				.loadingWord('view/projectManagement-sasac/expensesReim/module/beforehand/meetingCostApplyDetails.html')
	},
	/**
	 * 
	 * 表格金额合计更新
	 * 
	 * @param {Object}
	 *            thisTab费用表格标识0交通费1.住宿费2.其他费用3补助
	 * 
	 * 
	 */
	updateMoneySum : function(thisTab) {
		...
		if (thisTab == 3) {
			...
			$("#subsidy-list-info tbody .city-money").each(
					function(i, n) {
						cityMoneySum += parseFloat($yt_baseElement.rmoney($(n)
								.text()));
					});
	...
	/**
	 * trainApplyInfoList 师资-培训信息json 设置 师资培训信息列表数据
	 * 
	 * @param {Object}
	 *            list
	 */
	...
	setCostList:function(list){
		...
		$.each(list, function(i, n) {
			html='<tr>' +
					...
					'</td>' +
					'<td>' +
					'<span>' + $yt_baseElement.fmMoney(n.budgetCost) + '</span>' +
					'</td>' +
					'<td style="text-align:left;">' +
					'<p>' + n.accordingInstructions + '</p>' +
					'</td>' +
					'</tr>';
			if(n.costType == 1) {
				$('#TeachFeeTable').append(html);
			}
			if(n.costType == 2) {
				$('#trainingFeeTable').append(html);
			}
		});
	},
...
$("#afterTaxAllMoney").text($yt_baseElement.fmMoney(costTotalAfter));
	...
	/**
	 * teacherApplyInfoList 师资-讲师信息json 设置 师资讲师信息列表数据
	 * 
	 * @param {Object}
	 *            list
	 */
	setTeacherApplyInfoList : function(list) {
		...
		$
				.each(
						...
						function(i, n) {
							html += '<tr>'
									+ '<td class="lecturerName"><input type="hidden" class="lectureId" value="'
									+ n.lecturerId
									+ '"><span>'
									+ n.lecturerName
									+ '</span></td>'
		...
		// html += '<tr class="last"><td>合计</td><td></td><td></td><td
		// class="total-food" style="text-align:right;">' +
		// priorApproval.fmMoney(totalFood) + '</td><td class="total-traffic"
		// style="text-align:right;">' + priorApproval.fmMoney(totalTraffic) +
		// '</td></tr>';
		$('#lecturerTable tbody').html(html);
	},
	/**
	 * costTrainApplyInfoList 师资-培训费json 设置 师资培训费列表数据
	 * 
	 * @param {Object}
	 *            list
	 */
	setCostTrainApplyInfoList : function(list) {
		...
		$.each(list, function(i, n) {
			...
			html += '<tr><td>' + n.trainTypeName
					+ '</td><td class="moneyText">'
					+ standard + '</td><td>'
					+ trainOfNum + '</td><td>' + trainDays
					+ '</td><td class="moneyText">'
					+ $yt_baseElement.fmMoney(n.averageMoney) + '</td><td>'
					+ (n.remark || "无") + '</td></tr>';
			total += +n.averageMoney
		});
		html += '<tr class="last"><td>合计</td><td></td><td></td><td></td><td class="moneyText costTotal">'
				+ $yt_baseElement.fmMoney(total) + '</td><td></td></tr>';
				...
				.each(
						...
						function(i, n) {
							html += '<tr pid="'
									...
									+ n.lecturerName
									+ '</td>'
									+ '<td class="avg moneyText">'
									+ priorApproval.fmMoney(n.averageMoney)
									+ '</td>'
									+ '<td class="day">'
									+ n.foodOfDays
									+ '</td>'
									+ '<td class="moneyText sum-pay">'
	...
	/**
	 * //costTeachersLectureApplyInfoList 师资-讲课费json 设置 师资讲课费列表
	 * 
	 * @param {Object}
	 *            list
	 */
	setCostTeachersLectureApplyInfoList : function(list) {
		...
		$.each(
						...
						function(i, n) {
							html += '<tr pid="'
									...
									+ '</td>'
									+ '<td class="moneyText after">'
									+ priorApproval.fmMoney(n.afterTaxAmount)
									+ '</td>'
									+ '<td class="moneyText avg">'
									+ priorApproval.fmMoney(n.averageMoney)
									+ '</td>'
		...
		$('#lectureFeeTable tbody .costTotal-after').text(
				priorApproval.fmMoney(totalAfter));
		if (list.length == 0) {
			$('.lecture-money').hide();
				...
				.each(
						...
						function(i, n) {
							html += '<tr pid="'
									...
									+ n.arrivalTime
									+ '</td>'
									+ '<td class="sadd"><input type="hidden" class="scode" value="'
									+ n.goAddress
									+ '">'
									+ n.goAddressName
									+ '</td>'
									+ '<td class="eadd"><input type="hidden" class="ecode" value="'
									+ n.arrivalAddress
									+ '">'
									+ n.arrivalAddressName
									+ '</td>'
									+ '<td class="tname"><input type="hidden" class="tool" value="'
									+ n.vehicle
									+ '">'
									+ n.vehicleName
									+ '</td>'

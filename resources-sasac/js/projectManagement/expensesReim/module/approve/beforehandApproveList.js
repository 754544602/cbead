	...
	printBtnEvent:function(){
		...
		$("#printBeforBtn").click(function(){
			...
			if(selTrLen == 0){
				...
				var pageUrl = "view/projectManagement-sasac/expensesReim/module/print/beforeApplyDetailMain.html?expenditureAppId="+thisTrData.advanceAppId+ '&costType='+thisTrData.advanceCostType;//即将跳转的页面路径
...
//								'<td><a class="yt-link flowBtn">' + n.nodeNowState + '</a></td>' +
	...
	toDetails: function() {
		$('.yt-link.to-detail').off("click").on('click', function() {
			...
			/*页面跳转打开新页面*/
			var pageUrl = "view/projectManagement-sasac/expensesReim/module/beforehand/beforeApplyDetail.html?processInstanceId=" + processInstanceId+"&advanceId="+advanceId+"&fun="+fun;//即将跳转的页面路径
			//调用公用的打开新页面方法传输参数不需要左侧菜单
			$yt_baseElement.openNewPage(2,pageUrl);
		})
	},
	...
	goApprovePage: function() {
		...
		$(".apprv-btn").off().click(function() {
			...
			if(state == 'activitiStartTask') {
				//状态为申请人填报时 进入修改页面
				pageUrl = 'view/projectManagement-sasac/expensesReim/module/busiTripApply/serveApply.html?advanceId=' + advanceId;
			} else {
				//审批页面
				pageUrl = 'view/projectManagement-sasac/expensesReim/module/busiTripApply/priorApproval.html?advanceId=' + advanceId + '&fun=' + fun;
			}
			/**
			 * 调用显示loading方法
			 */

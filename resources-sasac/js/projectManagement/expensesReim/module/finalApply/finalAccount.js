(function($, window) {
	var serveFunds = {
		...
		expenditureList: [], //支出申请单列表数据
		expenditureInfoDetail: [], //支出申请单详细数据
		addressList: "", //全局的地区数据
		selUsersName: "", //用户名
		selUsersCode: "", //用户code
		usersInfoList: "", //出差人集合
		...
		//		//事前申请单和支出单选中与不选中判断
		//		hasOrNoAdvanceApplication:function(){
		//			if($(""))
		//		},
		/**
		 * 初始化组件
		...
		events: function() {
			...
			$('.clear-prior-approval').on('click', function() {
				...
				$yt_alert_Model.alertOne({
					...
					confirmFunction: function() {
						//清空其他费用明细列表的值
						$('#trainingFeeTable tbody .costTotal').text(0.00);
						$('#trainingFeeTable tbody').find("tr.trInfo").remove();
						//事前申请单号
						$('.prior-approval').val('');
						//支出申请单号
						...
						//可用余额
						$('#advanceAppBalance').text('0.00');
						//显示相关联的按钮及字段
						$('.advance-relevance').hide();
						//赋值id
						$('#advanceAppId').val('');
						//赋值id
						$('#checkadvanceAppId').val('');
						//预计收入总金额
						$('#predictTotal').val('');
						//隐藏导入按钮
						$('.export-but').hide();
						...
						$('.prj-name-tr').hide();
						//设置复选框不可选
						//$("#isExpenditureForm").attr("disabled",false);
						//导入按钮设置可用
						$('#trainDetails,#exportCostApp,#exportPredictCost,#importTrainingOther').attr("disabled", false);
						//导入数据集合置空
						serveFunds.beforeCostList = [];
					}
				});
			});
			//清除支出申请单选中
			$('.clear-expenditure-approval').on('click', function() {
				//显示提示信息
				$yt_alert_Model.alertOne({
					alertMsg: '变更支出申请单后，从支出申请单导入的数据将清空是否确定变更支出申请单？',
					confirmFunction: function() {
						//事前申请单号
						$('.expenditure-approval').val('');
						//隐藏导入按钮
						...
						me.clearPriorApproval();
						//隐藏清除按钮
						$('.clear-expenditure-approval').hide();
						//导入按钮设置可用
						$('#trainDetails,#exportCostApp,#exportPredictCost,#importTrainingOther').attr("disabled", false);
						//导入数据集合置空
						serveFunds.expenditureInfoDetail = [];
					}
				});
			});
			//项目预算表复选框设置
			/*$("#isExpenditureForm").on("click",function(){
				//根据复选框是否被选中来判断项目预算表input是否有效
				if($(this).is(":checked")){
					//选中，input框不可用
					$(".prior-approval").attr("disabled",true);
				}else{
					//未选中，input框可用
					$(".prior-approval").attr("disabled",false);
					//清空支出单内容
					$(".expenditure-approval").val("");
					//隐藏关闭图像按钮
					$(".clear-expenditure-approval").hide();
			...
			//决算申请事由
			$("#finalAppReason").on("change", function() {
				var value = $(this).val();
				if(value) {
					//项目名称
					$("#prjName").val(value);
			...
			/*
			 选择支出申请单列表点击事件
			 * */
			$('.expenditure-approval-list tbody').on('click','tr',function(){
			...
			$('.expenditure-approval-list tbody').on('change','input[type=checkbox]',function(){
				if(this.checked){
		...
		setSubmitEvent: function() {
			...
			$('#submitPayment').on('click', function() {
				...
				var surtaxMoneyAmount = $yt_baseElement.rmoney($("#surtax").text());
				var availableBalanceSheetAmount = $yt_baseElement.rmoney($("#advanceAppBalance").text());
				/**
				 * 
...
//					//获取部门预算可用余额
//					budgetMoney = $yt_baseElement.rmoney($('#deptBudgetBalanceAmount').text().replace('万元', '')) * 10000;
...
//				}
//				if(applyTotalMoneyAmount - surtaxMoneyAmount > budgetMoney) {
//					$yt_alert_Model.alertOne({
...
//					});
//				} else 
				if(applyTotalMoneyAmount - surtaxMoneyAmount > availableBalanceSheetAmount) {
					$yt_alert_Model.alertOne({
						haveCloseIcon: true, //是否带有关闭图标  
						leftBtnName: "确定", //左侧按钮名称,默认确定  
						cancelFunction: "", //取消按钮操作方法*/  
						alertMsg: "当前项目决算中支出费用金额超出项目预算可用余额,请对相关报销单进行修改后再进行决算", //提示信息  
						cancelFunction: function() { //点击确定按钮执行方法  
						},
					});
				} else {
		...
		uploadFile: function() {
			...
			$(".file-up-div").off().on('change', '.cont-file', function(obj) {
				...
				$.ajaxFileUpload({
					...
					success: function(data, textStatus) {
						...
						//图片预览
						$('#finalIdStr .file-pv img').showImg();
					},
			...
			//图片下载
			$('#finalIdStr').on('click', '.file-dw', function() {
					...
					$yt_alert_Model.alertOne({
						...
						confirmFunction: function() { //点击确定按钮执行方法  
							if(code) {
								...
								me.getExpenditureList(id);
//								me.getExpenditureAppInfoByAppId();
								//赋值支出申请单号
								me.setExpenditureVal();
								//赋值事前申请单号  清除验证样式及内容
								$('.prior-approval').val(code).attr('costType', types).removeClass('valid-hint').nextAll().find('.valid-font').text('');
								//可用余额advanceAppBalance
								var advBalan = $yt_baseElement.rmoney(balance) + $yt_baseElement.rmoney(serveFunds.adMoney)
								$('#advanceAppBalance').text(balance ? $yt_baseElement.fmMoney(advBalan) : '--');
								//培训费提示文字
...
//						me.getExpenditureAppInfoByAppId();
						...
						//设置复选框不可选
						//$("#isExpenditureForm").attr("disabled",true);
						me.getCostDetilData();
		 ...
		 * 
		 */
		getCostDetilData: function(costData, zcTr) {
			var me = this;
			//培训费其他费用明细详情
			//验证单据类型与事前申请单类型
			if($(".prior-approval").val() != '') {
				...
				if(costData != undefined) {
					...
					me.setCostTeachersFoodApplyInfoList(data.costExpenditureTeachersFoodApplyInfoList, true, zcTr);
					//costTeachersLectureApplyInfoList	师资-讲课费json
					me.setCostTeachersLectureApplyInfoList(data.costExpenditureTeachersLectureApplyInfoList, true, zcTr);
					//costTeachersTravelApplyInfoList	师资-城市间交通费json
					me.setCostTeachersTravelApplyInfoList(data.costExpenditureTeachersTravelApplyInfoList, true, zcTr);
					//costTeachersHotelApplyInfoList	师资-住宿费 json
					me.setCostTeachersHotelApplyInfoList(data.costExpenditureTeachersHotelApplyInfoList, true, zcTr);
					//计算总金额
					personnelFunds.updateTotalNum();
				} else {
					data = me.beforeCostList.costData; //项目预算
					//costTeachersFoodApplyInfoList	师资-伙食费json
					me.setCostTeachersFoodApplyInfoList(data.costExpenditureTeachersFoodApplyInfoList, true, zcTr);
					//costTeachersLectureApplyInfoList	师资-讲课费json
					me.setCostTeachersLectureApplyInfoList(data.costExpenditureTeachersLectureApplyInfoList, true, zcTr);
			...
			} else if($(".prior-approval").val() == '') {
				//取得保存的支出申请单数据
				//var data = me.expenditureInfoDetail.costData.costTrainApplyInfoList;
				//取得保存的支出申请单数据
				var data = me.expenditureInfoDetail.costData;
				//costTeachersFoodApplyInfoList	师资-伙食费json
				...
				//me.setCostTrainApplyInfoList(data, true);
				//计算总金额
				personnelFunds.updateTotalNum();
			} else {
		...
		setExpenditureVal: function() {
			...
			if(serveFunds.expenditureList.rows.length > 0) {
				$.each(serveFunds.expenditureList.rows, function(i, n) {
					...
					if (n.workFlowState == '已完成') {
						money += $yt_baseElement.rmoney(n.totalAmount);
					}
				})
				//去掉末尾逗号
			...
			serveFunds.adMoney = money;
		},
		/**
		 * 事前申请单选中后填充表头信息
		 */
		advanceDataFilling: function(d) {
			var me = this;
			//			var d = serveFunds.beforeCostList;
			//判断事前申请单是否为空
			if($('.prior-approval').val()) {
				$('#finalAppReason').val(d.advanceAppName); //reimAppName	支出事由
				//costTypeName	费用类型名称
		 ...
		 * 支出单选中后填充表头信息
		 */
		expenditureDataFilling: function(d) {
			var me = this;
			//判断事前申请单是否为空
			if($('.expenditure-approval').val()) {
				$('#finalAppReason').val(d.expenditureAppName); //	决算申请事由
				//costTypeName	费用类型名称
				var specialCodeArr = d.specialCode.split('-'); //specialCode	所属预算项目code
				var specialNameArr = d.specialName.split('-'); //specialName	所属预算项目名称
				//设置所属预算项目数据
		...
		expenditureApprovalEvent: function() {
			...
			$('.expenditure-approval').click(function() {
				...
				if($(".prior-approval").val() == '') {
					var advanceAppId = $("#checkadvanceAppId").val();
					//清空导入数据
					//me.clearPriorApproval();
					//清空其他费用明细列表的值
					//$('#trainingFeeTable tbody .costTotal').text(0.00);
					//$('#trainingFeeTable tbody').find("tr.trInfo").remove();
					//获取数据
					me.getExpenditureList(advanceAppId);
					//显示弹框及蒙层
					$yt_baseElement.showMongoliaLayer();
					...
					//事前申请单内容
					var advanceApplication = $(".prior-approval").val();
					if(advanceApplication) {
						$(".expenditure-common").hide();
					} else {
						$(".expenditure-common").show();
					}
				} else if($(".prior-approval").val() != '') {
					var advanceAppId = $("#checkadvanceAppId").val();
					//清空导入数据
			...
			//确定事件
			$('.expenditure-common').click(function() {
				/*$("#lectureFeeTable .yt-tbody tr:not(.end-tr)").empty();
				$("#hotelFeeTable .yt-tbody tr:not(.end-tr)").empty();
				$("#carFeeTable .yt-tbody tr:not(.end-tr)").empty();
				$("#dietFeeTable .yt-tbody tr:not(.end-tr)").empty();*/
				//判断是否有选中行
				if($(".expenditure-alert table tr.yt-table-active").length == 0) {
				...
				$(".zcTr").remove();
				//选中行的对象
				var tr = $('.expenditure-approval-list .yt-table-active');
				//选中行的code
				var code = '';
				//选中行的金额
				var money = 0;
				$(".expenditure-approval-list label.check").each(function(i, n) {
					code += $(n).parent().parent().attr('code') + (i < $('.yt-table tbody .check').length - 1 ? ',' : '');
					if ($(n).parents('tr').attr('workFlowState') == '已完成') {
						money += $yt_baseElement.rmoney($(n).parent().parent().attr('money'));
					}
				});
				//获取可用余额
				var advMoney = $yt_baseElement.rmoney($('#advanceAppBalance').text());
				console.log(money);
				//事前申请id
				var id = tr.attr('pid');

				var thisVal = $('.expenditure-approval').val();
				if(thisVal) {
					//提示信息
					$yt_alert_Model.alertOne({
						alertMsg: "变更支出申请单后，从支出申请单导入的数据将清空是否确定变更支出申请单？", //提示信息  
						confirmFunction: function() { //点击确定按钮执行方法  
							//清空导入数据
							me.clearPriorApproval();
							//清空其他费用明细列表的值
							//$('#trainingFeeTable tbody .costTotal').text(0.00);
							$('#trainingFeeTable tbody').find("tr.asas").remove();
							//隐藏弹框及蒙层
							$yt_baseElement.hideMongoliaLayer();
							$('.expenditure-alert').hide();
							$('#pop-modle-alert').hide();
							//显示清除按钮
							$('.clear-expenditure-approval').show();
							//成功后显示导入按钮 .index-main-div 
							$('.export-but').show();
							//清空id
							//$("#checkadvanceAppId").val("");
							//支出申请单
							$(".expenditure-approval").val(code);
							//可用余额
							$('#advanceAppBalance').text($yt_baseElement.fmMoney(advMoney + money));
							//获取事前申请数据
							var ids = '';
							...
							if(ids) {
								me.getReimAppInfoDetailByReimAppIds(ids);
								...
								$.each(idList, function(i, n) {
									...
									$.ajax({
										...
										data: {
											expenditureAppId: pkId
										},
										success: function(data) {
											if(data.flag == 0) {
												//var total = $yt_baseElement.rmoney($("#trainingFeeTable .costTotal").text());
												//$('#trainingFeeTable tbody tr.asas').remove();
												var html = "";
												var total = 0;
												if(data.data.costType == 'TRAVEL_APPLY') {
													html += '<tr class="trInfo asas" costnameval="COST_OTHER9" pid="">' + //类型写死  如数据库更改  需修改此处
														'<td class="cost-name">差旅费</td>' +
														'<td class="moneyText standard-money"></td>' +
														'<td class="people-num"></td>' +
														'<td class="day-num"></td>' +
														'<td class="moneyText smallplan-money">' + serveFunds.fmMoney(data.data.totalAmount) + '</td>' +
														'<td class="special-instruct"><input type="hidden" class="popM" value="4"></td>' +
														'</tr>';
													$('#trainingFeeTable tbody tr.last').before(html);
												} else {
													if(data.data.costData.costTrainApplyInfoList.length > 0) {
														$.each(data.data.costData.costTrainApplyInfoList, function(i, n) {
															html += '<tr class="trInfo asas" costnameval="' + n.trainType + '" pid="">' + //类型写死  如数据库更改  需修改此处
												...
												var zcTr = "zcTr";
												//costTeachersFoodApplyInfoList	师资-伙食费json
												/*serveFunds.setCostTeachersFoodApplyInfoList(data.data.costData.costTeachersFoodApplyInfoList, true, zcTr);
												//costTeachersLectureApplyInfoList	师资-讲课费json
												serveFunds.setCostTeachersLectureApplyInfoList(data.data.costData.costTeachersLectureApplyInfoList, true, zcTr);
												//costTeachersTravelApplyInfoList	师资-城市间交通费json
												serveFunds.setCostTeachersTravelApplyInfoList(data.data.costData.costTeachersTravelApplyInfoList, true, zcTr);
												//costTeachersHotelApplyInfoList	师资-住宿费 json
												serveFunds.setCostTeachersHotelApplyInfoList(data.data.costData.costTeachersHotelApplyInfoList, true, zcTr);*/
												var total = 0;
												$.each($("#trainingFeeTable tbody tr"), function(i, n) {
													if($(n).find(".smallplan-money").text() != "NaN" && $(n).find(".smallplan-money").text() != NaN && $(n).find(".smallplan-money").text() != "") {
														total += $yt_baseElement.rmoney($(n).find(".smallplan-money").text());
													}
												});
												$('#trainingFeeTable tbody tr.last').find(".costTotal").text($yt_baseElement.fmMoney(total));
												me.getCostDetilData(data.data.costData, zcTr);
											}
										}
									});
								});
		...
		getExpenditureList: function(advanceId) {
			...
			$("#checkadvanceAppId").val(advanceId);
			//分页区域
						...
						if(data.data.rows.length > 0) {
							...
							$.each(data.data.rows, function(i, n) {
								...
								if(appName.length > 17) {
									appName = n.appName.substr(0, 15) + "...";
								}
								trStr += '<tr pid="' + n.pkId + '" code="' + n.appNum + '" money="' + n.totalAmount + '" workFlowState="' + n.workFlowState + '">' +
									'<td><label style="position:relative;z-index:100" class="check-label yt-checkbox"><input type="checkbox" name="test" value="' + n.pkId + '"/></label>' + '</td>' +
									'<td>' + n.appNum + '</td>' +
									'<td>' + n.applicantUserName + '</td>' +
									'<td>' + n.applicantUserDeptName + '</td>' +
									'<td title=' + n.appName + '>' + appName + '</td>' +
							...
							//获取单号id的集合
							if(data.data.rows.length > 0) {
								serveFunds.pageByPar =  data.data.rows;
							}
						} else {
							//隐藏分页
							pageDiv.hide();
		 ...
		 * @param {Object} 
		 */
		getExpenditureAppInfoByAppId: function(){
			$.each(serveFunds.pageByPar, function(i, n) {
				$.ajax({
					type: "post",
					url: "sz/expenditureApp/getExpenditureAppInfoByAppId",
					data: {
						expenditureAppId: n.pkId
					},
					success: function(data) {
						if(data.flag == 0) {
							var total = 0;/*$yt_baseElement.rmoney($("#trainingFeeTable .costTotal").text());*/
							var totalAM = $yt_baseElement.rmoney($("#trainingFeeTable .costTotal").text());
							//$('#trainingFeeTable tbody tr.asas').remove();
							var html = "";
							if(data.data.costType == 'TRAVEL_APPLY') {
								html += '<tr class="trInfo asas" costnameval="COST_OTHER9" pid="">' + //类型写死  如数据库更改  需修改此处
...
//											total += + n.averageMoney;
							...
							$('#trainingFeeTable tbody tr.last').before(html);
							$('#trainingFeeTable tbody tr.last').find('.costTotal').text($yt_baseElement.fmMoney(total + totalAM))
							var zcTr = "zcTr";
							//costTeachersFoodApplyInfoList	师资-伙食费json
							/*serveFunds.setCostTeachersFoodApplyInfoList(data.data.costData.costTeachersFoodApplyInfoList, true, zcTr);
							//costTeachersLectureApplyInfoList	师资-讲课费json
							...
							$("#carFeeTable .yt-tbody tr:not(.end-tr)").empty();
							$("#dietFeeTable .yt-tbody tr:not(.end-tr)").empty();*/
							serveFunds.getCostDetilData(data.data.costData, zcTr);
						}
						/*else if(data.data.costType == "TRAIN_APPLY"){
		             		var otherInfo = data.data.costData.costTrainApplyInfoList;
		             		console.log(otherInfo);
		             		var html = "";
		             		var total = $(".costTotal").text();
		             		$.each(otherInfo, function(i,n) {
		             			//处理数据为零的事项
								...
								var trainDays = (n.trainDays == '0' || n.trainDays == '') ? '':n.trainDays;
			             		html += '<tr class="" costnameval="' + n.trainType + '" pid="">'+
									'<td class="cost-name">' + n.trainTypeName + '</td>'+
									'<td class="moneyText standard-money">' + standard + '</td>'+
									'<td class="people-num">' + trainOfNum + '</td>'+
									'<td class="day-num">' + trainDays + '</td>'+
									'<td class="moneyText smallplan-money">' + serveFunds.fmMoney(n.averageMoney) + '</td>'+
									'<td class="special-instruct"><input type="hidden" class="popM" value="4">' + n.remark + '</td>'+
									'</tr>';
									total += + n.averageMoney;
		             		});
							$('#trainingFeeTable tbody tr.last').before(html).find('.costTotal').text(serveFunds.fmMoney(total));
		 ...
		 */
		getReimAppInfoDetailByReimAppIds: function(ids) {
			var me = this;
			$.ajax({
				type: "post",
				url: "sz/expenditureApp/getExpenditureAppInfoListByAppIds",
				async: true,
				data: {
					expenditureAppIds: ids
				},
				success: function(data) {
					serveFunds.expenditureInfoDetail = data.data;
					$('.from-table-detail').setDatas(data.data.costData.trainApplyInfoList[0]);
					//支出申请自动填充表头
					serveFunds.expenditureDataFilling(data.data);
					//重新设置数据
					//serveFunds.getCostDetilData(data.data.costData);
					//serveFunds.getCostDetilData(data.data.costData);
					//获取数据集
					var datas = data.data.costData;
					//zcTr类名 用来标注导入的信息方便清除
					var zcTr = 'zcTr';
					//costTrainApplyInfoList	师资-培训费json
					//me.setCostTrainApplyInfoList(datas.costTrainApplyInfoList, true, zcTr);
					//costTeachersFoodApplyInfoList	师资-伙食费json
					me.setCostTeachersFoodApplyInfoList(datas.costTeachersFoodApplyInfoList, true, zcTr);
					//costTeachersLectureApplyInfoList	师资-讲课费json
					me.setCostTeachersLectureApplyInfoList(datas.costTeachersLectureApplyInfoList, true, zcTr);
					//costTeachersTravelApplyInfoList	师资-城市间交通费json
					me.setCostTeachersTravelApplyInfoList(datas.costTeachersTravelApplyInfoList, true, zcTr);
					//costTeachersHotelApplyInfoList	师资-住宿费 json
					me.setCostTeachersHotelApplyInfoList(datas.costTeachersHotelApplyInfoList, true, zcTr);
					//costNormalList	普通报销-费用明细/普通付款-付款明细 json
					//me.setCostNormalList(datas.costNormalList, true);
		
					//meetingList 会议详情json
					me.setMeetingList(datas.meetingList, true);
					//meetingCostList 会议费用详情
					me.setMeetingCostList(datas.meetingCostList, true);
					//计算总和
					me.updateTotalNum();
				}
			});
		...
		getPriorApprovalList: function() {
			...
			$('.prior-page').pageInfo({
				...
				pageNum: 5, //每页显示条数  
				pageSize: 3, //显示...的规律  
				url: "sz/advanceApp/getProjectFinalAccountsListToPageByParams", //ajax访问路径  
				objName: 'data',
				data: {
					type: 'FINAL_APP',
					costType: 'TRAIN_APPLY',
		...
		submitWorkFlow: function(subData, thisBtn) {
			$.ajax({
				...
				success: function(data) {
					$yt_alert_Model.prompt(data.message);
					if(data.flag == 0) {
						var pageUrl = "view/projectManagement-sasac/expensesReim/module/print/finalStatement.html?finalAppId=" + subData.appId;
		...
		getSubData: function() {
			...
			var costData = function() {
				return JSON.stringify({
					...
					expenseDetails:me.expenseDetails,
					expenseClassDetails:me.expenseClassDetails
				});
			};
			...
			return {
				advanceAppId: $("#advanceAppId").val(), //事前申请单id
				...
				costType: 'TRAIN_APPLY',
				expenditureTotal: $yt_baseElement.rmoney($(".count-val-num").text() || '0'),
				incomeTotal: $yt_baseElement.rmoney($("#afterTaxAllMoney").text() || '0'),
				predictTotal: $yt_baseElement.rmoney($("#predictTotal").text() || '0'),
				prjName: $('#prjName').val(),
				alicantUser: $("#alicantUser").val(),
				...
				costData: costData(),
				attIdStr: me.getFileList(),
				finalAttIdStr: me.getFinalFileList()
			};
		},
		/**
		 * 获取附件id字符串
		 */
		getFinalFileList: function() {
			var str = '';
			//获取所有的文件列表
			var list = $('#finalIdStr .li-div');
			$.each(list, function(i, n) {
		...
		costTaxInfoList: function() {
			var list = [];
			//取课酬费税金的值
			var taxAmount = serveFunds.rmoney($(".taxation .tax-amount").text());
			//取增值税销项税的值
			var addedValue = serveFunds.rmoney($(".taxation .added-value").text());
			//取	应缴附加税的值
			var surtax = serveFunds.rmoney($(".taxation .surtax").text());
			//税费信息
			list.push({
				taxation: "税费信息",
...
//			}] : [];
		...
		setHeringUnitList: function(list) {
			...
			var total = 0;
			$.each(list, function(i, n) {
				html += '<tr class="payee-unit-tr" pkid="" witeoffcompany="' + n.reverseTheWay + '" companyname="' + n.unitName + '" openbank="' + n.openBank + '" accountnumber="' + n.accounts + '" companymoney="' + n.amount + '" contractradio="' + n.isContract + '" companyspecial="' + n.remarks + '"><td class="com" value="Company">' + n.unitName + '</td><td>' + (n.reverseTheWayName ? n.reverseTheWayName : '无') + '</td><td style="text-align: right;" class="unitTotal">' + ($yt_baseElement.fmMoney(n.amount)) + '</td><td>' + n.openBank + '</td><td>' + n.accounts + '</td><td value="1">' + (n.isContract == 1 ? '有' : '无') + '</td><td>' + (n.remarks ? n.remarks : '') + '</td><td><span class="operate-update"><img src="../../../../../resources-sasac/images/common/edit-icon.png"></span><span class="operate-del"><img src="../../../../../resources-sasac/images/common/del-icon.png"></span></td></tr>';
				total += +n.amount;
			});
		...
		setHeringOtherList: function(list) {
			...
			var total = 0;
			$.each(list, function(i, n) {
				html += '<tr class="payee-other-tr" pkid="" witeoffother="' + n.reverseTheWay + '" othermoney="' + n.otherName + '" otherallmoney="' + n.amount + '" otherradio="' + n.isContract + '" otherspecial="' + n.remarks + '"><td class="oth" value="Other">' + n.otherName + '</td><td>' + n.reverseTheWayName + '</td><td style="text-align: right;" class="otherTotal">' + serveFunds.fmMoney(n.amount) + '</td><td>' + (n.isContract == 1 ? '有' : '无') + '</td><td>' + (n.remarks ? n.remarks : '') + '</td><td><span class="operate-update"><img src="../../../../../resources-sasac/images/common/edit-icon.png"></span><span class="operate-del"><img src="../../../../../resources-sasac/images/common/del-icon.png"></span></td></tr>';
				total += +n.amount;
			});
...
//				serveFunds.getAdvanceAppInfoDetailByAdvanceAppId(serveFunds.saveData.advanceAppId || '', true);
			...
			$('#finalAppId').val(d.finalAppId); //finalAppId	决算申请id
			$('#advanceAppId').val(d.advanceAppId); //advanceAppId	事前申请id
			$('#checkadvanceAppId').val(d.advanceAppId);
			$('.prior-approval').val(d.advanceAppNum).attr('costtype', d.costType); //事前申请编号
			$('.expenditure-approval').val(d.expenditureAppNum);
			if(d.advanceAppNum) {
				//赋值显示事前审批单可用余额
				$('#advanceAppBalance').text(d.advanceAppBalance ? serveFunds.fmMoney(d.advanceAppBalance) : '无');
				$('.advance-relevance').show();
			}
			...
			$('#formNum').text(d.finalAppNum || '--'); //决算申请单编号
			...
			//advanceAppAttList	附件集合
			me.showFileList(d.advanceAppAttList);
			me.showFinalFileList(d.attList);
			//costData	费用申请数据
		...
		showCostDetail: function(data) {
			...
			me.expenseClassDetails = data.expenseClassDetails;
			me.expenseDetails = data.expenseDetails;
			//税费信息
...
//			//收费总金额（税前）
//			$("#TotalMoneyUpper,.total-money-up").text(arabiaToChinese(applyTotalMoney || '0'));
...
//			$(".taxation .total").text($yt_baseElement.fmMoney(taxationTotal || '0'));
		...
		showFileList: function(list) {
			...
			if(list.length > 0) {
				...
				$('#attIdStr').html(ls);
				$('#attIdStr .file-dw').off().on(
					'click',
					function() {
						var id = $(this).parent().attr('fid');
						window.location.href = $yt_option.base_path +
							'fileUpDownload/download?pkId=' + id +
							'&isDownload=true'
					});
				$('#attIdStr .file-pv img').showImg()
			} else {
				$('#attIdStr').html('<div class="li-div"><span>暂无附件</span></div>');
			}

		},
		showFinalFileList: function(list) {
			var ls = '';
			$.each(list, function(i, n) {
				ls += '<div fId="' + n.attId + '" class="li-div"><span>' + n.attName + '</span><span class="del-file">x</span></div>';
			});
		...
		showBusinessFun: function() {
			...
			//加载区域页面
			sysCommon.loadingWord('view/projectManagement-sasac/expensesReim/module/reimApply/hospitalitySpending.html');
			if($('.prior-approval').val()) {
				//成功后显示导入按钮
		...
		showTravelFun: function() {
			...
			//$('.travel-div').show();
			//加载区域页面
			sysCommon.loadingWord('view/projectManagement-sasac/expensesReim/module/reimApply/travelSpending.html');
			if($('.prior-approval').val()) {
				//成功后显示导入按钮
				$('.index-main-div .advance-relevance').show();
				serveFunds.getAdvanceAppInfoDetailByAdvanceAppId(serveFunds.saveData.advanceAppId || '');
		...
		showTrainFun: function() {
			...
			$('#trainDetails').click(function() {
				...
				if($(".prior-approval").val() != '') {
					...
					//取得保存的支出申请单数据
					var list = me.expenditureInfoDetail.costData.trainApplyInfoList;
					//导入到列表中
					me.setTrainApplyInfoList(list, true);
				} else {
					$yt_alert_Model.prompt('事前申请单单据样式不符，无法导入');
			...
			$('#exportLecturer').click(function() {
				...
				if($(".prior-approval").val() != '') {
					...
					//取得保存的支出申请单数据
					var list = me.expenditureInfoDetail.costData.teacherApplyInfoList;
					//导入到列表中
					me.setTeacherApplyInfoList(list, true);
				} else {
					$yt_alert_Model.prompt('事前申请单单据样式不符，无法导入');
			...
			/*$('#exportCostApp').click(function() {
				...
				if($(".prior-approval").val() !='') {
					...
					//					me.setCostTrainApplyInfoList(data.costTrainApplyInfoList, true);
						//costTeachersFoodApplyInfoList	师资-伙食费json
						me.setCostTeachersFoodApplyInfoList(data.costExpenditureTeachersFoodApplyInfoList, true);
						//costTeachersLectureApplyInfoList	师资-讲课费json
						me.setCostTeachersLectureApplyInfoList(data.costExpenditureTeachersLectureApplyInfoList, true);
						//costTeachersTravelApplyInfoList	师资-城市间交通费json
						me.setCostTeachersTravelApplyInfoList(data.costExpenditureTeachersTravelApplyInfoList, true);
						//costTeachersHotelApplyInfoList	师资-住宿费 json
						me.setCostTeachersHotelApplyInfoList(data.costExpenditureTeachersHotelApplyInfoList, true);
				...
				}else if($(".prior-approval").val() =='' && $("#isExpenditureForm:checked").attr("name") == "test"){
					//取得保存的支出申请单数据
					var data = me.expenditureInfoDetail.costData;
					//costTeachersFoodApplyInfoList	师资-伙食费json
					me.setCostTeachersFoodApplyInfoList(data.costTeachersFoodApplyInfoList, true);
			...
			$('#exportPredictCost').click(function() {
				...
				if($(".prior-approval").val() != '') {
					...
					//取得保存的支出申请单数据
					var data = me.expenditureInfoDetail.costData.costPredictInfoList;
					//导入到列表中
					me.setCostPredictInfoList(data, true);
					//计算总金额
					personnelFunds.updateTotalNum();
				} else {
					$yt_alert_Model.prompt('事前申请单单据样式不符，无法导入');
			...
			$('#importTrainingOther').click(function() {
				...
				if($(".prior-approval").val() != '') {
					...
					//me.setTeacherApplyInfoList(list);
					//costTrainApplyInfoList	师资-培训费json
					//me.setCostTrainApplyInfoList(data.costExpenditureTrainApplyInfoList, true);
		...
		getAdvanceAppInfoDetailByAdvanceAppId: function(id, upDate) {
			$.ajax({
				type: "post",
				url: "sz/advanceApp/getFinalExpenditureAppInfoDetail",
				async: false,
				data: {
					advanceAppId: id
				},
				success: function(data) {
					...
					if(data.flag == 0) {
						//如果是预算做过变更
						var advanceAppId = "";
						if(data.data.advanceAppIdUpdate != '') {
							advanceAppId = data.data.advanceAppIdUpdate;
						} else {
							advanceAppId = data.data.advanceAppId;
						}
						$('#advanceAppId').text(advanceAppId);
						if(data.data.costData.costPredictInfoList.length > 0) {
							$('#predictTotal').text(data.data.costData.costPredictInfoList[0].averageMoney);
						}
						...
						$('.from-table-detail').setDatas(data.data.costData.trainApplyInfoList[0]);
						serveFunds.expenseClassDetails = data.data.costData.expenseClassDetails;
						serveFunds.expenseDetails = data.data.costData.expenseDetails;
						//师资-课酬费
						serveFunds.setTeacherClassExpenseDetails(data.data.costData.expenseClassDetails);
						//师资-差旅费
						serveFunds.setTeacherTrainDetails(data.data.costData.expenseDetails);
						//costTrainApplyInfoList	师资-培训费json
						serveFunds.setCostTrainApplyInfoList(data.data.costData.costExpenditureTrainApplyInfoList);
						//如果选中事前申请单，则表头自动填充信息
//						serveFunds.advanceDataFilling(data.data);
						//如果基本信息中有选择的事前审批单，将事前审批单中的相关附件中的文件导入，导入的文件不可删除
//						serveFunds.exportBeforeFiles(data.data.attList);
						/*//获取预算项目可用余额
						$("#availableBalanceOfProjectBudgetSheet").text($yt_baseElement.fmMoney(data.data.advanceAmount));*/
					}
				}
			});
		...
		setOtherExpenses: function(list) {
			...
			for(var i = 0; i < arr.length; i++) {
				var ai = arr[i];
				//判断map对象中是否有相同的值
				if(!map[ai.trainTypeName]) {
					dest.push({
						averageMoney: ai.averageMoney,
						createTime: ai.createTime,
						createUser: ai.createUser,
						id: ai.id,
						reason: ai.reason,
						remark: ai.remark,
						setMethod: ai.setMethod,
						standard: ai.standard,
						teachersTrainId: ai.teachersTrainId,
						trainAmount: ai.trainAmount,
						trainDays: ai.trainDays,
						trainOfNum: ai.trainOfNum,
						trainType: ai.trainType,
						trainTypeName: ai.trainTypeName,
						updateTime: ai.updateTime,
						updateUser: ai.updateUser
					});
					map[ai.trainTypeName] = ai;
				} else {
					//判断是否是其他费用
					if(ai.trainTypeName == "其他费用") {
						dest.push(ai);
					} else {
						//不是其他费用合并数据
						for(var j = 0; j < dest.length; j++) {
							var dj = dest[j];
							if(dj.trainTypeName == ai.trainTypeName) {
								dj.averageMoney = parseFloat(dj.averageMoney) + parseFloat(ai.averageMoney) + '.00';
								dj.remark = dj.remark;
								dj.standard = "0.00";
								dj.trainDays = "0";
								dj.trainOfNum = "0";
								dj.trainTypeName = dj.trainTypeName;
								break;
							}
						}
					}
...
//					'<td class="day-num">' + n.trainDays + '</td>'+
		 ...
		 * @param {Object} list
		 */
		setCostTeachersFoodApplyInfoList: function(list, exports, zcTr) {
			var html = '';
			var total = 0;
			$.each(list, function(i, n) {
				html += '<tr class="' + (exports ? 'exports' : '') + '  ' + (zcTr == undefined ? '' : 'zcTr') + '" pid="' + n.foodId + '">' +
		 ...
		 * @param {Object} list
		 */
		setCostTeachersLectureApplyInfoList: function(list, exports, zcTr) {
			var html = '';
			var total = 0;
			var afterTotal = 0;
			$.each(list, function(i, n) {
				html += '<tr class="' + (exports ? 'exports' : '') + '  ' + (zcTr == undefined ? '' : 'zcTr') + '" pid="' + n.teachersLectureId + '">' +
				...
				//afterTotal += $yt_baseElement.rmoney(n.afterTaxAmount);
			});
			//html += '<tr class="last"><td>合计</td><td></td><td></td><td class="total-food"  style="text-align:right;">' + serveFunds.fmMoney(totalFood) + '</td><td class="total-traffic"  style="text-align:right;">' + serveFunds.fmMoney(totalTraffic) + '</td></tr>';
			...
			//			});
			$('#lectureFeeTable tbody tr:not(.end-tr)').each(function(i, n) {
				afterTotal += $yt_baseElement.rmoney($(n).find('.after').text());
			});
			$('#lectureFeeTable tbody .costTotal-after').text(serveFunds.fmMoney(afterTotal));
			$('#lectureFeeTable tbody tr:not(.end-tr)').each(function(i, n) {
				total += $yt_baseElement.rmoney($(n).find('.sum-pay').text());
			});
		 ...
		 * @param {Object} list
		 */
		setCostTeachersTravelApplyInfoList: function(list, exports, zcTr) {
			var html = '';
			var total = 0;
			$.each(list, function(i, n) {
				html += '<tr class="' + (exports ? 'exports' : '') + '  ' + (zcTr == undefined ? '' : 'zcTr') + '" pid="' + n.teachersTravelId + '">' +
					'<td class="uname"><input type="hidden" class="lectureId" value="' + n.lecturerId + '">' + n.lecturerName + '</td>' +
					...
					'<td style="text-align: left !important;" class="dec"><input type="hidden" class="popM" value="1"/>' + (n.remarks || '') + '</td>' +
					/*'<span class="operate-update"><img src="../../../../../resources-sasac/images/common/edit-icon.png"></span>' +
					'<span class="operate-del"><img src="../../../../../resources-sasac/images/common/del-icon.png"></span>' +
					'</td>*/
					'</tr>';
		 ...
		 * @param {Object} list
		 */
		setCostTeachersHotelApplyInfoList: function(list, exports, zcTr) {
			var html = '';
			var total = 0;
			$.each(list, function(i, n) {
				html += '<tr class="' + (exports ? 'exports' : '') + '  ' + (zcTr == undefined ? '' : 'zcTr') + '" pid="' + n.teachersHotelId + '" level="' + n.lecturerLevelName + '">' +
					'<td class="name"><input type="hidden" class="lectureId" value="' + n.lecturerId + '">' + n.lecturerName + '</td>' +
					'<td class="sdate">' + n.startTime + '</td>' +
					'<td class="edate">' + n.endTime + '</td>' +
					'<td class="moneyText avg">' + serveFunds.fmMoney(n.averageMoney) + '</td>' +
		...
		setMeetingList: function(list, exports) {
			$.each(list, function(i, n) {
				...
				if(n.meetDays) {
					$('#calculationSession').text(n.meetDays).css('color', '#333').removeClass('calculation-Identification'); //meetDays 会期
				}
		...
		setCostPredictInfoList: function(list, exports) {
			...
			$.each(list, function(i, n) {
				html += '<tr class="' + (exports ? 'exports' : '') + '" pid="' + n.teachersTrainId + '">' +
					...
					'<td class="moneyText predict-smallplan-money sum-pay">' + serveFunds.fmMoney(n.averageMoney) + '</td>' +
			...
			html += '<tr class="last"><td>合计</td><td></td><td></td><td class="costTotal-after" style="text-align: right;display: none;">'+ $yt_baseElement.fmMoney(costTotalAfter) +'</td><td class="costTotal" style="text-align: right;">'+ $yt_baseElement.fmMoney(costTotal) +'</td><td></td><td></td></tr>'
			//$('#predictCostTable tbody tr.last').find('.costTotal-after').text()
		...
		clearPriorApproval: function() {
			...
			switch(costType) {
				...
				case 'TRAIN_APPLY': //培训费支出
					...
					var lectureSum = 0;
					$("#expenseDetails tbody tr:not(.end-tr)").remove();
					$("#expenseDetails tbody tr:not(.end-tr)").each(function() {
						var thisObj = $(this);
						lectureSum += $yt_baseElement.rmoney(thisObj.find(".sum-pay").text());
					});
					$("#expenseDetails .costTotal,#expenseDetails .costTotal-before").text($yt_baseElement.fmMoney(lectureSum));
					$("#expenseDetails .costTotal-after").text($yt_baseElement.fmMoney(lectureSum));
					var hotelSum = 0;
					$("#hotelFeeTable tbody tr:not(.end-tr)").each(function() {
						var thisObj = $(this);

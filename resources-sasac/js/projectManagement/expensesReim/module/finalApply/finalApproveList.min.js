				...
				.pageInfo(
						{
							...
							pageNum : 15,
							pageSize : 10,
							url : 'sz/advanceApp/getUserAdvanceAppInfoWFListToPageByParams',
							data : {
												...
												.each(
														...
														function(i, n) {
															trStr += '<tr><td><a class="yt-link to-detail">'
																	...
																	+ n.processInstanceId
																	+ '"/></td><td class="tl"><div style="white-space: normal;word-break: break-all;word-wrap: break-word;">'
																	+ (n.advanceAppName == "" ? "--"
																			: n.advanceAppName)
																	+ '</div></td><td style="text-align: right;">'
																	+ (n.advanceAmount == "" ? "--"
																			: $yt_baseElement
																					.fmMoney(n.advanceAmount))
																	+ '</td>'
																	+ '<td>'
																	+ n.applicantUserName
																	+ '</td><td><span>'
																	...
																	+ n.stagnationTime
																	+ '</td>'
																	+ '<td><a class="yt-link apprv-btn" taskKey="'
																	+ n.taskKey
																	+ '">'
																	+ (n.taskKey == 'activitiStartTask' ? '处理'
																			: '审批')
																	+ '</a><span class="center-line">|</span><a class="yt-link log-mod">日志</a></td></tr>';
															trStr = $(trStr)
																	.data(
																			"dataStr",
																			n);
		...
		$('.yt-link.to-detail')
				...
				.on(
						...
						function() {
							...
							var processInstanceId = $(this).parents('tr').find(
									'.processInstanceId').val();
							var advanceId = $(this).parents('tr').find(
									'.hid-advance-id').val();
							var fun = $(this).attr('fun');
							var pageUrl = "view/system-sasac/expensesReim/module/beforehand/beforeApplyDetail.html?processInstanceId="
									+ processInstanceId
									+ "&advanceId="
									+ advanceId + "&fun=" + fun;
							$yt_baseElement.openNewPage(2, pageUrl)
										...
										$
												.each(
														...
														function(i, n) {
															...
															trStr += '<td class="app-state-td"><span class="">'
																	+ (n.changeState == 0 ? '未修改'
																			: '已修改')
																	+ '</span></td><td><a class="yt-link log-mod">日志</a></td></tr>';
															trStr = $(trStr)
																	.data(
																			"dataStr",
																			n);
				...
				.click(
						function() {
							...
							if (state == 'activitiStartTask') {
								pageUrl = 'view/system-sasac/expensesReim/module/busiTripApply/serveApply.html?advanceId='
										+ advanceId
							} else {
								pageUrl = 'view/system-sasac/expensesReim/module/busiTripApply/priorApproval.html?advanceId='
										+ advanceId + '&fun=' + fun
							}

		...
		getReimAppInfoDetailByReimAppId: function(advanceAppId) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					//调用获取审批流程数据方法
					sysCommon.getApproveFlowData("SZ_EXPENDITURE_APP", d.processInstanceId);
					//数据回显
					me.showDetail(d);
					//判断当前节点
...
// 
		...
		gatheringUnitDetail: function(list) {
			...
			$.each(list, function(i, n) {
				$('.pay-detail-tabel .last').before('<tr class="pay-row" pkId="' + n.unitId + '" ctype="GATHERING_UNIT">' +
					...
					'<td><input class="yt-input money-input trans" placeholder="请输入" value="' + (serveExamine.fmMoney(n.amount)) + '"></td>' +
					'<td class="row-total">' + (serveExamine.fmMoney(n.amount)) + '</td>' +
					'</tr>');
		...
		gatheringPersonDetail: function(list) {
			...
			$.each(list, function(i, n) {
				$('.pay-detail-tabel .last').before('<tr class="pay-row" pkId="' + n.personalId + '" ctype="GATHERING_PERSON">' +
					...
					'<td class="write" style="text-align:right;padding-right:5px;">' + serveExamine.fmMoney(n.writeOffAmount) + '</td>' +
		...
		showDetail: function(d) {
			...
			$('#costTypeName').text(d.costTypeName); //单据样式
			//培训费提示文字
			if(d.costType == "TRAIN_APPLY") {
				$("#noContain").show();
			} else {
			...
			if(list != 0) {
				$.each(list, function(i, n) {
					html += '<tr pid="' + n.trainAppId + '" trainType="' + n.trainType + '">' +
						...
						'<td style="text-align: center" class="trainingPerNumber">' + (n.remark || '无') + '</td>' +
						'</tr>';
		...
		setCostTeachersLectureApplyInfoList: function(list) {
			...
			if(list != 0) {
				...
				var costTotal = $yt_baseElement.rmoney($("#lectureFeeTable .costTotal")
						.text());
				var costTotalAfter = $yt_baseElement.rmoney($(
						"#lectureFeeTable .costTotal-after").text());
				var taxAmount = Number(costTotal-costTotalAfter);
				
				$("#taxAmount").text($yt_baseElement.fmMoney(taxAmount || '0')).attr(
						'num', taxAmount);
				$("#payTeacherAll").text($yt_baseElement.fmMoney(taxAmount || '0')).attr(
						'num', taxAmount);

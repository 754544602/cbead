(function($, window) {
	var serveFinal = {
		...
		init: function() {
			var ts = this;
			serveFinal.appId = $yt_common.GetQueryString('appId'); //决算申请id
...
//				}
				...
				if($(this).val() == "printExpend") {
					pageUrl = "view/projectManagement-sasac/expensesReim/module/print/finalStatement.html?finalAppId=" + serveFinal.appId+"&costType="+serveFinal.costType;
				}
				//打印支出凭单明细
...
//				}
			 ...
			 //编辑页面跳转
	        $("#finalEdit").on("click",function(){
				 //即将跳转页面
				var pageUrl=$yt_option.websit_path+'view/projectManagement-sasac/expensesReim/module/finalApply/finalAccount.html?appId='+serveFinal.appId;
				//调用公用的跳转方法
...
//			/$('#expenditureAppNum').text(d.expenditureAppNumList); //支出申请编号
...
//			}
			...
			//获取流程日志
			var flowLogHtml = sysCommon.getCommentByProcessInstanceId(serveFinal.processInstanceId);
...
//			}
		...
		setCostPredictInfoList: function(list) {
			...
			$.each(list, function(i, n) {
				html += '<tr  pid="">' +
						...
						'<td class="predict-people-num">' + $yt_baseElement.fmMoney(n.predictPeopleNum) + '</td>' +
						'<td class="moneyText predict-smallplan-money">' + $yt_baseElement.fmMoney(n.averageMoney) + '</td>' +
		...
		showGeneralFun: function() {
			...
			$('.index-main-div').show();
			//加载区域页面
			sysCommon.loadingWord('view/projectManagement-sasac/expensesReim/module/reimApply/costDetailApproval.html');
		},
		/**
		 * 差旅
		 */
		...
		clearexamine: function() {
			$("#clearBtn").off().on("click", function() {
				$yt_common.parentAction({
					...
					data: {
						url: 'view/projectManagement-sasac/expensesReim/module/reimApply/reimApproveList.html' //要跳转的页面路径
					}
				});
			});
		},

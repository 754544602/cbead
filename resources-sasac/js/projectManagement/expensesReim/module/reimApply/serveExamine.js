(function($, window) {
	var serveExamine = {
		...
		events: function() {
			...
			$('#submitPayment').on('click', function() {
				...
				if($yt_valid.validForm($('.approve-div'))) {
					//补领金额
					var cmReplaceMoney = me.rmoney($('#cmReplaceMoney').text());
					var total = me.rmoney($('.payroll-working .amount-table .total').text());
					//支付金额
					var paymentAmount = me.rmoney($('#paymentAmount').val());
					if($(".payroll-working").is(':visible') && paymentAmount != cmReplaceMoney){
						$yt_alert_Model.prompt('补领方式金额合计必须等于补领金额');
					} else if ($(".payroll-working").is(':visible') && total != cmReplaceMoney){
						//支付明细中的支付金额数值必须等于补领金额，如果不等于，提示信息“支付金额不等于补领金额”
						$yt_alert_Model.prompt('支付金额不等于补领金额');
					} else {
						//获取提交数据
						var d = me.getSubData();
						//获取存储数据
		...
		showDetail: function(d) {
			...
			//paymentAmount	支付金额
			$('#paymentAmount').val(d.paymentAmount);
		...
		submitReimAppInfo: function(subData,thisBtn) {
			$.ajax({
				...
				success: function(data) {
					...
					if(data.flag == 0) {
						//跳转列表页面
						window.location.href = $yt_option.websit_path + 'view/system-sasac/expensesReim/module/reimApply/reimApproveList.html?state=1';
					}
					thisBtn.attr('disabled',false).removeClass('btn-disabled');
				}
			});
